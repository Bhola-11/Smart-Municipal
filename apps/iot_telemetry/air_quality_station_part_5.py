...
Module: Urban Environmental AQI & PM2.5 Monitors (Part 5)
...
class AirQualityStationService5Engine:
    """
...
class AirQualityStationService5Workflow:
    """
    ...
    def __init__(self, record: AirQualityStationService5Record):
        self.record = record
...
class AirQualityStationService5Manager:
    """
    Service registry and data aggregator for Urban Environmental AQI & PM2.5 Monitors.
    """
    def __init__(self):
        self.store: Dict[str, AirQualityStationService5Record] = {}
        self.engine = AirQualityStationService5Engine()

    def create(self, op_index: float = 50.0) -> AirQualityStationService5Record:
        rec = AirQualityStationService5Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[AirQualityStationService5Record]:
...
def test_subsystem_air_quality_station_5() -> bool:
    mgr = AirQualityStationService5Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = AirQualityStationService5Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_air_quality_station_5()
    print(f"air_quality_station Part 5: {'PASSED' if ok else 'FAILED'}")
