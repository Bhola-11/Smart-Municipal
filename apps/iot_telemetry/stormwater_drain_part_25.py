...
Module: Stormwater Sump & Culvert Flood Monitors (Part 25)
...
class StormwaterDrainService25Engine:
    """
...
class StormwaterDrainService25Workflow:
    """
    ...
    def __init__(self, record: StormwaterDrainService25Record):
        self.record = record
...
class StormwaterDrainService25Manager:
    """
    Service registry and data aggregator for Stormwater Sump & Culvert Flood Monitors.
    """
    def __init__(self):
        self.store: Dict[str, StormwaterDrainService25Record] = {}
        self.engine = StormwaterDrainService25Engine()

    def create(self, op_index: float = 50.0) -> StormwaterDrainService25Record:
        rec = StormwaterDrainService25Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[StormwaterDrainService25Record]:
...
def test_subsystem_stormwater_drain_25() -> bool:
    mgr = StormwaterDrainService25Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = StormwaterDrainService25Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_stormwater_drain_25()
    print(f"stormwater_drain Part 25: {'PASSED' if ok else 'FAILED'}")
