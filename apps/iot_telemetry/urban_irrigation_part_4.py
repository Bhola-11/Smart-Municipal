...
Module: Municipal Parks Soil Moisture Telemetry (Part 4)
...
class UrbanIrrigationService4Engine:
    """
...
class UrbanIrrigationService4Workflow:
    """
    ...
    def __init__(self, record: UrbanIrrigationService4Record):
        self.record = record
...
class UrbanIrrigationService4Manager:
    """
    Service registry and data aggregator for Municipal Parks Soil Moisture Telemetry.
    """
    def __init__(self):
        self.store: Dict[str, UrbanIrrigationService4Record] = {}
        self.engine = UrbanIrrigationService4Engine()

    def create(self, op_index: float = 50.0) -> UrbanIrrigationService4Record:
        rec = UrbanIrrigationService4Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[UrbanIrrigationService4Record]:
...
def test_subsystem_urban_irrigation_4() -> bool:
    mgr = UrbanIrrigationService4Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = UrbanIrrigationService4Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_urban_irrigation_4()
    print(f"urban_irrigation Part 4: {'PASSED' if ok else 'FAILED'}")
