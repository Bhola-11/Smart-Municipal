...
Module: Drinking Water Chlorination Analyzers (Part 1)
...
class PotableChlorineService1Engine:
    """
...
class PotableChlorineService1Workflow:
    """
    ...
    def __init__(self, record: PotableChlorineService1Record):
        self.record = record
...
class PotableChlorineService1Manager:
    """
    Service registry and data aggregator for Drinking Water Chlorination Analyzers.
    """
    def __init__(self):
        self.store: Dict[str, PotableChlorineService1Record] = {}
        self.engine = PotableChlorineService1Engine()

    def create(self, op_index: float = 50.0) -> PotableChlorineService1Record:
        rec = PotableChlorineService1Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[PotableChlorineService1Record]:
...
def test_subsystem_potable_chlorine_1() -> bool:
    mgr = PotableChlorineService1Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = PotableChlorineService1Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_potable_chlorine_1()
    print(f"potable_chlorine Part 1: {'PASSED' if ok else 'FAILED'}")
