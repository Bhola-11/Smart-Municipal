...
Module: Drinking Water Chlorination Analyzers (Part 8)
...
class PotableChlorineService8Engine:
    """
...
class PotableChlorineService8Workflow:
    """
    ...
    def __init__(self, record: PotableChlorineService8Record):
        self.record = record
...
class PotableChlorineService8Manager:
    """
    Service registry and data aggregator for Drinking Water Chlorination Analyzers.
    """
    def __init__(self):
        self.store: Dict[str, PotableChlorineService8Record] = {}
        self.engine = PotableChlorineService8Engine()

    def create(self, op_index: float = 50.0) -> PotableChlorineService8Record:
        rec = PotableChlorineService8Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[PotableChlorineService8Record]:
...
def test_subsystem_potable_chlorine_8() -> bool:
    mgr = PotableChlorineService8Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = PotableChlorineService8Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_potable_chlorine_8()
    print(f"potable_chlorine Part 8: {'PASSED' if ok else 'FAILED'}")
