...
Module: Urban Environmental AQI & PM2.5 Monitors (Part 6)
...
class AirQualityStationService6Engine:
    """
...
class AirQualityStationService6Workflow:
    """
    ...
    def __init__(self, record: AirQualityStationService6Record):
        self.record = record
...
class AirQualityStationService6Manager:
    """
    Service registry and data aggregator for Urban Environmental AQI & PM2.5 Monitors.
    """
    def __init__(self):
        self.store: Dict[str, AirQualityStationService6Record] = {}
        self.engine = AirQualityStationService6Engine()

    def create(self, op_index: float = 50.0) -> AirQualityStationService6Record:
        rec = AirQualityStationService6Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[AirQualityStationService6Record]:
...
def test_subsystem_air_quality_station_6() -> bool:
    mgr = AirQualityStationService6Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = AirQualityStationService6Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_air_quality_station_6()
    print(f"air_quality_station Part 6: {'PASSED' if ok else 'FAILED'}")
