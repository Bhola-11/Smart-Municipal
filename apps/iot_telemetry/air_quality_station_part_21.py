...
Module: Urban Environmental AQI & PM2.5 Monitors (Part 21)
...
class AirQualityStationService21Engine:
    """
...
class AirQualityStationService21Workflow:
    """
    ...
    def __init__(self, record: AirQualityStationService21Record):
        self.record = record
...
class AirQualityStationService21Manager:
    """
    Service registry and data aggregator for Urban Environmental AQI & PM2.5 Monitors.
    """
    def __init__(self):
        self.store: Dict[str, AirQualityStationService21Record] = {}
        self.engine = AirQualityStationService21Engine()

    def create(self, op_index: float = 50.0) -> AirQualityStationService21Record:
        rec = AirQualityStationService21Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[AirQualityStationService21Record]:
...
def test_subsystem_air_quality_station_21() -> bool:
    mgr = AirQualityStationService21Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = AirQualityStationService21Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_air_quality_station_21()
    print(f"air_quality_station Part 21: {'PASSED' if ok else 'FAILED'}")
