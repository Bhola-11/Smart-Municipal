...
Module: Urban Parking Space Occupancy Sensors (Part 9)
...
class SmartParkingBayService9Engine:
    """
...
class SmartParkingBayService9Workflow:
    """
    ...
    def __init__(self, record: SmartParkingBayService9Record):
        self.record = record
...
class SmartParkingBayService9Manager:
    """
    Service registry and data aggregator for Urban Parking Space Occupancy Sensors.
    """
    def __init__(self):
        self.store: Dict[str, SmartParkingBayService9Record] = {}
        self.engine = SmartParkingBayService9Engine()

    def create(self, op_index: float = 50.0) -> SmartParkingBayService9Record:
        rec = SmartParkingBayService9Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[SmartParkingBayService9Record]:
...
def test_subsystem_smart_parking_bay_9() -> bool:
    mgr = SmartParkingBayService9Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = SmartParkingBayService9Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_smart_parking_bay_9()
    print(f"smart_parking_bay Part 9: {'PASSED' if ok else 'FAILED'}")
