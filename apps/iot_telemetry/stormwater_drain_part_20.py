...
Module: Stormwater Sump & Culvert Flood Monitors (Part 20)
...
class StormwaterDrainService20Engine:
    """
...
class StormwaterDrainService20Workflow:
    """
    ...
    def __init__(self, record: StormwaterDrainService20Record):
        self.record = record
...
class StormwaterDrainService20Manager:
    """
    Service registry and data aggregator for Stormwater Sump & Culvert Flood Monitors.
    """
    def __init__(self):
        self.store: Dict[str, StormwaterDrainService20Record] = {}
        self.engine = StormwaterDrainService20Engine()

    def create(self, op_index: float = 50.0) -> StormwaterDrainService20Record:
        rec = StormwaterDrainService20Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[StormwaterDrainService20Record]:
...
def test_subsystem_stormwater_drain_20() -> bool:
    mgr = StormwaterDrainService20Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = StormwaterDrainService20Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_stormwater_drain_20()
    print(f"stormwater_drain Part 20: {'PASSED' if ok else 'FAILED'}")
