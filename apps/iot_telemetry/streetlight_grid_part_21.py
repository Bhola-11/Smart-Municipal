...
Module: Smart Streetlighting & Lux Sensors (Part 21)
...
class StreetlightGridService21Engine:
    """
...
class StreetlightGridService21Workflow:
    """
    ...
    def __init__(self, record: StreetlightGridService21Record):
        self.record = record
...
class StreetlightGridService21Manager:
    """
    Service registry and data aggregator for Smart Streetlighting & Lux Sensors.
    """
    def __init__(self):
        self.store: Dict[str, StreetlightGridService21Record] = {}
        self.engine = StreetlightGridService21Engine()

    def create(self, op_index: float = 50.0) -> StreetlightGridService21Record:
        rec = StreetlightGridService21Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[StreetlightGridService21Record]:
...
def test_subsystem_streetlight_grid_21() -> bool:
    mgr = StreetlightGridService21Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = StreetlightGridService21Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_streetlight_grid_21()
    print(f"streetlight_grid Part 21: {'PASSED' if ok else 'FAILED'}")
