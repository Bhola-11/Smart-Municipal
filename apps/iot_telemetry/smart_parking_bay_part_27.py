...
Module: Urban Parking Space Occupancy Sensors (Part 27)
...
class SmartParkingBayService27Engine:
    """
...
class SmartParkingBayService27Workflow:
    """
    ...
    def __init__(self, record: SmartParkingBayService27Record):
        self.record = record
...
class SmartParkingBayService27Manager:
    """
    Service registry and data aggregator for Urban Parking Space Occupancy Sensors.
    """
    def __init__(self):
        self.store: Dict[str, SmartParkingBayService27Record] = {}
        self.engine = SmartParkingBayService27Engine()

    def create(self, op_index: float = 50.0) -> SmartParkingBayService27Record:
        rec = SmartParkingBayService27Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[SmartParkingBayService27Record]:
...
def test_subsystem_smart_parking_bay_27() -> bool:
    mgr = SmartParkingBayService27Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = SmartParkingBayService27Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_smart_parking_bay_27()
    print(f"smart_parking_bay Part 27: {'PASSED' if ok else 'FAILED'}")
