...
Module: Drinking Water Chlorination Analyzers (Part 20)
...
class PotableChlorineService20Engine:
    """
...
class PotableChlorineService20Workflow:
    """
    ...
    def __init__(self, record: PotableChlorineService20Record):
        self.record = record
...
class PotableChlorineService20Manager:
    """
    Service registry and data aggregator for Drinking Water Chlorination Analyzers.
    """
    def __init__(self):
        self.store: Dict[str, PotableChlorineService20Record] = {}
        self.engine = PotableChlorineService20Engine()

    def create(self, op_index: float = 50.0) -> PotableChlorineService20Record:
        rec = PotableChlorineService20Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[PotableChlorineService20Record]:
...
def test_subsystem_potable_chlorine_20() -> bool:
    mgr = PotableChlorineService20Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = PotableChlorineService20Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_potable_chlorine_20()
    print(f"potable_chlorine Part 20: {'PASSED' if ok else 'FAILED'}")
