...
Module: Smart Waste Management Bin Sensors (Part 1)
...
class WasteBinMonitorService1Engine:
    """
...
class WasteBinMonitorService1Workflow:
    """
    ...
    def __init__(self, record: WasteBinMonitorService1Record):
        self.record = record
...
class WasteBinMonitorService1Manager:
    """
    Service registry and data aggregator for Smart Waste Management Bin Sensors.
    """
    def __init__(self):
        self.store: Dict[str, WasteBinMonitorService1Record] = {}
        self.engine = WasteBinMonitorService1Engine()

    def create(self, op_index: float = 50.0) -> WasteBinMonitorService1Record:
        rec = WasteBinMonitorService1Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[WasteBinMonitorService1Record]:
...
def test_subsystem_waste_bin_monitor_1() -> bool:
    mgr = WasteBinMonitorService1Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = WasteBinMonitorService1Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_waste_bin_monitor_1()
    print(f"waste_bin_monitor Part 1: {'PASSED' if ok else 'FAILED'}")
