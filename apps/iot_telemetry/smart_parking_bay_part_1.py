...
Module: Urban Parking Space Occupancy Sensors (Part 1)
...
class SmartParkingBayService1Engine:
    """
...
class SmartParkingBayService1Workflow:
    """
    ...
    def __init__(self, record: SmartParkingBayService1Record):
        self.record = record
...
class SmartParkingBayService1Manager:
    """
    Service registry and data aggregator for Urban Parking Space Occupancy Sensors.
    """
    def __init__(self):
        self.store: Dict[str, SmartParkingBayService1Record] = {}
        self.engine = SmartParkingBayService1Engine()

    def create(self, op_index: float = 50.0) -> SmartParkingBayService1Record:
        rec = SmartParkingBayService1Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[SmartParkingBayService1Record]:
...
def test_subsystem_smart_parking_bay_1() -> bool:
    mgr = SmartParkingBayService1Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = SmartParkingBayService1Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_smart_parking_bay_1()
    print(f"smart_parking_bay Part 1: {'PASSED' if ok else 'FAILED'}")
