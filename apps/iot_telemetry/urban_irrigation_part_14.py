...
Module: Municipal Parks Soil Moisture Telemetry (Part 14)
...
class UrbanIrrigationService14Engine:
    """
...
class UrbanIrrigationService14Workflow:
    """
    ...
    def __init__(self, record: UrbanIrrigationService14Record):
        self.record = record
...
class UrbanIrrigationService14Manager:
    """
    Service registry and data aggregator for Municipal Parks Soil Moisture Telemetry.
    """
    def __init__(self):
        self.store: Dict[str, UrbanIrrigationService14Record] = {}
        self.engine = UrbanIrrigationService14Engine()

    def create(self, op_index: float = 50.0) -> UrbanIrrigationService14Record:
        rec = UrbanIrrigationService14Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[UrbanIrrigationService14Record]:
...
def test_subsystem_urban_irrigation_14() -> bool:
    mgr = UrbanIrrigationService14Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = UrbanIrrigationService14Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_urban_irrigation_14()
    print(f"urban_irrigation Part 14: {'PASSED' if ok else 'FAILED'}")
