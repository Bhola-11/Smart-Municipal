...
Module: Urban Environmental AQI & PM2.5 Monitors (Part 14)
...
class AirQualityStationService14Engine:
    """
...
class AirQualityStationService14Workflow:
    """
    ...
    def __init__(self, record: AirQualityStationService14Record):
        self.record = record
...
class AirQualityStationService14Manager:
    """
    Service registry and data aggregator for Urban Environmental AQI & PM2.5 Monitors.
    """
    def __init__(self):
        self.store: Dict[str, AirQualityStationService14Record] = {}
        self.engine = AirQualityStationService14Engine()

    def create(self, op_index: float = 50.0) -> AirQualityStationService14Record:
        rec = AirQualityStationService14Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[AirQualityStationService14Record]:
...
def test_subsystem_air_quality_station_14() -> bool:
    mgr = AirQualityStationService14Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = AirQualityStationService14Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_air_quality_station_14()
    print(f"air_quality_station Part 14: {'PASSED' if ok else 'FAILED'}")
