...
Module: Municipal Water Supply Pressure & Flow (Part 14)
...
class WaterPressureNetworkService14Engine:
    """
...
class WaterPressureNetworkService14Workflow:
    """
    ...
    def __init__(self, record: WaterPressureNetworkService14Record):
        self.record = record
...
class WaterPressureNetworkService14Manager:
    """
    Service registry and data aggregator for Municipal Water Supply Pressure & Flow.
    """
    def __init__(self):
        self.store: Dict[str, WaterPressureNetworkService14Record] = {}
        self.engine = WaterPressureNetworkService14Engine()

    def create(self, op_index: float = 50.0) -> WaterPressureNetworkService14Record:
        rec = WaterPressureNetworkService14Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[WaterPressureNetworkService14Record]:
...
def test_subsystem_water_pressure_network_14() -> bool:
    mgr = WaterPressureNetworkService14Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = WaterPressureNetworkService14Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_water_pressure_network_14()
    print(f"water_pressure_network Part 14: {'PASSED' if ok else 'FAILED'}")
