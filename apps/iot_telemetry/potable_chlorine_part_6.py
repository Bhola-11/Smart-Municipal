...
Module: Drinking Water Chlorination Analyzers (Part 6)
...
class PotableChlorineService6Engine:
    """
...
class PotableChlorineService6Workflow:
    """
    ...
    def __init__(self, record: PotableChlorineService6Record):
        self.record = record
...
class PotableChlorineService6Manager:
    """
    Service registry and data aggregator for Drinking Water Chlorination Analyzers.
    """
    def __init__(self):
        self.store: Dict[str, PotableChlorineService6Record] = {}
        self.engine = PotableChlorineService6Engine()

    def create(self, op_index: float = 50.0) -> PotableChlorineService6Record:
        rec = PotableChlorineService6Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[PotableChlorineService6Record]:
...
def test_subsystem_potable_chlorine_6() -> bool:
    mgr = PotableChlorineService6Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = PotableChlorineService6Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_potable_chlorine_6()
    print(f"potable_chlorine Part 6: {'PASSED' if ok else 'FAILED'}")
