...
Module: Municipal Parks Soil Moisture Telemetry (Part 8)
...
class UrbanIrrigationService8Engine:
    """
...
class UrbanIrrigationService8Workflow:
    """
    ...
    def __init__(self, record: UrbanIrrigationService8Record):
        self.record = record
...
class UrbanIrrigationService8Manager:
    """
    Service registry and data aggregator for Municipal Parks Soil Moisture Telemetry.
    """
    def __init__(self):
        self.store: Dict[str, UrbanIrrigationService8Record] = {}
        self.engine = UrbanIrrigationService8Engine()

    def create(self, op_index: float = 50.0) -> UrbanIrrigationService8Record:
        rec = UrbanIrrigationService8Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[UrbanIrrigationService8Record]:
...
def test_subsystem_urban_irrigation_8() -> bool:
    mgr = UrbanIrrigationService8Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = UrbanIrrigationService8Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_urban_irrigation_8()
    print(f"urban_irrigation Part 8: {'PASSED' if ok else 'FAILED'}")
