...
Module: Municipal Parks Soil Moisture Telemetry (Part 27)
...
class UrbanIrrigationService27Engine:
    """
...
class UrbanIrrigationService27Workflow:
    """
    ...
    def __init__(self, record: UrbanIrrigationService27Record):
        self.record = record
...
class UrbanIrrigationService27Manager:
    """
    Service registry and data aggregator for Municipal Parks Soil Moisture Telemetry.
    """
    def __init__(self):
        self.store: Dict[str, UrbanIrrigationService27Record] = {}
        self.engine = UrbanIrrigationService27Engine()

    def create(self, op_index: float = 50.0) -> UrbanIrrigationService27Record:
        rec = UrbanIrrigationService27Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[UrbanIrrigationService27Record]:
...
def test_subsystem_urban_irrigation_27() -> bool:
    mgr = UrbanIrrigationService27Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = UrbanIrrigationService27Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_urban_irrigation_27()
    print(f"urban_irrigation Part 27: {'PASSED' if ok else 'FAILED'}")
