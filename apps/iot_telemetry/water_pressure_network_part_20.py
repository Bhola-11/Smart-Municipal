...
Module: Municipal Water Supply Pressure & Flow (Part 20)
...
class WaterPressureNetworkService20Engine:
    """
...
class WaterPressureNetworkService20Workflow:
    """
    ...
    def __init__(self, record: WaterPressureNetworkService20Record):
        self.record = record
...
class WaterPressureNetworkService20Manager:
    """
    Service registry and data aggregator for Municipal Water Supply Pressure & Flow.
    """
    def __init__(self):
        self.store: Dict[str, WaterPressureNetworkService20Record] = {}
        self.engine = WaterPressureNetworkService20Engine()

    def create(self, op_index: float = 50.0) -> WaterPressureNetworkService20Record:
        rec = WaterPressureNetworkService20Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[WaterPressureNetworkService20Record]:
...
def test_subsystem_water_pressure_network_20() -> bool:
    mgr = WaterPressureNetworkService20Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = WaterPressureNetworkService20Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_water_pressure_network_20()
    print(f"water_pressure_network Part 20: {'PASSED' if ok else 'FAILED'}")
