...
Module: Drinking Water Chlorination Analyzers (Part 12)
...
class PotableChlorineService12Engine:
    """
...
class PotableChlorineService12Workflow:
    """
    ...
    def __init__(self, record: PotableChlorineService12Record):
        self.record = record
...
class PotableChlorineService12Manager:
    """
    Service registry and data aggregator for Drinking Water Chlorination Analyzers.
    """
    def __init__(self):
        self.store: Dict[str, PotableChlorineService12Record] = {}
        self.engine = PotableChlorineService12Engine()

    def create(self, op_index: float = 50.0) -> PotableChlorineService12Record:
        rec = PotableChlorineService12Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[PotableChlorineService12Record]:
...
def test_subsystem_potable_chlorine_12() -> bool:
    mgr = PotableChlorineService12Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = PotableChlorineService12Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_potable_chlorine_12()
    print(f"potable_chlorine Part 12: {'PASSED' if ok else 'FAILED'}")
