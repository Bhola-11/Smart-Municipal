...
Module: Urban Environmental AQI & PM2.5 Monitors (Part 18)
...
class AirQualityStationService18Engine:
    """
...
class AirQualityStationService18Workflow:
    """
    ...
    def __init__(self, record: AirQualityStationService18Record):
        self.record = record
...
class AirQualityStationService18Manager:
    """
    Service registry and data aggregator for Urban Environmental AQI & PM2.5 Monitors.
    """
    def __init__(self):
        self.store: Dict[str, AirQualityStationService18Record] = {}
        self.engine = AirQualityStationService18Engine()

    def create(self, op_index: float = 50.0) -> AirQualityStationService18Record:
        rec = AirQualityStationService18Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[AirQualityStationService18Record]:
...
def test_subsystem_air_quality_station_18() -> bool:
    mgr = AirQualityStationService18Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = AirQualityStationService18Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_air_quality_station_18()
    print(f"air_quality_station Part 18: {'PASSED' if ok else 'FAILED'}")
