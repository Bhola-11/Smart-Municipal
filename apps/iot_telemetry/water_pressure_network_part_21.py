...
Module: Municipal Water Supply Pressure & Flow (Part 21)
...
class WaterPressureNetworkService21Engine:
    """
...
class WaterPressureNetworkService21Workflow:
    """
    ...
    def __init__(self, record: WaterPressureNetworkService21Record):
        self.record = record
...
class WaterPressureNetworkService21Manager:
    """
    Service registry and data aggregator for Municipal Water Supply Pressure & Flow.
    """
    def __init__(self):
        self.store: Dict[str, WaterPressureNetworkService21Record] = {}
        self.engine = WaterPressureNetworkService21Engine()

    def create(self, op_index: float = 50.0) -> WaterPressureNetworkService21Record:
        rec = WaterPressureNetworkService21Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[WaterPressureNetworkService21Record]:
...
def test_subsystem_water_pressure_network_21() -> bool:
    mgr = WaterPressureNetworkService21Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = WaterPressureNetworkService21Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_water_pressure_network_21()
    print(f"water_pressure_network Part 21: {'PASSED' if ok else 'FAILED'}")
