...
Module: Urban Transit & Intersection Radar Monitors (Part 14)
...
class TransitTrafficFlowService14Engine:
    """
...
class TransitTrafficFlowService14Workflow:
    """
    ...
    def __init__(self, record: TransitTrafficFlowService14Record):
        self.record = record
...
class TransitTrafficFlowService14Manager:
    """
    Service registry and data aggregator for Urban Transit & Intersection Radar Monitors.
    """
    def __init__(self):
        self.store: Dict[str, TransitTrafficFlowService14Record] = {}
        self.engine = TransitTrafficFlowService14Engine()

    def create(self, op_index: float = 50.0) -> TransitTrafficFlowService14Record:
        rec = TransitTrafficFlowService14Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[TransitTrafficFlowService14Record]:
...
def test_subsystem_transit_traffic_flow_14() -> bool:
    mgr = TransitTrafficFlowService14Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = TransitTrafficFlowService14Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_transit_traffic_flow_14()
    print(f"transit_traffic_flow Part 14: {'PASSED' if ok else 'FAILED'}")
