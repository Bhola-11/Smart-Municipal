...
Module: Urban Parking Space Occupancy Sensors (Part 8)
...
class SmartParkingBayService8Engine:
    """
...
class SmartParkingBayService8Workflow:
    """
    ...
    def __init__(self, record: SmartParkingBayService8Record):
        self.record = record
...
class SmartParkingBayService8Manager:
    """
    Service registry and data aggregator for Urban Parking Space Occupancy Sensors.
    """
    def __init__(self):
        self.store: Dict[str, SmartParkingBayService8Record] = {}
        self.engine = SmartParkingBayService8Engine()

    def create(self, op_index: float = 50.0) -> SmartParkingBayService8Record:
        rec = SmartParkingBayService8Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[SmartParkingBayService8Record]:
...
def test_subsystem_smart_parking_bay_8() -> bool:
    mgr = SmartParkingBayService8Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = SmartParkingBayService8Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_smart_parking_bay_8()
    print(f"smart_parking_bay Part 8: {'PASSED' if ok else 'FAILED'}")
