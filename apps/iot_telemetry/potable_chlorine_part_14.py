...
Module: Drinking Water Chlorination Analyzers (Part 14)
...
class PotableChlorineService14Engine:
    """
...
class PotableChlorineService14Workflow:
    """
    ...
    def __init__(self, record: PotableChlorineService14Record):
        self.record = record
...
class PotableChlorineService14Manager:
    """
    Service registry and data aggregator for Drinking Water Chlorination Analyzers.
    """
    def __init__(self):
        self.store: Dict[str, PotableChlorineService14Record] = {}
        self.engine = PotableChlorineService14Engine()

    def create(self, op_index: float = 50.0) -> PotableChlorineService14Record:
        rec = PotableChlorineService14Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[PotableChlorineService14Record]:
...
def test_subsystem_potable_chlorine_14() -> bool:
    mgr = PotableChlorineService14Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = PotableChlorineService14Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_potable_chlorine_14()
    print(f"potable_chlorine Part 14: {'PASSED' if ok else 'FAILED'}")
