...
Module: Smart Waste Management Bin Sensors (Part 21)
...
class WasteBinMonitorService21Engine:
    """
...
class WasteBinMonitorService21Workflow:
    """
    ...
    def __init__(self, record: WasteBinMonitorService21Record):
        self.record = record
...
class WasteBinMonitorService21Manager:
    """
    Service registry and data aggregator for Smart Waste Management Bin Sensors.
    """
    def __init__(self):
        self.store: Dict[str, WasteBinMonitorService21Record] = {}
        self.engine = WasteBinMonitorService21Engine()

    def create(self, op_index: float = 50.0) -> WasteBinMonitorService21Record:
        rec = WasteBinMonitorService21Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[WasteBinMonitorService21Record]:
...
def test_subsystem_waste_bin_monitor_21() -> bool:
    mgr = WasteBinMonitorService21Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = WasteBinMonitorService21Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_waste_bin_monitor_21()
    print(f"waste_bin_monitor Part 21: {'PASSED' if ok else 'FAILED'}")
