...
Module: Urban Environmental AQI & PM2.5 Monitors (Part 30)
...
class AirQualityStationService30Engine:
    """
...
class AirQualityStationService30Workflow:
    """
    ...
    def __init__(self, record: AirQualityStationService30Record):
        self.record = record
...
class AirQualityStationService30Manager:
    """
    Service registry and data aggregator for Urban Environmental AQI & PM2.5 Monitors.
    """
    def __init__(self):
        self.store: Dict[str, AirQualityStationService30Record] = {}
        self.engine = AirQualityStationService30Engine()

    def create(self, op_index: float = 50.0) -> AirQualityStationService30Record:
        rec = AirQualityStationService30Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[AirQualityStationService30Record]:
...
def test_subsystem_air_quality_station_30() -> bool:
    mgr = AirQualityStationService30Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = AirQualityStationService30Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_air_quality_station_30()
    print(f"air_quality_station Part 30: {'PASSED' if ok else 'FAILED'}")
