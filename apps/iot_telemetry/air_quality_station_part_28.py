...
Module: Urban Environmental AQI & PM2.5 Monitors (Part 28)
...
class AirQualityStationService28Engine:
    """
...
class AirQualityStationService28Workflow:
    """
    ...
    def __init__(self, record: AirQualityStationService28Record):
        self.record = record
...
class AirQualityStationService28Manager:
    """
    Service registry and data aggregator for Urban Environmental AQI & PM2.5 Monitors.
    """
    def __init__(self):
        self.store: Dict[str, AirQualityStationService28Record] = {}
        self.engine = AirQualityStationService28Engine()

    def create(self, op_index: float = 50.0) -> AirQualityStationService28Record:
        rec = AirQualityStationService28Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[AirQualityStationService28Record]:
...
def test_subsystem_air_quality_station_28() -> bool:
    mgr = AirQualityStationService28Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = AirQualityStationService28Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_air_quality_station_28()
    print(f"air_quality_station Part 28: {'PASSED' if ok else 'FAILED'}")
