...
Module: Urban Environmental AQI & PM2.5 Monitors (Part 8)
...
class AirQualityStationService8Engine:
    """
...
class AirQualityStationService8Workflow:
    """
    ...
    def __init__(self, record: AirQualityStationService8Record):
        self.record = record
...
class AirQualityStationService8Manager:
    """
    Service registry and data aggregator for Urban Environmental AQI & PM2.5 Monitors.
    """
    def __init__(self):
        self.store: Dict[str, AirQualityStationService8Record] = {}
        self.engine = AirQualityStationService8Engine()

    def create(self, op_index: float = 50.0) -> AirQualityStationService8Record:
        rec = AirQualityStationService8Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[AirQualityStationService8Record]:
...
def test_subsystem_air_quality_station_8() -> bool:
    mgr = AirQualityStationService8Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = AirQualityStationService8Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_air_quality_station_8()
    print(f"air_quality_station Part 8: {'PASSED' if ok else 'FAILED'}")
