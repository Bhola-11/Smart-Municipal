...
Module: Urban Parking Space Occupancy Sensors (Part 14)
...
class SmartParkingBayService14Engine:
    """
...
class SmartParkingBayService14Workflow:
    """
    ...
    def __init__(self, record: SmartParkingBayService14Record):
        self.record = record
...
class SmartParkingBayService14Manager:
    """
    Service registry and data aggregator for Urban Parking Space Occupancy Sensors.
    """
    def __init__(self):
        self.store: Dict[str, SmartParkingBayService14Record] = {}
        self.engine = SmartParkingBayService14Engine()

    def create(self, op_index: float = 50.0) -> SmartParkingBayService14Record:
        rec = SmartParkingBayService14Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[SmartParkingBayService14Record]:
...
def test_subsystem_smart_parking_bay_14() -> bool:
    mgr = SmartParkingBayService14Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = SmartParkingBayService14Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_smart_parking_bay_14()
    print(f"smart_parking_bay Part 14: {'PASSED' if ok else 'FAILED'}")
