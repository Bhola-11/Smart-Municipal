...
Module: Smart Streetlighting & Lux Sensors (Part 25)
...
class StreetlightGridService25Engine:
    """
...
class StreetlightGridService25Workflow:
    """
    ...
    def __init__(self, record: StreetlightGridService25Record):
        self.record = record
...
class StreetlightGridService25Manager:
    """
    Service registry and data aggregator for Smart Streetlighting & Lux Sensors.
    """
    def __init__(self):
        self.store: Dict[str, StreetlightGridService25Record] = {}
        self.engine = StreetlightGridService25Engine()

    def create(self, op_index: float = 50.0) -> StreetlightGridService25Record:
        rec = StreetlightGridService25Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[StreetlightGridService25Record]:
...
def test_subsystem_streetlight_grid_25() -> bool:
    mgr = StreetlightGridService25Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = StreetlightGridService25Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_streetlight_grid_25()
    print(f"streetlight_grid Part 25: {'PASSED' if ok else 'FAILED'}")
