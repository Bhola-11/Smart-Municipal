...
Module: Municipal Parks Soil Moisture Telemetry (Part 17)
...
class UrbanIrrigationService17Engine:
    """
...
class UrbanIrrigationService17Workflow:
    """
    ...
    def __init__(self, record: UrbanIrrigationService17Record):
        self.record = record
...
class UrbanIrrigationService17Manager:
    """
    Service registry and data aggregator for Municipal Parks Soil Moisture Telemetry.
    """
    def __init__(self):
        self.store: Dict[str, UrbanIrrigationService17Record] = {}
        self.engine = UrbanIrrigationService17Engine()

    def create(self, op_index: float = 50.0) -> UrbanIrrigationService17Record:
        rec = UrbanIrrigationService17Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[UrbanIrrigationService17Record]:
...
def test_subsystem_urban_irrigation_17() -> bool:
    mgr = UrbanIrrigationService17Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = UrbanIrrigationService17Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_urban_irrigation_17()
    print(f"urban_irrigation Part 17: {'PASSED' if ok else 'FAILED'}")
