...
Module: Drinking Water Chlorination Analyzers (Part 25)
...
class PotableChlorineService25Engine:
    """
...
class PotableChlorineService25Workflow:
    """
    ...
    def __init__(self, record: PotableChlorineService25Record):
        self.record = record
...
class PotableChlorineService25Manager:
    """
    Service registry and data aggregator for Drinking Water Chlorination Analyzers.
    """
    def __init__(self):
        self.store: Dict[str, PotableChlorineService25Record] = {}
        self.engine = PotableChlorineService25Engine()

    def create(self, op_index: float = 50.0) -> PotableChlorineService25Record:
        rec = PotableChlorineService25Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[PotableChlorineService25Record]:
...
def test_subsystem_potable_chlorine_25() -> bool:
    mgr = PotableChlorineService25Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = PotableChlorineService25Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_potable_chlorine_25()
    print(f"potable_chlorine Part 25: {'PASSED' if ok else 'FAILED'}")
