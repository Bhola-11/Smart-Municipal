...
Module: Urban Environmental AQI & PM2.5 Monitors (Part 25)
...
class AirQualityStationService25Engine:
    """
...
class AirQualityStationService25Workflow:
    """
    ...
    def __init__(self, record: AirQualityStationService25Record):
        self.record = record
...
class AirQualityStationService25Manager:
    """
    Service registry and data aggregator for Urban Environmental AQI & PM2.5 Monitors.
    """
    def __init__(self):
        self.store: Dict[str, AirQualityStationService25Record] = {}
        self.engine = AirQualityStationService25Engine()

    def create(self, op_index: float = 50.0) -> AirQualityStationService25Record:
        rec = AirQualityStationService25Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[AirQualityStationService25Record]:
...
def test_subsystem_air_quality_station_25() -> bool:
    mgr = AirQualityStationService25Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = AirQualityStationService25Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_air_quality_station_25()
    print(f"air_quality_station Part 25: {'PASSED' if ok else 'FAILED'}")
