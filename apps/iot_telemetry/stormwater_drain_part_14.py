...
Module: Stormwater Sump & Culvert Flood Monitors (Part 14)
...
class StormwaterDrainService14Engine:
    """
...
class StormwaterDrainService14Workflow:
    """
    ...
    def __init__(self, record: StormwaterDrainService14Record):
        self.record = record
...
class StormwaterDrainService14Manager:
    """
    Service registry and data aggregator for Stormwater Sump & Culvert Flood Monitors.
    """
    def __init__(self):
        self.store: Dict[str, StormwaterDrainService14Record] = {}
        self.engine = StormwaterDrainService14Engine()

    def create(self, op_index: float = 50.0) -> StormwaterDrainService14Record:
        rec = StormwaterDrainService14Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[StormwaterDrainService14Record]:
...
def test_subsystem_stormwater_drain_14() -> bool:
    mgr = StormwaterDrainService14Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = StormwaterDrainService14Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_stormwater_drain_14()
    print(f"stormwater_drain Part 14: {'PASSED' if ok else 'FAILED'}")
