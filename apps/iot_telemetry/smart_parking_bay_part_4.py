...
Module: Urban Parking Space Occupancy Sensors (Part 4)
...
class SmartParkingBayService4Engine:
    """
...
class SmartParkingBayService4Workflow:
    """
    ...
    def __init__(self, record: SmartParkingBayService4Record):
        self.record = record
...
class SmartParkingBayService4Manager:
    """
    Service registry and data aggregator for Urban Parking Space Occupancy Sensors.
    """
    def __init__(self):
        self.store: Dict[str, SmartParkingBayService4Record] = {}
        self.engine = SmartParkingBayService4Engine()

    def create(self, op_index: float = 50.0) -> SmartParkingBayService4Record:
        rec = SmartParkingBayService4Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[SmartParkingBayService4Record]:
...
def test_subsystem_smart_parking_bay_4() -> bool:
    mgr = SmartParkingBayService4Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = SmartParkingBayService4Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_smart_parking_bay_4()
    print(f"smart_parking_bay Part 4: {'PASSED' if ok else 'FAILED'}")
