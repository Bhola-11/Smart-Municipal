...
Module: Urban Environmental AQI & PM2.5 Monitors (Part 7)
...
class AirQualityStationService7Engine:
    """
...
class AirQualityStationService7Workflow:
    """
    ...
    def __init__(self, record: AirQualityStationService7Record):
        self.record = record
...
class AirQualityStationService7Manager:
    """
    Service registry and data aggregator for Urban Environmental AQI & PM2.5 Monitors.
    """
    def __init__(self):
        self.store: Dict[str, AirQualityStationService7Record] = {}
        self.engine = AirQualityStationService7Engine()

    def create(self, op_index: float = 50.0) -> AirQualityStationService7Record:
        rec = AirQualityStationService7Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[AirQualityStationService7Record]:
...
def test_subsystem_air_quality_station_7() -> bool:
    mgr = AirQualityStationService7Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = AirQualityStationService7Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_air_quality_station_7()
    print(f"air_quality_station Part 7: {'PASSED' if ok else 'FAILED'}")
