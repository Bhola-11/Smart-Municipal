...
Module: Urban Environmental AQI & PM2.5 Monitors (Part 4)
...
class AirQualityStationService4Engine:
    """
...
class AirQualityStationService4Workflow:
    """
    ...
    def __init__(self, record: AirQualityStationService4Record):
        self.record = record
...
class AirQualityStationService4Manager:
    """
    Service registry and data aggregator for Urban Environmental AQI & PM2.5 Monitors.
    """
    def __init__(self):
        self.store: Dict[str, AirQualityStationService4Record] = {}
        self.engine = AirQualityStationService4Engine()

    def create(self, op_index: float = 50.0) -> AirQualityStationService4Record:
        rec = AirQualityStationService4Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[AirQualityStationService4Record]:
...
def test_subsystem_air_quality_station_4() -> bool:
    mgr = AirQualityStationService4Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = AirQualityStationService4Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_air_quality_station_4()
    print(f"air_quality_station Part 4: {'PASSED' if ok else 'FAILED'}")
