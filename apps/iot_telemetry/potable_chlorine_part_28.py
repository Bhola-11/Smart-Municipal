...
Module: Drinking Water Chlorination Analyzers (Part 28)
...
class PotableChlorineService28Engine:
    """
...
class PotableChlorineService28Workflow:
    """
    ...
    def __init__(self, record: PotableChlorineService28Record):
        self.record = record
...
class PotableChlorineService28Manager:
    """
    Service registry and data aggregator for Drinking Water Chlorination Analyzers.
    """
    def __init__(self):
        self.store: Dict[str, PotableChlorineService28Record] = {}
        self.engine = PotableChlorineService28Engine()

    def create(self, op_index: float = 50.0) -> PotableChlorineService28Record:
        rec = PotableChlorineService28Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[PotableChlorineService28Record]:
...
def test_subsystem_potable_chlorine_28() -> bool:
    mgr = PotableChlorineService28Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = PotableChlorineService28Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_potable_chlorine_28()
    print(f"potable_chlorine Part 28: {'PASSED' if ok else 'FAILED'}")
