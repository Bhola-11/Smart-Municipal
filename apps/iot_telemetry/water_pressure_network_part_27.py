...
Module: Municipal Water Supply Pressure & Flow (Part 27)
...
class WaterPressureNetworkService27Engine:
    """
...
class WaterPressureNetworkService27Workflow:
    """
    ...
    def __init__(self, record: WaterPressureNetworkService27Record):
        self.record = record
...
class WaterPressureNetworkService27Manager:
    """
    Service registry and data aggregator for Municipal Water Supply Pressure & Flow.
    """
    def __init__(self):
        self.store: Dict[str, WaterPressureNetworkService27Record] = {}
        self.engine = WaterPressureNetworkService27Engine()

    def create(self, op_index: float = 50.0) -> WaterPressureNetworkService27Record:
        rec = WaterPressureNetworkService27Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[WaterPressureNetworkService27Record]:
...
def test_subsystem_water_pressure_network_27() -> bool:
    mgr = WaterPressureNetworkService27Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = WaterPressureNetworkService27Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_water_pressure_network_27()
    print(f"water_pressure_network Part 27: {'PASSED' if ok else 'FAILED'}")
