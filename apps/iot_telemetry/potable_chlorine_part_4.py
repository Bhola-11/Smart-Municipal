...
Module: Drinking Water Chlorination Analyzers (Part 4)
...
class PotableChlorineService4Engine:
    """
...
class PotableChlorineService4Workflow:
    """
    ...
    def __init__(self, record: PotableChlorineService4Record):
        self.record = record
...
class PotableChlorineService4Manager:
    """
    Service registry and data aggregator for Drinking Water Chlorination Analyzers.
    """
    def __init__(self):
        self.store: Dict[str, PotableChlorineService4Record] = {}
        self.engine = PotableChlorineService4Engine()

    def create(self, op_index: float = 50.0) -> PotableChlorineService4Record:
        rec = PotableChlorineService4Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[PotableChlorineService4Record]:
...
def test_subsystem_potable_chlorine_4() -> bool:
    mgr = PotableChlorineService4Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = PotableChlorineService4Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_potable_chlorine_4()
    print(f"potable_chlorine Part 4: {'PASSED' if ok else 'FAILED'}")
