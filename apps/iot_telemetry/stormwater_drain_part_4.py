...
Module: Stormwater Sump & Culvert Flood Monitors (Part 4)
...
class StormwaterDrainService4Engine:
    """
...
class StormwaterDrainService4Workflow:
    """
    ...
    def __init__(self, record: StormwaterDrainService4Record):
        self.record = record
...
class StormwaterDrainService4Manager:
    """
    Service registry and data aggregator for Stormwater Sump & Culvert Flood Monitors.
    """
    def __init__(self):
        self.store: Dict[str, StormwaterDrainService4Record] = {}
        self.engine = StormwaterDrainService4Engine()

    def create(self, op_index: float = 50.0) -> StormwaterDrainService4Record:
        rec = StormwaterDrainService4Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[StormwaterDrainService4Record]:
...
def test_subsystem_stormwater_drain_4() -> bool:
    mgr = StormwaterDrainService4Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = StormwaterDrainService4Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_stormwater_drain_4()
    print(f"stormwater_drain Part 4: {'PASSED' if ok else 'FAILED'}")
