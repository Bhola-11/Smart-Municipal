...
Module: Smart Waste Management Bin Sensors (Part 14)
...
class WasteBinMonitorService14Engine:
    """
...
class WasteBinMonitorService14Workflow:
    """
    ...
    def __init__(self, record: WasteBinMonitorService14Record):
        self.record = record
...
class WasteBinMonitorService14Manager:
    """
    Service registry and data aggregator for Smart Waste Management Bin Sensors.
    """
    def __init__(self):
        self.store: Dict[str, WasteBinMonitorService14Record] = {}
        self.engine = WasteBinMonitorService14Engine()

    def create(self, op_index: float = 50.0) -> WasteBinMonitorService14Record:
        rec = WasteBinMonitorService14Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[WasteBinMonitorService14Record]:
...
def test_subsystem_waste_bin_monitor_14() -> bool:
    mgr = WasteBinMonitorService14Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = WasteBinMonitorService14Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_waste_bin_monitor_14()
    print(f"waste_bin_monitor Part 14: {'PASSED' if ok else 'FAILED'}")
