...
Module: Urban Parking Space Occupancy Sensors (Part 20)
...
class SmartParkingBayService20Engine:
    """
...
class SmartParkingBayService20Workflow:
    """
    ...
    def __init__(self, record: SmartParkingBayService20Record):
        self.record = record
...
class SmartParkingBayService20Manager:
    """
    Service registry and data aggregator for Urban Parking Space Occupancy Sensors.
    """
    def __init__(self):
        self.store: Dict[str, SmartParkingBayService20Record] = {}
        self.engine = SmartParkingBayService20Engine()

    def create(self, op_index: float = 50.0) -> SmartParkingBayService20Record:
        rec = SmartParkingBayService20Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[SmartParkingBayService20Record]:
...
def test_subsystem_smart_parking_bay_20() -> bool:
    mgr = SmartParkingBayService20Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = SmartParkingBayService20Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_smart_parking_bay_20()
    print(f"smart_parking_bay Part 20: {'PASSED' if ok else 'FAILED'}")
