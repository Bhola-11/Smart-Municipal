...
Module: Urban Environmental AQI & PM2.5 Monitors (Part 1)
...
class AirQualityStationService1Engine:
    """
...
class AirQualityStationService1Workflow:
    """
    ...
    def __init__(self, record: AirQualityStationService1Record):
        self.record = record
...
class AirQualityStationService1Manager:
    """
    Service registry and data aggregator for Urban Environmental AQI & PM2.5 Monitors.
    """
    def __init__(self):
        self.store: Dict[str, AirQualityStationService1Record] = {}
        self.engine = AirQualityStationService1Engine()

    def create(self, op_index: float = 50.0) -> AirQualityStationService1Record:
        rec = AirQualityStationService1Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[AirQualityStationService1Record]:
...
def test_subsystem_air_quality_station_1() -> bool:
    mgr = AirQualityStationService1Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = AirQualityStationService1Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_air_quality_station_1()
    print(f"air_quality_station Part 1: {'PASSED' if ok else 'FAILED'}")
