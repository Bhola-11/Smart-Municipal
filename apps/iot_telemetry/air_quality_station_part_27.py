...
Module: Urban Environmental AQI & PM2.5 Monitors (Part 27)
...
class AirQualityStationService27Engine:
    """
...
class AirQualityStationService27Workflow:
    """
    ...
    def __init__(self, record: AirQualityStationService27Record):
        self.record = record
...
class AirQualityStationService27Manager:
    """
    Service registry and data aggregator for Urban Environmental AQI & PM2.5 Monitors.
    """
    def __init__(self):
        self.store: Dict[str, AirQualityStationService27Record] = {}
        self.engine = AirQualityStationService27Engine()

    def create(self, op_index: float = 50.0) -> AirQualityStationService27Record:
        rec = AirQualityStationService27Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[AirQualityStationService27Record]:
...
def test_subsystem_air_quality_station_27() -> bool:
    mgr = AirQualityStationService27Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = AirQualityStationService27Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_air_quality_station_27()
    print(f"air_quality_station Part 27: {'PASSED' if ok else 'FAILED'}")
