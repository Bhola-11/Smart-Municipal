...
Module: Smart Streetlighting & Lux Sensors (Part 8)
...
class StreetlightGridService8Engine:
    """
...
class StreetlightGridService8Workflow:
    """
    ...
    def __init__(self, record: StreetlightGridService8Record):
        self.record = record
...
class StreetlightGridService8Manager:
    """
    Service registry and data aggregator for Smart Streetlighting & Lux Sensors.
    """
    def __init__(self):
        self.store: Dict[str, StreetlightGridService8Record] = {}
        self.engine = StreetlightGridService8Engine()

    def create(self, op_index: float = 50.0) -> StreetlightGridService8Record:
        rec = StreetlightGridService8Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[StreetlightGridService8Record]:
...
def test_subsystem_streetlight_grid_8() -> bool:
    mgr = StreetlightGridService8Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = StreetlightGridService8Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_streetlight_grid_8()
    print(f"streetlight_grid Part 8: {'PASSED' if ok else 'FAILED'}")
