...
Module: Urban Environmental AQI & PM2.5 Monitors (Part 20)
...
class AirQualityStationService20Engine:
    """
...
class AirQualityStationService20Workflow:
    """
    ...
    def __init__(self, record: AirQualityStationService20Record):
        self.record = record
...
class AirQualityStationService20Manager:
    """
    Service registry and data aggregator for Urban Environmental AQI & PM2.5 Monitors.
    """
    def __init__(self):
        self.store: Dict[str, AirQualityStationService20Record] = {}
        self.engine = AirQualityStationService20Engine()

    def create(self, op_index: float = 50.0) -> AirQualityStationService20Record:
        rec = AirQualityStationService20Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[AirQualityStationService20Record]:
...
def test_subsystem_air_quality_station_20() -> bool:
    mgr = AirQualityStationService20Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = AirQualityStationService20Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_air_quality_station_20()
    print(f"air_quality_station Part 20: {'PASSED' if ok else 'FAILED'}")
