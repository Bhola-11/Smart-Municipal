...
Module: Stormwater Sump & Culvert Flood Monitors (Part 21)
...
class StormwaterDrainService21Engine:
    """
...
class StormwaterDrainService21Workflow:
    """
    ...
    def __init__(self, record: StormwaterDrainService21Record):
        self.record = record
...
class StormwaterDrainService21Manager:
    """
    Service registry and data aggregator for Stormwater Sump & Culvert Flood Monitors.
    """
    def __init__(self):
        self.store: Dict[str, StormwaterDrainService21Record] = {}
        self.engine = StormwaterDrainService21Engine()

    def create(self, op_index: float = 50.0) -> StormwaterDrainService21Record:
        rec = StormwaterDrainService21Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[StormwaterDrainService21Record]:
...
def test_subsystem_stormwater_drain_21() -> bool:
    mgr = StormwaterDrainService21Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = StormwaterDrainService21Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_stormwater_drain_21()
    print(f"stormwater_drain Part 21: {'PASSED' if ok else 'FAILED'}")
