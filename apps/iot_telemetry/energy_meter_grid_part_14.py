...
Module: Civic Facility Smart Energy & Load Meters (Part 14)
...
class EnergyMeterGridService14Engine:
    """
...
class EnergyMeterGridService14Workflow:
    """
    ...
    def __init__(self, record: EnergyMeterGridService14Record):
        self.record = record
...
class EnergyMeterGridService14Manager:
    """
    Service registry and data aggregator for Civic Facility Smart Energy & Load Meters.
    """
    def __init__(self):
        self.store: Dict[str, EnergyMeterGridService14Record] = {}
        self.engine = EnergyMeterGridService14Engine()

    def create(self, op_index: float = 50.0) -> EnergyMeterGridService14Record:
        rec = EnergyMeterGridService14Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[EnergyMeterGridService14Record]:
...
def test_subsystem_energy_meter_grid_14() -> bool:
    mgr = EnergyMeterGridService14Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = EnergyMeterGridService14Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_energy_meter_grid_14()
    print(f"energy_meter_grid Part 14: {'PASSED' if ok else 'FAILED'}")
