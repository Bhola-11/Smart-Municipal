...
Module: Municipal Parks Soil Moisture Telemetry (Part 25)
...
class UrbanIrrigationService25Engine:
    """
...
class UrbanIrrigationService25Workflow:
    """
    ...
    def __init__(self, record: UrbanIrrigationService25Record):
        self.record = record
...
class UrbanIrrigationService25Manager:
    """
    Service registry and data aggregator for Municipal Parks Soil Moisture Telemetry.
    """
    def __init__(self):
        self.store: Dict[str, UrbanIrrigationService25Record] = {}
        self.engine = UrbanIrrigationService25Engine()

    def create(self, op_index: float = 50.0) -> UrbanIrrigationService25Record:
        rec = UrbanIrrigationService25Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[UrbanIrrigationService25Record]:
...
def test_subsystem_urban_irrigation_25() -> bool:
    mgr = UrbanIrrigationService25Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = UrbanIrrigationService25Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_urban_irrigation_25()
    print(f"urban_irrigation Part 25: {'PASSED' if ok else 'FAILED'}")
