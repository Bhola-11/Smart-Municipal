...
Module: Municipal Water Supply Pressure & Flow (Part 4)
...
class WaterPressureNetworkService4Engine:
    """
...
class WaterPressureNetworkService4Workflow:
    """
    ...
    def __init__(self, record: WaterPressureNetworkService4Record):
        self.record = record
...
class WaterPressureNetworkService4Manager:
    """
    Service registry and data aggregator for Municipal Water Supply Pressure & Flow.
    """
    def __init__(self):
        self.store: Dict[str, WaterPressureNetworkService4Record] = {}
        self.engine = WaterPressureNetworkService4Engine()

    def create(self, op_index: float = 50.0) -> WaterPressureNetworkService4Record:
        rec = WaterPressureNetworkService4Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[WaterPressureNetworkService4Record]:
...
def test_subsystem_water_pressure_network_4() -> bool:
    mgr = WaterPressureNetworkService4Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = WaterPressureNetworkService4Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_water_pressure_network_4()
    print(f"water_pressure_network Part 4: {'PASSED' if ok else 'FAILED'}")
