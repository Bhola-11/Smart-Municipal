...
Module: Municipal Parks Soil Moisture Telemetry (Part 30)
...
class UrbanIrrigationService30Engine:
    """
...
class UrbanIrrigationService30Workflow:
    """
    ...
    def __init__(self, record: UrbanIrrigationService30Record):
        self.record = record
...
class UrbanIrrigationService30Manager:
    """
    Service registry and data aggregator for Municipal Parks Soil Moisture Telemetry.
    """
    def __init__(self):
        self.store: Dict[str, UrbanIrrigationService30Record] = {}
        self.engine = UrbanIrrigationService30Engine()

    def create(self, op_index: float = 50.0) -> UrbanIrrigationService30Record:
        rec = UrbanIrrigationService30Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[UrbanIrrigationService30Record]:
...
def test_subsystem_urban_irrigation_30() -> bool:
    mgr = UrbanIrrigationService30Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = UrbanIrrigationService30Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_urban_irrigation_30()
    print(f"urban_irrigation Part 30: {'PASSED' if ok else 'FAILED'}")
