...
Module: Urban Environmental AQI & PM2.5 Monitors (Part 9)
...
class AirQualityStationService9Engine:
    """
...
class AirQualityStationService9Workflow:
    """
    ...
    def __init__(self, record: AirQualityStationService9Record):
        self.record = record
...
class AirQualityStationService9Manager:
    """
    Service registry and data aggregator for Urban Environmental AQI & PM2.5 Monitors.
    """
    def __init__(self):
        self.store: Dict[str, AirQualityStationService9Record] = {}
        self.engine = AirQualityStationService9Engine()

    def create(self, op_index: float = 50.0) -> AirQualityStationService9Record:
        rec = AirQualityStationService9Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[AirQualityStationService9Record]:
...
def test_subsystem_air_quality_station_9() -> bool:
    mgr = AirQualityStationService9Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = AirQualityStationService9Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_air_quality_station_9()
    print(f"air_quality_station Part 9: {'PASSED' if ok else 'FAILED'}")
