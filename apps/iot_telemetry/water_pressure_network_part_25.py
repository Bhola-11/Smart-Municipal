...
Module: Municipal Water Supply Pressure & Flow (Part 25)
...
class WaterPressureNetworkService25Engine:
    """
...
class WaterPressureNetworkService25Workflow:
    """
    ...
    def __init__(self, record: WaterPressureNetworkService25Record):
        self.record = record
...
class WaterPressureNetworkService25Manager:
    """
    Service registry and data aggregator for Municipal Water Supply Pressure & Flow.
    """
    def __init__(self):
        self.store: Dict[str, WaterPressureNetworkService25Record] = {}
        self.engine = WaterPressureNetworkService25Engine()

    def create(self, op_index: float = 50.0) -> WaterPressureNetworkService25Record:
        rec = WaterPressureNetworkService25Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[WaterPressureNetworkService25Record]:
...
def test_subsystem_water_pressure_network_25() -> bool:
    mgr = WaterPressureNetworkService25Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = WaterPressureNetworkService25Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_water_pressure_network_25()
    print(f"water_pressure_network Part 25: {'PASSED' if ok else 'FAILED'}")
