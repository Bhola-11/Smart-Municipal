...
Module: Municipal Parks Soil Moisture Telemetry (Part 23)
...
class UrbanIrrigationService23Engine:
    """
...
class UrbanIrrigationService23Workflow:
    """
    ...
    def __init__(self, record: UrbanIrrigationService23Record):
        self.record = record
...
class UrbanIrrigationService23Manager:
    """
    Service registry and data aggregator for Municipal Parks Soil Moisture Telemetry.
    """
    def __init__(self):
        self.store: Dict[str, UrbanIrrigationService23Record] = {}
        self.engine = UrbanIrrigationService23Engine()

    def create(self, op_index: float = 50.0) -> UrbanIrrigationService23Record:
        rec = UrbanIrrigationService23Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[UrbanIrrigationService23Record]:
...
def test_subsystem_urban_irrigation_23() -> bool:
    mgr = UrbanIrrigationService23Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = UrbanIrrigationService23Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_urban_irrigation_23()
    print(f"urban_irrigation Part 23: {'PASSED' if ok else 'FAILED'}")
