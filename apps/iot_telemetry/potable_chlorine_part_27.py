...
Module: Drinking Water Chlorination Analyzers (Part 27)
...
class PotableChlorineService27Engine:
    """
...
class PotableChlorineService27Workflow:
    """
    ...
    def __init__(self, record: PotableChlorineService27Record):
        self.record = record
...
class PotableChlorineService27Manager:
    """
    Service registry and data aggregator for Drinking Water Chlorination Analyzers.
    """
    def __init__(self):
        self.store: Dict[str, PotableChlorineService27Record] = {}
        self.engine = PotableChlorineService27Engine()

    def create(self, op_index: float = 50.0) -> PotableChlorineService27Record:
        rec = PotableChlorineService27Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[PotableChlorineService27Record]:
...
def test_subsystem_potable_chlorine_27() -> bool:
    mgr = PotableChlorineService27Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = PotableChlorineService27Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_potable_chlorine_27()
    print(f"potable_chlorine Part 27: {'PASSED' if ok else 'FAILED'}")
