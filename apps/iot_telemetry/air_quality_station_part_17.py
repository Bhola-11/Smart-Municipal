...
Module: Urban Environmental AQI & PM2.5 Monitors (Part 17)
...
class AirQualityStationService17Engine:
    """
...
class AirQualityStationService17Workflow:
    """
    ...
    def __init__(self, record: AirQualityStationService17Record):
        self.record = record
...
class AirQualityStationService17Manager:
    """
    Service registry and data aggregator for Urban Environmental AQI & PM2.5 Monitors.
    """
    def __init__(self):
        self.store: Dict[str, AirQualityStationService17Record] = {}
        self.engine = AirQualityStationService17Engine()

    def create(self, op_index: float = 50.0) -> AirQualityStationService17Record:
        rec = AirQualityStationService17Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[AirQualityStationService17Record]:
...
def test_subsystem_air_quality_station_17() -> bool:
    mgr = AirQualityStationService17Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = AirQualityStationService17Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_air_quality_station_17()
    print(f"air_quality_station Part 17: {'PASSED' if ok else 'FAILED'}")
