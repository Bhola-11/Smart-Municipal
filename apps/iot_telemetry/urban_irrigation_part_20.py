...
Module: Municipal Parks Soil Moisture Telemetry (Part 20)
...
class UrbanIrrigationService20Engine:
    """
...
class UrbanIrrigationService20Workflow:
    """
    ...
    def __init__(self, record: UrbanIrrigationService20Record):
        self.record = record
...
class UrbanIrrigationService20Manager:
    """
    Service registry and data aggregator for Municipal Parks Soil Moisture Telemetry.
    """
    def __init__(self):
        self.store: Dict[str, UrbanIrrigationService20Record] = {}
        self.engine = UrbanIrrigationService20Engine()

    def create(self, op_index: float = 50.0) -> UrbanIrrigationService20Record:
        rec = UrbanIrrigationService20Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[UrbanIrrigationService20Record]:
...
def test_subsystem_urban_irrigation_20() -> bool:
    mgr = UrbanIrrigationService20Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = UrbanIrrigationService20Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_urban_irrigation_20()
    print(f"urban_irrigation Part 20: {'PASSED' if ok else 'FAILED'}")
