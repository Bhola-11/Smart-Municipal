...
Module: Municipal Parks Soil Moisture Telemetry (Part 21)
...
class UrbanIrrigationService21Engine:
    """
...
class UrbanIrrigationService21Workflow:
    """
    ...
    def __init__(self, record: UrbanIrrigationService21Record):
        self.record = record
...
class UrbanIrrigationService21Manager:
    """
    Service registry and data aggregator for Municipal Parks Soil Moisture Telemetry.
    """
    def __init__(self):
        self.store: Dict[str, UrbanIrrigationService21Record] = {}
        self.engine = UrbanIrrigationService21Engine()

    def create(self, op_index: float = 50.0) -> UrbanIrrigationService21Record:
        rec = UrbanIrrigationService21Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[UrbanIrrigationService21Record]:
...
def test_subsystem_urban_irrigation_21() -> bool:
    mgr = UrbanIrrigationService21Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = UrbanIrrigationService21Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_urban_irrigation_21()
    print(f"urban_irrigation Part 21: {'PASSED' if ok else 'FAILED'}")
