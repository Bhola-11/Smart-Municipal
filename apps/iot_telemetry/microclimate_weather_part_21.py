...
Module: Automated Municipal Micro-Climate Stations (Part 21)
...
class MicroclimateWeatherService21Engine:
    """
...
class MicroclimateWeatherService21Workflow:
    """
    ...
    def __init__(self, record: MicroclimateWeatherService21Record):
        self.record = record
...
class MicroclimateWeatherService21Manager:
    """
    Service registry and data aggregator for Automated Municipal Micro-Climate Stations.
    """
    def __init__(self):
        self.store: Dict[str, MicroclimateWeatherService21Record] = {}
        self.engine = MicroclimateWeatherService21Engine()

    def create(self, op_index: float = 50.0) -> MicroclimateWeatherService21Record:
        rec = MicroclimateWeatherService21Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[MicroclimateWeatherService21Record]:
...
def test_subsystem_microclimate_weather_21() -> bool:
    mgr = MicroclimateWeatherService21Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = MicroclimateWeatherService21Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_microclimate_weather_21()
    print(f"microclimate_weather Part 21: {'PASSED' if ok else 'FAILED'}")
