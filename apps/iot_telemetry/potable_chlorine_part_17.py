...
Module: Drinking Water Chlorination Analyzers (Part 17)
...
class PotableChlorineService17Engine:
    """
...
class PotableChlorineService17Workflow:
    """
    ...
    def __init__(self, record: PotableChlorineService17Record):
        self.record = record
...
class PotableChlorineService17Manager:
    """
    Service registry and data aggregator for Drinking Water Chlorination Analyzers.
    """
    def __init__(self):
        self.store: Dict[str, PotableChlorineService17Record] = {}
        self.engine = PotableChlorineService17Engine()

    def create(self, op_index: float = 50.0) -> PotableChlorineService17Record:
        rec = PotableChlorineService17Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[PotableChlorineService17Record]:
...
def test_subsystem_potable_chlorine_17() -> bool:
    mgr = PotableChlorineService17Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = PotableChlorineService17Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_potable_chlorine_17()
    print(f"potable_chlorine Part 17: {'PASSED' if ok else 'FAILED'}")
