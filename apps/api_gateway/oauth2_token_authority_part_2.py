...
Module: OAuth2 / JWT Token Authentication Authority (Part 2)
...
class Oauth2TokenAuthorityService2Engine:
    """
...
class Oauth2TokenAuthorityService2Workflow:
    """
    ...
    def __init__(self, record: Oauth2TokenAuthorityService2Record):
        self.record = record
...
class Oauth2TokenAuthorityService2Manager:
    """
    Service registry and data aggregator for OAuth2 / JWT Token Authentication Authority.
    """
    def __init__(self):
        self.store: Dict[str, Oauth2TokenAuthorityService2Record] = {}
        self.engine = Oauth2TokenAuthorityService2Engine()

    def create(self, op_index: float = 50.0) -> Oauth2TokenAuthorityService2Record:
        rec = Oauth2TokenAuthorityService2Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[Oauth2TokenAuthorityService2Record]:
...
def test_subsystem_oauth2_token_authority_2() -> bool:
    mgr = Oauth2TokenAuthorityService2Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = Oauth2TokenAuthorityService2Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_oauth2_token_authority_2()
    print(f"oauth2_token_authority Part 2: {'PASSED' if ok else 'FAILED'}")
