...
Module: RESTful API v1 Municipal Resource Controllers (Part 1)
...
class RestV1ControllersService1Engine:
    """
...
class RestV1ControllersService1Workflow:
    """
    ...
    def __init__(self, record: RestV1ControllersService1Record):
        self.record = record
...
class RestV1ControllersService1Manager:
    """
    Service registry and data aggregator for RESTful API v1 Municipal Resource Controllers.
    """
    def __init__(self):
        self.store: Dict[str, RestV1ControllersService1Record] = {}
        self.engine = RestV1ControllersService1Engine()

    def create(self, op_index: float = 50.0) -> RestV1ControllersService1Record:
        rec = RestV1ControllersService1Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[RestV1ControllersService1Record]:
...
def test_subsystem_rest_v1_controllers_1() -> bool:
    mgr = RestV1ControllersService1Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = RestV1ControllersService1Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_rest_v1_controllers_1()
    print(f"rest_v1_controllers Part 1: {'PASSED' if ok else 'FAILED'}")
