...
Module: HMAC-SHA256 Signed Webhook Event Dispatcher (Part 8)
...
class HmacWebhookPublisherService8Engine:
    """
...
class HmacWebhookPublisherService8Workflow:
    """
    ...
    def __init__(self, record: HmacWebhookPublisherService8Record):
        self.record = record
...
class HmacWebhookPublisherService8Manager:
    """
    Service registry and data aggregator for HMAC-SHA256 Signed Webhook Event Dispatcher.
    """
    def __init__(self):
        self.store: Dict[str, HmacWebhookPublisherService8Record] = {}
        self.engine = HmacWebhookPublisherService8Engine()

    def create(self, op_index: float = 50.0) -> HmacWebhookPublisherService8Record:
        rec = HmacWebhookPublisherService8Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[HmacWebhookPublisherService8Record]:
...
def test_subsystem_hmac_webhook_publisher_8() -> bool:
    mgr = HmacWebhookPublisherService8Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = HmacWebhookPublisherService8Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_hmac_webhook_publisher_8()
    print(f"hmac_webhook_publisher Part 8: {'PASSED' if ok else 'FAILED'}")
