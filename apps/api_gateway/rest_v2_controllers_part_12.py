...
Module: RESTful API v2 Hypermedia Hateoas Controllers (Part 12)
...
class RestV2ControllersService12Engine:
    """
...
class RestV2ControllersService12Workflow:
    """
    ...
    def __init__(self, record: RestV2ControllersService12Record):
        self.record = record
...
class RestV2ControllersService12Manager:
    """
    Service registry and data aggregator for RESTful API v2 Hypermedia Hateoas Controllers.
    """
    def __init__(self):
        self.store: Dict[str, RestV2ControllersService12Record] = {}
        self.engine = RestV2ControllersService12Engine()

    def create(self, op_index: float = 50.0) -> RestV2ControllersService12Record:
        rec = RestV2ControllersService12Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[RestV2ControllersService12Record]:
...
def test_subsystem_rest_v2_controllers_12() -> bool:
    mgr = RestV2ControllersService12Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = RestV2ControllersService12Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_rest_v2_controllers_12()
    print(f"rest_v2_controllers Part 12: {'PASSED' if ok else 'FAILED'}")
