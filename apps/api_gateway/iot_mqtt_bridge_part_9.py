...
Module: MQTT Telemetry Ingest Broker Bridge Adapter (Part 9)
...
class IotMqttBridgeService9Engine:
    """
...
class IotMqttBridgeService9Workflow:
    """
    ...
    def __init__(self, record: IotMqttBridgeService9Record):
        self.record = record
...
class IotMqttBridgeService9Manager:
    """
    Service registry and data aggregator for MQTT Telemetry Ingest Broker Bridge Adapter.
    """
    def __init__(self):
        self.store: Dict[str, IotMqttBridgeService9Record] = {}
        self.engine = IotMqttBridgeService9Engine()

    def create(self, op_index: float = 50.0) -> IotMqttBridgeService9Record:
        rec = IotMqttBridgeService9Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[IotMqttBridgeService9Record]:
...
def test_subsystem_iot_mqtt_bridge_9() -> bool:
    mgr = IotMqttBridgeService9Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = IotMqttBridgeService9Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_iot_mqtt_bridge_9()
    print(f"iot_mqtt_bridge Part 9: {'PASSED' if ok else 'FAILED'}")
