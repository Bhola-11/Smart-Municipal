...
Module: MQTT Telemetry Ingest Broker Bridge Adapter (Part 20)
...
class IotMqttBridgeService20Engine:
    """
...
class IotMqttBridgeService20Workflow:
    """
    ...
    def __init__(self, record: IotMqttBridgeService20Record):
        self.record = record
...
class IotMqttBridgeService20Manager:
    """
    Service registry and data aggregator for MQTT Telemetry Ingest Broker Bridge Adapter.
    """
    def __init__(self):
        self.store: Dict[str, IotMqttBridgeService20Record] = {}
        self.engine = IotMqttBridgeService20Engine()

    def create(self, op_index: float = 50.0) -> IotMqttBridgeService20Record:
        rec = IotMqttBridgeService20Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[IotMqttBridgeService20Record]:
...
def test_subsystem_iot_mqtt_bridge_20() -> bool:
    mgr = IotMqttBridgeService20Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = IotMqttBridgeService20Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_iot_mqtt_bridge_20()
    print(f"iot_mqtt_bridge Part 20: {'PASSED' if ok else 'FAILED'}")
