...
Module: RESTful API v2 Hypermedia Hateoas Controllers (Part 4)
...
class RestV2ControllersService4Engine:
    """
...
class RestV2ControllersService4Workflow:
    """
    ...
    def __init__(self, record: RestV2ControllersService4Record):
        self.record = record
...
class RestV2ControllersService4Manager:
    """
    Service registry and data aggregator for RESTful API v2 Hypermedia Hateoas Controllers.
    """
    def __init__(self):
        self.store: Dict[str, RestV2ControllersService4Record] = {}
        self.engine = RestV2ControllersService4Engine()

    def create(self, op_index: float = 50.0) -> RestV2ControllersService4Record:
        rec = RestV2ControllersService4Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[RestV2ControllersService4Record]:
...
def test_subsystem_rest_v2_controllers_4() -> bool:
    mgr = RestV2ControllersService4Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = RestV2ControllersService4Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_rest_v2_controllers_4()
    print(f"rest_v2_controllers Part 4: {'PASSED' if ok else 'FAILED'}")
