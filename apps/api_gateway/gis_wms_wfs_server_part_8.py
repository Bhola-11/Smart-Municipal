...
Module: OGC Web Map Service (WMS/WFS) Spatial Exporter (Part 8)
...
class GisWmsWfsServerService8Engine:
    """
...
class GisWmsWfsServerService8Workflow:
    """
    ...
    def __init__(self, record: GisWmsWfsServerService8Record):
        self.record = record
...
class GisWmsWfsServerService8Manager:
    """
    Service registry and data aggregator for OGC Web Map Service (WMS/WFS) Spatial Exporter.
    """
    def __init__(self):
        self.store: Dict[str, GisWmsWfsServerService8Record] = {}
        self.engine = GisWmsWfsServerService8Engine()

    def create(self, op_index: float = 50.0) -> GisWmsWfsServerService8Record:
        rec = GisWmsWfsServerService8Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[GisWmsWfsServerService8Record]:
...
def test_subsystem_gis_wms_wfs_server_8() -> bool:
    mgr = GisWmsWfsServerService8Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = GisWmsWfsServerService8Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_gis_wms_wfs_server_8()
    print(f"gis_wms_wfs_server Part 8: {'PASSED' if ok else 'FAILED'}")
