...
Module: OGC Web Map Service (WMS/WFS) Spatial Exporter (Part 25)
...
class GisWmsWfsServerService25Engine:
    """
...
class GisWmsWfsServerService25Workflow:
    """
    ...
    def __init__(self, record: GisWmsWfsServerService25Record):
        self.record = record
...
class GisWmsWfsServerService25Manager:
    """
    Service registry and data aggregator for OGC Web Map Service (WMS/WFS) Spatial Exporter.
    """
    def __init__(self):
        self.store: Dict[str, GisWmsWfsServerService25Record] = {}
        self.engine = GisWmsWfsServerService25Engine()

    def create(self, op_index: float = 50.0) -> GisWmsWfsServerService25Record:
        rec = GisWmsWfsServerService25Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[GisWmsWfsServerService25Record]:
...
def test_subsystem_gis_wms_wfs_server_25() -> bool:
    mgr = GisWmsWfsServerService25Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = GisWmsWfsServerService25Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_gis_wms_wfs_server_25()
    print(f"gis_wms_wfs_server Part 25: {'PASSED' if ok else 'FAILED'}")
