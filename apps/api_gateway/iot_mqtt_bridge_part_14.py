...
Module: MQTT Telemetry Ingest Broker Bridge Adapter (Part 14)
...
class IotMqttBridgeService14Engine:
    """
...
class IotMqttBridgeService14Workflow:
    """
    ...
    def __init__(self, record: IotMqttBridgeService14Record):
        self.record = record
...
class IotMqttBridgeService14Manager:
    """
    Service registry and data aggregator for MQTT Telemetry Ingest Broker Bridge Adapter.
    """
    def __init__(self):
        self.store: Dict[str, IotMqttBridgeService14Record] = {}
        self.engine = IotMqttBridgeService14Engine()

    def create(self, op_index: float = 50.0) -> IotMqttBridgeService14Record:
        rec = IotMqttBridgeService14Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[IotMqttBridgeService14Record]:
...
def test_subsystem_iot_mqtt_bridge_14() -> bool:
    mgr = IotMqttBridgeService14Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = IotMqttBridgeService14Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_iot_mqtt_bridge_14()
    print(f"iot_mqtt_bridge Part 14: {'PASSED' if ok else 'FAILED'}")
