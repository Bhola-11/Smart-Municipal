...
Module: OGC Web Map Service (WMS/WFS) Spatial Exporter (Part 17)
...
class GisWmsWfsServerService17Engine:
    """
...
class GisWmsWfsServerService17Workflow:
    """
    ...
    def __init__(self, record: GisWmsWfsServerService17Record):
        self.record = record
...
class GisWmsWfsServerService17Manager:
    """
    Service registry and data aggregator for OGC Web Map Service (WMS/WFS) Spatial Exporter.
    """
    def __init__(self):
        self.store: Dict[str, GisWmsWfsServerService17Record] = {}
        self.engine = GisWmsWfsServerService17Engine()

    def create(self, op_index: float = 50.0) -> GisWmsWfsServerService17Record:
        rec = GisWmsWfsServerService17Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[GisWmsWfsServerService17Record]:
...
def test_subsystem_gis_wms_wfs_server_17() -> bool:
    mgr = GisWmsWfsServerService17Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = GisWmsWfsServerService17Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_gis_wms_wfs_server_17()
    print(f"gis_wms_wfs_server Part 17: {'PASSED' if ok else 'FAILED'}")
