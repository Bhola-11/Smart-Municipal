...
Module: MQTT Telemetry Ingest Broker Bridge Adapter (Part 26)
...
class IotMqttBridgeService26Engine:
    """
...
class IotMqttBridgeService26Workflow:
    """
    ...
    def __init__(self, record: IotMqttBridgeService26Record):
        self.record = record
...
class IotMqttBridgeService26Manager:
    """
    Service registry and data aggregator for MQTT Telemetry Ingest Broker Bridge Adapter.
    """
    def __init__(self):
        self.store: Dict[str, IotMqttBridgeService26Record] = {}
        self.engine = IotMqttBridgeService26Engine()

    def create(self, op_index: float = 50.0) -> IotMqttBridgeService26Record:
        rec = IotMqttBridgeService26Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[IotMqttBridgeService26Record]:
...
def test_subsystem_iot_mqtt_bridge_26() -> bool:
    mgr = IotMqttBridgeService26Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = IotMqttBridgeService26Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_iot_mqtt_bridge_26()
    print(f"iot_mqtt_bridge Part 26: {'PASSED' if ok else 'FAILED'}")
