...
Module: OAuth2 / JWT Token Authentication Authority (Part 5)
...
class Oauth2TokenAuthorityService5Engine:
    """
...
class Oauth2TokenAuthorityService5Workflow:
    """
    ...
    def __init__(self, record: Oauth2TokenAuthorityService5Record):
        self.record = record
...
class Oauth2TokenAuthorityService5Manager:
    """
    Service registry and data aggregator for OAuth2 / JWT Token Authentication Authority.
    """
    def __init__(self):
        self.store: Dict[str, Oauth2TokenAuthorityService5Record] = {}
        self.engine = Oauth2TokenAuthorityService5Engine()

    def create(self, op_index: float = 50.0) -> Oauth2TokenAuthorityService5Record:
        rec = Oauth2TokenAuthorityService5Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[Oauth2TokenAuthorityService5Record]:
...
def test_subsystem_oauth2_token_authority_5() -> bool:
    mgr = Oauth2TokenAuthorityService5Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = Oauth2TokenAuthorityService5Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_oauth2_token_authority_5()
    print(f"oauth2_token_authority Part 5: {'PASSED' if ok else 'FAILED'}")
