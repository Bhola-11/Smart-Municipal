...
Module: MQTT Telemetry Ingest Broker Bridge Adapter (Part 5)
...
class IotMqttBridgeService5Engine:
    """
...
class IotMqttBridgeService5Workflow:
    """
    ...
    def __init__(self, record: IotMqttBridgeService5Record):
        self.record = record
...
class IotMqttBridgeService5Manager:
    """
    Service registry and data aggregator for MQTT Telemetry Ingest Broker Bridge Adapter.
    """
    def __init__(self):
        self.store: Dict[str, IotMqttBridgeService5Record] = {}
        self.engine = IotMqttBridgeService5Engine()

    def create(self, op_index: float = 50.0) -> IotMqttBridgeService5Record:
        rec = IotMqttBridgeService5Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[IotMqttBridgeService5Record]:
...
def test_subsystem_iot_mqtt_bridge_5() -> bool:
    mgr = IotMqttBridgeService5Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = IotMqttBridgeService5Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_iot_mqtt_bridge_5()
    print(f"iot_mqtt_bridge Part 5: {'PASSED' if ok else 'FAILED'}")
