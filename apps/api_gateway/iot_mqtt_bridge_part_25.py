...
Module: MQTT Telemetry Ingest Broker Bridge Adapter (Part 25)
...
class IotMqttBridgeService25Engine:
    """
...
class IotMqttBridgeService25Workflow:
    """
    ...
    def __init__(self, record: IotMqttBridgeService25Record):
        self.record = record
...
class IotMqttBridgeService25Manager:
    """
    Service registry and data aggregator for MQTT Telemetry Ingest Broker Bridge Adapter.
    """
    def __init__(self):
        self.store: Dict[str, IotMqttBridgeService25Record] = {}
        self.engine = IotMqttBridgeService25Engine()

    def create(self, op_index: float = 50.0) -> IotMqttBridgeService25Record:
        rec = IotMqttBridgeService25Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[IotMqttBridgeService25Record]:
...
def test_subsystem_iot_mqtt_bridge_25() -> bool:
    mgr = IotMqttBridgeService25Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = IotMqttBridgeService25Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_iot_mqtt_bridge_25()
    print(f"iot_mqtt_bridge Part 25: {'PASSED' if ok else 'FAILED'}")
