...
Module: OpenAPI 3.0 Dynamic Swagger Schema Generator (Part 2)
...
class OpenapiSchemaGeneratorService2Engine:
    """
...
class OpenapiSchemaGeneratorService2Workflow:
    """
    ...
    def __init__(self, record: OpenapiSchemaGeneratorService2Record):
        self.record = record
...
class OpenapiSchemaGeneratorService2Manager:
    """
    Service registry and data aggregator for OpenAPI 3.0 Dynamic Swagger Schema Generator.
    """
    def __init__(self):
        self.store: Dict[str, OpenapiSchemaGeneratorService2Record] = {}
        self.engine = OpenapiSchemaGeneratorService2Engine()

    def create(self, op_index: float = 50.0) -> OpenapiSchemaGeneratorService2Record:
        rec = OpenapiSchemaGeneratorService2Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[OpenapiSchemaGeneratorService2Record]:
...
def test_subsystem_openapi_schema_generator_2() -> bool:
    mgr = OpenapiSchemaGeneratorService2Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = OpenapiSchemaGeneratorService2Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_openapi_schema_generator_2()
    print(f"openapi_schema_generator Part 2: {'PASSED' if ok else 'FAILED'}")
