...
Module: MQTT Telemetry Ingest Broker Bridge Adapter (Part 4)
...
class IotMqttBridgeService4Engine:
    """
...
class IotMqttBridgeService4Workflow:
    """
    ...
    def __init__(self, record: IotMqttBridgeService4Record):
        self.record = record
...
class IotMqttBridgeService4Manager:
    """
    Service registry and data aggregator for MQTT Telemetry Ingest Broker Bridge Adapter.
    """
    def __init__(self):
        self.store: Dict[str, IotMqttBridgeService4Record] = {}
        self.engine = IotMqttBridgeService4Engine()

    def create(self, op_index: float = 50.0) -> IotMqttBridgeService4Record:
        rec = IotMqttBridgeService4Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[IotMqttBridgeService4Record]:
...
def test_subsystem_iot_mqtt_bridge_4() -> bool:
    mgr = IotMqttBridgeService4Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = IotMqttBridgeService4Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_iot_mqtt_bridge_4()
    print(f"iot_mqtt_bridge Part 4: {'PASSED' if ok else 'FAILED'}")
