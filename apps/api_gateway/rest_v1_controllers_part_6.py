...
Module: RESTful API v1 Municipal Resource Controllers (Part 6)
...
class RestV1ControllersService6Engine:
    """
...
class RestV1ControllersService6Workflow:
    """
    ...
    def __init__(self, record: RestV1ControllersService6Record):
        self.record = record
...
class RestV1ControllersService6Manager:
    """
    Service registry and data aggregator for RESTful API v1 Municipal Resource Controllers.
    """
    def __init__(self):
        self.store: Dict[str, RestV1ControllersService6Record] = {}
        self.engine = RestV1ControllersService6Engine()

    def create(self, op_index: float = 50.0) -> RestV1ControllersService6Record:
        rec = RestV1ControllersService6Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[RestV1ControllersService6Record]:
...
def test_subsystem_rest_v1_controllers_6() -> bool:
    mgr = RestV1ControllersService6Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = RestV1ControllersService6Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_rest_v1_controllers_6()
    print(f"rest_v1_controllers Part 6: {'PASSED' if ok else 'FAILED'}")
