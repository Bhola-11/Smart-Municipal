...
Module: MQTT Telemetry Ingest Broker Bridge Adapter (Part 28)
...
class IotMqttBridgeService28Engine:
    """
...
class IotMqttBridgeService28Workflow:
    """
    ...
    def __init__(self, record: IotMqttBridgeService28Record):
        self.record = record
...
class IotMqttBridgeService28Manager:
    """
    Service registry and data aggregator for MQTT Telemetry Ingest Broker Bridge Adapter.
    """
    def __init__(self):
        self.store: Dict[str, IotMqttBridgeService28Record] = {}
        self.engine = IotMqttBridgeService28Engine()

    def create(self, op_index: float = 50.0) -> IotMqttBridgeService28Record:
        rec = IotMqttBridgeService28Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[IotMqttBridgeService28Record]:
...
def test_subsystem_iot_mqtt_bridge_28() -> bool:
    mgr = IotMqttBridgeService28Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = IotMqttBridgeService28Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_iot_mqtt_bridge_28()
    print(f"iot_mqtt_bridge Part 28: {'PASSED' if ok else 'FAILED'}")
