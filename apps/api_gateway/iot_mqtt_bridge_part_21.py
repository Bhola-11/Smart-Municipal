...
Module: MQTT Telemetry Ingest Broker Bridge Adapter (Part 21)
...
class IotMqttBridgeService21Engine:
    """
...
class IotMqttBridgeService21Workflow:
    """
    ...
    def __init__(self, record: IotMqttBridgeService21Record):
        self.record = record
...
class IotMqttBridgeService21Manager:
    """
    Service registry and data aggregator for MQTT Telemetry Ingest Broker Bridge Adapter.
    """
    def __init__(self):
        self.store: Dict[str, IotMqttBridgeService21Record] = {}
        self.engine = IotMqttBridgeService21Engine()

    def create(self, op_index: float = 50.0) -> IotMqttBridgeService21Record:
        rec = IotMqttBridgeService21Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[IotMqttBridgeService21Record]:
...
def test_subsystem_iot_mqtt_bridge_21() -> bool:
    mgr = IotMqttBridgeService21Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = IotMqttBridgeService21Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_iot_mqtt_bridge_21()
    print(f"iot_mqtt_bridge Part 21: {'PASSED' if ok else 'FAILED'}")
