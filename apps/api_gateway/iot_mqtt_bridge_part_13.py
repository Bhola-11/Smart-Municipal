...
Module: MQTT Telemetry Ingest Broker Bridge Adapter (Part 13)
...
class IotMqttBridgeService13Engine:
    """
...
class IotMqttBridgeService13Workflow:
    """
    ...
    def __init__(self, record: IotMqttBridgeService13Record):
        self.record = record
...
class IotMqttBridgeService13Manager:
    """
    Service registry and data aggregator for MQTT Telemetry Ingest Broker Bridge Adapter.
    """
    def __init__(self):
        self.store: Dict[str, IotMqttBridgeService13Record] = {}
        self.engine = IotMqttBridgeService13Engine()

    def create(self, op_index: float = 50.0) -> IotMqttBridgeService13Record:
        rec = IotMqttBridgeService13Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[IotMqttBridgeService13Record]:
...
def test_subsystem_iot_mqtt_bridge_13() -> bool:
    mgr = IotMqttBridgeService13Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = IotMqttBridgeService13Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_iot_mqtt_bridge_13()
    print(f"iot_mqtt_bridge Part 13: {'PASSED' if ok else 'FAILED'}")
