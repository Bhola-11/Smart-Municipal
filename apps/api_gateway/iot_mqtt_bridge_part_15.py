...
Module: MQTT Telemetry Ingest Broker Bridge Adapter (Part 15)
...
class IotMqttBridgeService15Engine:
    """
...
class IotMqttBridgeService15Workflow:
    """
    ...
    def __init__(self, record: IotMqttBridgeService15Record):
        self.record = record
...
class IotMqttBridgeService15Manager:
    """
    Service registry and data aggregator for MQTT Telemetry Ingest Broker Bridge Adapter.
    """
    def __init__(self):
        self.store: Dict[str, IotMqttBridgeService15Record] = {}
        self.engine = IotMqttBridgeService15Engine()

    def create(self, op_index: float = 50.0) -> IotMqttBridgeService15Record:
        rec = IotMqttBridgeService15Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[IotMqttBridgeService15Record]:
...
def test_subsystem_iot_mqtt_bridge_15() -> bool:
    mgr = IotMqttBridgeService15Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = IotMqttBridgeService15Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_iot_mqtt_bridge_15()
    print(f"iot_mqtt_bridge Part 15: {'PASSED' if ok else 'FAILED'}")
