...
Module: RESTful API v2 Hypermedia Hateoas Controllers (Part 17)
...
class RestV2ControllersService17Engine:
    """
...
class RestV2ControllersService17Workflow:
    """
    ...
    def __init__(self, record: RestV2ControllersService17Record):
        self.record = record
...
class RestV2ControllersService17Manager:
    """
    Service registry and data aggregator for RESTful API v2 Hypermedia Hateoas Controllers.
    """
    def __init__(self):
        self.store: Dict[str, RestV2ControllersService17Record] = {}
        self.engine = RestV2ControllersService17Engine()

    def create(self, op_index: float = 50.0) -> RestV2ControllersService17Record:
        rec = RestV2ControllersService17Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[RestV2ControllersService17Record]:
...
def test_subsystem_rest_v2_controllers_17() -> bool:
    mgr = RestV2ControllersService17Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = RestV2ControllersService17Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_rest_v2_controllers_17()
    print(f"rest_v2_controllers Part 17: {'PASSED' if ok else 'FAILED'}")
