...
Module: OGC Web Map Service (WMS/WFS) Spatial Exporter (Part 4)
...
class GisWmsWfsServerService4Engine:
    """
...
class GisWmsWfsServerService4Workflow:
    """
    ...
    def __init__(self, record: GisWmsWfsServerService4Record):
        self.record = record
...
class GisWmsWfsServerService4Manager:
    """
    Service registry and data aggregator for OGC Web Map Service (WMS/WFS) Spatial Exporter.
    """
    def __init__(self):
        self.store: Dict[str, GisWmsWfsServerService4Record] = {}
        self.engine = GisWmsWfsServerService4Engine()

    def create(self, op_index: float = 50.0) -> GisWmsWfsServerService4Record:
        rec = GisWmsWfsServerService4Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[GisWmsWfsServerService4Record]:
...
def test_subsystem_gis_wms_wfs_server_4() -> bool:
    mgr = GisWmsWfsServerService4Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = GisWmsWfsServerService4Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_gis_wms_wfs_server_4()
    print(f"gis_wms_wfs_server Part 4: {'PASSED' if ok else 'FAILED'}")
