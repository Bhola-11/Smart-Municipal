...
Module: OGC Web Map Service (WMS/WFS) Spatial Exporter (Part 28)
...
class GisWmsWfsServerService28Engine:
    """
...
class GisWmsWfsServerService28Workflow:
    """
    ...
    def __init__(self, record: GisWmsWfsServerService28Record):
        self.record = record
...
class GisWmsWfsServerService28Manager:
    """
    Service registry and data aggregator for OGC Web Map Service (WMS/WFS) Spatial Exporter.
    """
    def __init__(self):
        self.store: Dict[str, GisWmsWfsServerService28Record] = {}
        self.engine = GisWmsWfsServerService28Engine()

    def create(self, op_index: float = 50.0) -> GisWmsWfsServerService28Record:
        rec = GisWmsWfsServerService28Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[GisWmsWfsServerService28Record]:
...
def test_subsystem_gis_wms_wfs_server_28() -> bool:
    mgr = GisWmsWfsServerService28Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = GisWmsWfsServerService28Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_gis_wms_wfs_server_28()
    print(f"gis_wms_wfs_server Part 28: {'PASSED' if ok else 'FAILED'}")
