...
Module: RESTful API v2 Hypermedia Hateoas Controllers (Part 26)
...
class RestV2ControllersService26Engine:
    """
...
class RestV2ControllersService26Workflow:
    """
    ...
    def __init__(self, record: RestV2ControllersService26Record):
        self.record = record
...
class RestV2ControllersService26Manager:
    """
    Service registry and data aggregator for RESTful API v2 Hypermedia Hateoas Controllers.
    """
    def __init__(self):
        self.store: Dict[str, RestV2ControllersService26Record] = {}
        self.engine = RestV2ControllersService26Engine()

    def create(self, op_index: float = 50.0) -> RestV2ControllersService26Record:
        rec = RestV2ControllersService26Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[RestV2ControllersService26Record]:
...
def test_subsystem_rest_v2_controllers_26() -> bool:
    mgr = RestV2ControllersService26Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = RestV2ControllersService26Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_rest_v2_controllers_26()
    print(f"rest_v2_controllers Part 26: {'PASSED' if ok else 'FAILED'}")
