...
Module: MQTT Telemetry Ingest Broker Bridge Adapter (Part 2)
...
class IotMqttBridgeService2Engine:
    """
...
class IotMqttBridgeService2Workflow:
    """
    ...
    def __init__(self, record: IotMqttBridgeService2Record):
        self.record = record
...
class IotMqttBridgeService2Manager:
    """
    Service registry and data aggregator for MQTT Telemetry Ingest Broker Bridge Adapter.
    """
    def __init__(self):
        self.store: Dict[str, IotMqttBridgeService2Record] = {}
        self.engine = IotMqttBridgeService2Engine()

    def create(self, op_index: float = 50.0) -> IotMqttBridgeService2Record:
        rec = IotMqttBridgeService2Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[IotMqttBridgeService2Record]:
...
def test_subsystem_iot_mqtt_bridge_2() -> bool:
    mgr = IotMqttBridgeService2Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = IotMqttBridgeService2Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_iot_mqtt_bridge_2()
    print(f"iot_mqtt_bridge Part 2: {'PASSED' if ok else 'FAILED'}")
