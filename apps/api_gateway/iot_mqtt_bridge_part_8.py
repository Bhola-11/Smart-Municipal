...
Module: MQTT Telemetry Ingest Broker Bridge Adapter (Part 8)
...
class IotMqttBridgeService8Engine:
    """
...
class IotMqttBridgeService8Workflow:
    """
    ...
    def __init__(self, record: IotMqttBridgeService8Record):
        self.record = record
...
class IotMqttBridgeService8Manager:
    """
    Service registry and data aggregator for MQTT Telemetry Ingest Broker Bridge Adapter.
    """
    def __init__(self):
        self.store: Dict[str, IotMqttBridgeService8Record] = {}
        self.engine = IotMqttBridgeService8Engine()

    def create(self, op_index: float = 50.0) -> IotMqttBridgeService8Record:
        rec = IotMqttBridgeService8Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[IotMqttBridgeService8Record]:
...
def test_subsystem_iot_mqtt_bridge_8() -> bool:
    mgr = IotMqttBridgeService8Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = IotMqttBridgeService8Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_iot_mqtt_bridge_8()
    print(f"iot_mqtt_bridge Part 8: {'PASSED' if ok else 'FAILED'}")
