...
Module: MQTT Telemetry Ingest Broker Bridge Adapter (Part 18)
...
class IotMqttBridgeService18Engine:
    """
...
class IotMqttBridgeService18Workflow:
    """
    ...
    def __init__(self, record: IotMqttBridgeService18Record):
        self.record = record
...
class IotMqttBridgeService18Manager:
    """
    Service registry and data aggregator for MQTT Telemetry Ingest Broker Bridge Adapter.
    """
    def __init__(self):
        self.store: Dict[str, IotMqttBridgeService18Record] = {}
        self.engine = IotMqttBridgeService18Engine()

    def create(self, op_index: float = 50.0) -> IotMqttBridgeService18Record:
        rec = IotMqttBridgeService18Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[IotMqttBridgeService18Record]:
...
def test_subsystem_iot_mqtt_bridge_18() -> bool:
    mgr = IotMqttBridgeService18Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = IotMqttBridgeService18Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_iot_mqtt_bridge_18()
    print(f"iot_mqtt_bridge Part 18: {'PASSED' if ok else 'FAILED'}")
