...
Module: OAuth2 / JWT Token Authentication Authority (Part 14)
...
class Oauth2TokenAuthorityService14Engine:
    """
...
class Oauth2TokenAuthorityService14Workflow:
    """
    ...
    def __init__(self, record: Oauth2TokenAuthorityService14Record):
        self.record = record
...
class Oauth2TokenAuthorityService14Manager:
    """
    Service registry and data aggregator for OAuth2 / JWT Token Authentication Authority.
    """
    def __init__(self):
        self.store: Dict[str, Oauth2TokenAuthorityService14Record] = {}
        self.engine = Oauth2TokenAuthorityService14Engine()

    def create(self, op_index: float = 50.0) -> Oauth2TokenAuthorityService14Record:
        rec = Oauth2TokenAuthorityService14Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[Oauth2TokenAuthorityService14Record]:
...
def test_subsystem_oauth2_token_authority_14() -> bool:
    mgr = Oauth2TokenAuthorityService14Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = Oauth2TokenAuthorityService14Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_oauth2_token_authority_14()
    print(f"oauth2_token_authority Part 14: {'PASSED' if ok else 'FAILED'}")
