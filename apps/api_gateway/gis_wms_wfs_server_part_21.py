...
Module: OGC Web Map Service (WMS/WFS) Spatial Exporter (Part 21)
...
class GisWmsWfsServerService21Engine:
    """
...
class GisWmsWfsServerService21Workflow:
    """
    ...
    def __init__(self, record: GisWmsWfsServerService21Record):
        self.record = record
...
class GisWmsWfsServerService21Manager:
    """
    Service registry and data aggregator for OGC Web Map Service (WMS/WFS) Spatial Exporter.
    """
    def __init__(self):
        self.store: Dict[str, GisWmsWfsServerService21Record] = {}
        self.engine = GisWmsWfsServerService21Engine()

    def create(self, op_index: float = 50.0) -> GisWmsWfsServerService21Record:
        rec = GisWmsWfsServerService21Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[GisWmsWfsServerService21Record]:
...
def test_subsystem_gis_wms_wfs_server_21() -> bool:
    mgr = GisWmsWfsServerService21Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = GisWmsWfsServerService21Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_gis_wms_wfs_server_21()
    print(f"gis_wms_wfs_server Part 21: {'PASSED' if ok else 'FAILED'}")
