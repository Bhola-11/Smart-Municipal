...
Module: RESTful API v2 Hypermedia Hateoas Controllers (Part 6)
...
class RestV2ControllersService6Engine:
    """
...
class RestV2ControllersService6Workflow:
    """
    ...
    def __init__(self, record: RestV2ControllersService6Record):
        self.record = record
...
class RestV2ControllersService6Manager:
    """
    Service registry and data aggregator for RESTful API v2 Hypermedia Hateoas Controllers.
    """
    def __init__(self):
        self.store: Dict[str, RestV2ControllersService6Record] = {}
        self.engine = RestV2ControllersService6Engine()

    def create(self, op_index: float = 50.0) -> RestV2ControllersService6Record:
        rec = RestV2ControllersService6Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[RestV2ControllersService6Record]:
...
def test_subsystem_rest_v2_controllers_6() -> bool:
    mgr = RestV2ControllersService6Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = RestV2ControllersService6Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_rest_v2_controllers_6()
    print(f"rest_v2_controllers Part 6: {'PASSED' if ok else 'FAILED'}")
