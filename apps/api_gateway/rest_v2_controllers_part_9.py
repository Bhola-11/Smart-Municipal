...
Module: RESTful API v2 Hypermedia Hateoas Controllers (Part 9)
...
class RestV2ControllersService9Engine:
    """
...
class RestV2ControllersService9Workflow:
    """
    ...
    def __init__(self, record: RestV2ControllersService9Record):
        self.record = record
...
class RestV2ControllersService9Manager:
    """
    Service registry and data aggregator for RESTful API v2 Hypermedia Hateoas Controllers.
    """
    def __init__(self):
        self.store: Dict[str, RestV2ControllersService9Record] = {}
        self.engine = RestV2ControllersService9Engine()

    def create(self, op_index: float = 50.0) -> RestV2ControllersService9Record:
        rec = RestV2ControllersService9Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[RestV2ControllersService9Record]:
...
def test_subsystem_rest_v2_controllers_9() -> bool:
    mgr = RestV2ControllersService9Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = RestV2ControllersService9Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_rest_v2_controllers_9()
    print(f"rest_v2_controllers Part 9: {'PASSED' if ok else 'FAILED'}")
