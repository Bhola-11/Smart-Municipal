...
Module: RESTful API v1 Municipal Resource Controllers (Part 30)
...
class RestV1ControllersService30Engine:
    """
...
class RestV1ControllersService30Workflow:
    """
    ...
    def __init__(self, record: RestV1ControllersService30Record):
        self.record = record
...
class RestV1ControllersService30Manager:
    """
    Service registry and data aggregator for RESTful API v1 Municipal Resource Controllers.
    """
    def __init__(self):
        self.store: Dict[str, RestV1ControllersService30Record] = {}
        self.engine = RestV1ControllersService30Engine()

    def create(self, op_index: float = 50.0) -> RestV1ControllersService30Record:
        rec = RestV1ControllersService30Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[RestV1ControllersService30Record]:
...
def test_subsystem_rest_v1_controllers_30() -> bool:
    mgr = RestV1ControllersService30Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = RestV1ControllersService30Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_rest_v1_controllers_30()
    print(f"rest_v1_controllers Part 30: {'PASSED' if ok else 'FAILED'}")
