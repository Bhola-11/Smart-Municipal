...
Module: Fraud Prevention Vendor Debarment Registry (Part 8)
...
class VendorDebarmentRegistryService8Engine:
    """
...
class VendorDebarmentRegistryService8Workflow:
    """
    ...
    def __init__(self, record: VendorDebarmentRegistryService8Record):
        self.record = record
...
class VendorDebarmentRegistryService8Manager:
    """
    Service registry and data aggregator for Fraud Prevention Vendor Debarment Registry.
    """
    def __init__(self):
        self.store: Dict[str, VendorDebarmentRegistryService8Record] = {}
        self.engine = VendorDebarmentRegistryService8Engine()

    def create(self, op_index: float = 50.0) -> VendorDebarmentRegistryService8Record:
        rec = VendorDebarmentRegistryService8Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[VendorDebarmentRegistryService8Record]:
...
def test_subsystem_vendor_debarment_registry_8() -> bool:
    mgr = VendorDebarmentRegistryService8Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = VendorDebarmentRegistryService8Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_vendor_debarment_registry_8()
    print(f"vendor_debarment_registry Part 8: {'PASSED' if ok else 'FAILED'}")
