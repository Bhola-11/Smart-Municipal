...
Module: Civil Engineering Digital Measurement Book (M-Book) (Part 4)
...
class MeasurementBookMbookService4Engine:
    """
...
class MeasurementBookMbookService4Workflow:
    """
    ...
    def __init__(self, record: MeasurementBookMbookService4Record):
        self.record = record
...
class MeasurementBookMbookService4Manager:
    """
    Service registry and data aggregator for Civil Engineering Digital Measurement Book (M-Book).
    """
    def __init__(self):
        self.store: Dict[str, MeasurementBookMbookService4Record] = {}
        self.engine = MeasurementBookMbookService4Engine()

    def create(self, op_index: float = 50.0) -> MeasurementBookMbookService4Record:
        rec = MeasurementBookMbookService4Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[MeasurementBookMbookService4Record]:
...
def test_subsystem_measurement_book_mbook_4() -> bool:
    mgr = MeasurementBookMbookService4Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = MeasurementBookMbookService4Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_measurement_book_mbook_4()
    print(f"measurement_book_mbook Part 4: {'PASSED' if ok else 'FAILED'}")
