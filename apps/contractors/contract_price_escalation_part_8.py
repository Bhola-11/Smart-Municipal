...
Module: WPI/CPI Formula Based Price Escalation Index (Part 8)
...
class ContractPriceEscalationService8Engine:
    """
...
class ContractPriceEscalationService8Workflow:
    """
    ...
    def __init__(self, record: ContractPriceEscalationService8Record):
        self.record = record
...
class ContractPriceEscalationService8Manager:
    """
    Service registry and data aggregator for WPI/CPI Formula Based Price Escalation Index.
    """
    def __init__(self):
        self.store: Dict[str, ContractPriceEscalationService8Record] = {}
        self.engine = ContractPriceEscalationService8Engine()

    def create(self, op_index: float = 50.0) -> ContractPriceEscalationService8Record:
        rec = ContractPriceEscalationService8Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[ContractPriceEscalationService8Record]:
...
def test_subsystem_contract_price_escalation_8() -> bool:
    mgr = ContractPriceEscalationService8Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = ContractPriceEscalationService8Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_contract_price_escalation_8()
    print(f"contract_price_escalation Part 8: {'PASSED' if ok else 'FAILED'}")
