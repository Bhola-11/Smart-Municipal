...
Module: Fraud Prevention Vendor Debarment Registry (Part 20)
...
class VendorDebarmentRegistryService20Engine:
    """
...
class VendorDebarmentRegistryService20Workflow:
    """
    ...
    def __init__(self, record: VendorDebarmentRegistryService20Record):
        self.record = record
...
class VendorDebarmentRegistryService20Manager:
    """
    Service registry and data aggregator for Fraud Prevention Vendor Debarment Registry.
    """
    def __init__(self):
        self.store: Dict[str, VendorDebarmentRegistryService20Record] = {}
        self.engine = VendorDebarmentRegistryService20Engine()

    def create(self, op_index: float = 50.0) -> VendorDebarmentRegistryService20Record:
        rec = VendorDebarmentRegistryService20Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[VendorDebarmentRegistryService20Record]:
...
def test_subsystem_vendor_debarment_registry_20() -> bool:
    mgr = VendorDebarmentRegistryService20Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = VendorDebarmentRegistryService20Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_vendor_debarment_registry_20()
    print(f"vendor_debarment_registry Part 20: {'PASSED' if ok else 'FAILED'}")
