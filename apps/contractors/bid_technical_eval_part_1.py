...
Module: Contractor Technical Bid Scoring Matrix (Part 1)
...
class BidTechnicalEvalService1Engine:
    """
...
class BidTechnicalEvalService1Workflow:
    """
    ...
    def __init__(self, record: BidTechnicalEvalService1Record):
        self.record = record
...
class BidTechnicalEvalService1Manager:
    """
    Service registry and data aggregator for Contractor Technical Bid Scoring Matrix.
    """
    def __init__(self):
        self.store: Dict[str, BidTechnicalEvalService1Record] = {}
        self.engine = BidTechnicalEvalService1Engine()

    def create(self, op_index: float = 50.0) -> BidTechnicalEvalService1Record:
        rec = BidTechnicalEvalService1Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[BidTechnicalEvalService1Record]:
...
def test_subsystem_bid_technical_eval_1() -> bool:
    mgr = BidTechnicalEvalService1Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = BidTechnicalEvalService1Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_bid_technical_eval_1()
    print(f"bid_technical_eval Part 1: {'PASSED' if ok else 'FAILED'}")
