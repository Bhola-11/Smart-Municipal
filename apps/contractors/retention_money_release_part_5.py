...
Module: Contractor Bank Guarantee & Retention Tracking (Part 5)
...
class RetentionMoneyReleaseService5Engine:
    """
...
class RetentionMoneyReleaseService5Workflow:
    """
    ...
    def __init__(self, record: RetentionMoneyReleaseService5Record):
        self.record = record
...
class RetentionMoneyReleaseService5Manager:
    """
    Service registry and data aggregator for Contractor Bank Guarantee & Retention Tracking.
    """
    def __init__(self):
        self.store: Dict[str, RetentionMoneyReleaseService5Record] = {}
        self.engine = RetentionMoneyReleaseService5Engine()

    def create(self, op_index: float = 50.0) -> RetentionMoneyReleaseService5Record:
        rec = RetentionMoneyReleaseService5Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[RetentionMoneyReleaseService5Record]:
...
def test_subsystem_retention_money_release_5() -> bool:
    mgr = RetentionMoneyReleaseService5Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = RetentionMoneyReleaseService5Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_retention_money_release_5()
    print(f"retention_money_release Part 5: {'PASSED' if ok else 'FAILED'}")
