...
Module: Contractor Technical Bid Scoring Matrix (Part 5)
...
class BidTechnicalEvalService5Engine:
    """
...
class BidTechnicalEvalService5Workflow:
    """
    ...
    def __init__(self, record: BidTechnicalEvalService5Record):
        self.record = record
...
class BidTechnicalEvalService5Manager:
    """
    Service registry and data aggregator for Contractor Technical Bid Scoring Matrix.
    """
    def __init__(self):
        self.store: Dict[str, BidTechnicalEvalService5Record] = {}
        self.engine = BidTechnicalEvalService5Engine()

    def create(self, op_index: float = 50.0) -> BidTechnicalEvalService5Record:
        rec = BidTechnicalEvalService5Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[BidTechnicalEvalService5Record]:
...
def test_subsystem_bid_technical_eval_5() -> bool:
    mgr = BidTechnicalEvalService5Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = BidTechnicalEvalService5Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_bid_technical_eval_5()
    print(f"bid_technical_eval Part 5: {'PASSED' if ok else 'FAILED'}")
