"""
CivicFlow Enterprise Production: vendor_tendering_flow Part 91
...
@dataclass
class VendorTenderingFlowService91Record:
    record_id: str = field(default_factory=lambda: str(uuid.uuid4()))
    entity_code: str = "vendor_tendering_flow_91"
...
class VendorTenderingFlowService91Engine:
...
class VendorTenderingFlowService91Workflow:
    STATES = ["SUBMITTED", "INSPECTION_PENDING", "VERIFIED_APPROVED", "IN_EXECUTION", "QUALITY_AUDIT", "COMPLETED_CERTIFIED"]

    def __init__(self, record: VendorTenderingFlowService91Record):
        self.record = record
...
class VendorTenderingFlowService91Manager:
    def __init__(self):
        self.store: Dict[str, VendorTenderingFlowService91Record] = {}
        self.engine = VendorTenderingFlowService91Engine()

    def create(self, op_index: float = 50.0) -> VendorTenderingFlowService91Record:
        rec = VendorTenderingFlowService91Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[VendorTenderingFlowService91Record]:
