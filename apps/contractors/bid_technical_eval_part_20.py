...
Module: Contractor Technical Bid Scoring Matrix (Part 20)
...
class BidTechnicalEvalService20Engine:
    """
...
class BidTechnicalEvalService20Workflow:
    """
    ...
    def __init__(self, record: BidTechnicalEvalService20Record):
        self.record = record
...
class BidTechnicalEvalService20Manager:
    """
    Service registry and data aggregator for Contractor Technical Bid Scoring Matrix.
    """
    def __init__(self):
        self.store: Dict[str, BidTechnicalEvalService20Record] = {}
        self.engine = BidTechnicalEvalService20Engine()

    def create(self, op_index: float = 50.0) -> BidTechnicalEvalService20Record:
        rec = BidTechnicalEvalService20Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[BidTechnicalEvalService20Record]:
...
def test_subsystem_bid_technical_eval_20() -> bool:
    mgr = BidTechnicalEvalService20Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = BidTechnicalEvalService20Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_bid_technical_eval_20()
    print(f"bid_technical_eval Part 20: {'PASSED' if ok else 'FAILED'}")
