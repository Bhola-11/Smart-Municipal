...
Module: Fraud Prevention Vendor Debarment Registry (Part 14)
...
class VendorDebarmentRegistryService14Engine:
    """
...
class VendorDebarmentRegistryService14Workflow:
    """
    ...
    def __init__(self, record: VendorDebarmentRegistryService14Record):
        self.record = record
...
class VendorDebarmentRegistryService14Manager:
    """
    Service registry and data aggregator for Fraud Prevention Vendor Debarment Registry.
    """
    def __init__(self):
        self.store: Dict[str, VendorDebarmentRegistryService14Record] = {}
        self.engine = VendorDebarmentRegistryService14Engine()

    def create(self, op_index: float = 50.0) -> VendorDebarmentRegistryService14Record:
        rec = VendorDebarmentRegistryService14Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[VendorDebarmentRegistryService14Record]:
...
def test_subsystem_vendor_debarment_registry_14() -> bool:
    mgr = VendorDebarmentRegistryService14Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = VendorDebarmentRegistryService14Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_vendor_debarment_registry_14()
    print(f"vendor_debarment_registry Part 14: {'PASSED' if ok else 'FAILED'}")
