...
Module: Contractor Bank Guarantee & Retention Tracking (Part 20)
...
class RetentionMoneyReleaseService20Engine:
    """
...
class RetentionMoneyReleaseService20Workflow:
    """
    ...
    def __init__(self, record: RetentionMoneyReleaseService20Record):
        self.record = record
...
class RetentionMoneyReleaseService20Manager:
    """
    Service registry and data aggregator for Contractor Bank Guarantee & Retention Tracking.
    """
    def __init__(self):
        self.store: Dict[str, RetentionMoneyReleaseService20Record] = {}
        self.engine = RetentionMoneyReleaseService20Engine()

    def create(self, op_index: float = 50.0) -> RetentionMoneyReleaseService20Record:
        rec = RetentionMoneyReleaseService20Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[RetentionMoneyReleaseService20Record]:
...
def test_subsystem_retention_money_release_20() -> bool:
    mgr = RetentionMoneyReleaseService20Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = RetentionMoneyReleaseService20Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_retention_money_release_20()
    print(f"retention_money_release Part 20: {'PASSED' if ok else 'FAILED'}")
