...
Module: Civil Material Concrete & Bitumen Lab Tests (Part 18)
...
class MaterialQualityTestingService18Engine:
    """
...
class MaterialQualityTestingService18Workflow:
    """
    ...
    def __init__(self, record: MaterialQualityTestingService18Record):
        self.record = record
...
class MaterialQualityTestingService18Manager:
    """
    Service registry and data aggregator for Civil Material Concrete & Bitumen Lab Tests.
    """
    def __init__(self):
        self.store: Dict[str, MaterialQualityTestingService18Record] = {}
        self.engine = MaterialQualityTestingService18Engine()

    def create(self, op_index: float = 50.0) -> MaterialQualityTestingService18Record:
        rec = MaterialQualityTestingService18Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[MaterialQualityTestingService18Record]:
...
def test_subsystem_material_quality_testing_18() -> bool:
    mgr = MaterialQualityTestingService18Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = MaterialQualityTestingService18Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_material_quality_testing_18()
    print(f"material_quality_testing Part 18: {'PASSED' if ok else 'FAILED'}")
