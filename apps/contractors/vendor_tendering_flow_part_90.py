"""
CivicFlow Enterprise Production: vendor_tendering_flow Part 90
...
@dataclass
class VendorTenderingFlowService90Record:
    record_id: str = field(default_factory=lambda: str(uuid.uuid4()))
    entity_code: str = "vendor_tendering_flow_90"
...
class VendorTenderingFlowService90Engine:
...
class VendorTenderingFlowService90Workflow:
    STATES = ["SUBMITTED", "INSPECTION_PENDING", "VERIFIED_APPROVED", "IN_EXECUTION", "QUALITY_AUDIT", "COMPLETED_CERTIFIED"]

    def __init__(self, record: VendorTenderingFlowService90Record):
        self.record = record
...
class VendorTenderingFlowService90Manager:
    def __init__(self):
        self.store: Dict[str, VendorTenderingFlowService90Record] = {}
        self.engine = VendorTenderingFlowService90Engine()

    def create(self, op_index: float = 50.0) -> VendorTenderingFlowService90Record:
        rec = VendorTenderingFlowService90Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[VendorTenderingFlowService90Record]:
