...
Module: Notice Inviting Tender (NIT) Publication Engine (Part 5)
...
class TenderNoticesService5Engine:
    """
...
class TenderNoticesService5Workflow:
    """
    ...
    def __init__(self, record: TenderNoticesService5Record):
        self.record = record
...
class TenderNoticesService5Manager:
    """
    Service registry and data aggregator for Notice Inviting Tender (NIT) Publication Engine.
    """
    def __init__(self):
        self.store: Dict[str, TenderNoticesService5Record] = {}
        self.engine = TenderNoticesService5Engine()

    def create(self, op_index: float = 50.0) -> TenderNoticesService5Record:
        rec = TenderNoticesService5Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[TenderNoticesService5Record]:
...
def test_subsystem_tender_notices_5() -> bool:
    mgr = TenderNoticesService5Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = TenderNoticesService5Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_tender_notices_5()
    print(f"tender_notices Part 5: {'PASSED' if ok else 'FAILED'}")
