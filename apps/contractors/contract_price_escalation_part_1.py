...
Module: WPI/CPI Formula Based Price Escalation Index (Part 1)
...
class ContractPriceEscalationService1Engine:
    """
...
class ContractPriceEscalationService1Workflow:
    """
    ...
    def __init__(self, record: ContractPriceEscalationService1Record):
        self.record = record
...
class ContractPriceEscalationService1Manager:
    """
    Service registry and data aggregator for WPI/CPI Formula Based Price Escalation Index.
    """
    def __init__(self):
        self.store: Dict[str, ContractPriceEscalationService1Record] = {}
        self.engine = ContractPriceEscalationService1Engine()

    def create(self, op_index: float = 50.0) -> ContractPriceEscalationService1Record:
        rec = ContractPriceEscalationService1Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[ContractPriceEscalationService1Record]:
...
def test_subsystem_contract_price_escalation_1() -> bool:
    mgr = ContractPriceEscalationService1Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = ContractPriceEscalationService1Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_contract_price_escalation_1()
    print(f"contract_price_escalation Part 1: {'PASSED' if ok else 'FAILED'}")
