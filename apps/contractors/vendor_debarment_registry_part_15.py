...
Module: Fraud Prevention Vendor Debarment Registry (Part 15)
...
class VendorDebarmentRegistryService15Engine:
    """
...
class VendorDebarmentRegistryService15Workflow:
    """
    ...
    def __init__(self, record: VendorDebarmentRegistryService15Record):
        self.record = record
...
class VendorDebarmentRegistryService15Manager:
    """
    Service registry and data aggregator for Fraud Prevention Vendor Debarment Registry.
    """
    def __init__(self):
        self.store: Dict[str, VendorDebarmentRegistryService15Record] = {}
        self.engine = VendorDebarmentRegistryService15Engine()

    def create(self, op_index: float = 50.0) -> VendorDebarmentRegistryService15Record:
        rec = VendorDebarmentRegistryService15Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[VendorDebarmentRegistryService15Record]:
...
def test_subsystem_vendor_debarment_registry_15() -> bool:
    mgr = VendorDebarmentRegistryService15Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = VendorDebarmentRegistryService15Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_vendor_debarment_registry_15()
    print(f"vendor_debarment_registry Part 15: {'PASSED' if ok else 'FAILED'}")
