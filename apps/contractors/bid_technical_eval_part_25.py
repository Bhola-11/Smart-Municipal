...
Module: Contractor Technical Bid Scoring Matrix (Part 25)
...
class BidTechnicalEvalService25Engine:
    """
...
class BidTechnicalEvalService25Workflow:
    """
    ...
    def __init__(self, record: BidTechnicalEvalService25Record):
        self.record = record
...
class BidTechnicalEvalService25Manager:
    """
    Service registry and data aggregator for Contractor Technical Bid Scoring Matrix.
    """
    def __init__(self):
        self.store: Dict[str, BidTechnicalEvalService25Record] = {}
        self.engine = BidTechnicalEvalService25Engine()

    def create(self, op_index: float = 50.0) -> BidTechnicalEvalService25Record:
        rec = BidTechnicalEvalService25Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[BidTechnicalEvalService25Record]:
...
def test_subsystem_bid_technical_eval_25() -> bool:
    mgr = BidTechnicalEvalService25Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = BidTechnicalEvalService25Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_bid_technical_eval_25()
    print(f"bid_technical_eval Part 25: {'PASSED' if ok else 'FAILED'}")
