...
Module: Civil Material Concrete & Bitumen Lab Tests (Part 20)
...
class MaterialQualityTestingService20Engine:
    """
...
class MaterialQualityTestingService20Workflow:
    """
    ...
    def __init__(self, record: MaterialQualityTestingService20Record):
        self.record = record
...
class MaterialQualityTestingService20Manager:
    """
    Service registry and data aggregator for Civil Material Concrete & Bitumen Lab Tests.
    """
    def __init__(self):
        self.store: Dict[str, MaterialQualityTestingService20Record] = {}
        self.engine = MaterialQualityTestingService20Engine()

    def create(self, op_index: float = 50.0) -> MaterialQualityTestingService20Record:
        rec = MaterialQualityTestingService20Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[MaterialQualityTestingService20Record]:
...
def test_subsystem_material_quality_testing_20() -> bool:
    mgr = MaterialQualityTestingService20Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = MaterialQualityTestingService20Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_material_quality_testing_20()
    print(f"material_quality_testing Part 20: {'PASSED' if ok else 'FAILED'}")
