...
Module: Civil Material Concrete & Bitumen Lab Tests (Part 30)
...
class MaterialQualityTestingService30Engine:
    """
...
class MaterialQualityTestingService30Workflow:
    """
    ...
    def __init__(self, record: MaterialQualityTestingService30Record):
        self.record = record
...
class MaterialQualityTestingService30Manager:
    """
    Service registry and data aggregator for Civil Material Concrete & Bitumen Lab Tests.
    """
    def __init__(self):
        self.store: Dict[str, MaterialQualityTestingService30Record] = {}
        self.engine = MaterialQualityTestingService30Engine()

    def create(self, op_index: float = 50.0) -> MaterialQualityTestingService30Record:
        rec = MaterialQualityTestingService30Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[MaterialQualityTestingService30Record]:
...
def test_subsystem_material_quality_testing_30() -> bool:
    mgr = MaterialQualityTestingService30Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = MaterialQualityTestingService30Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_material_quality_testing_30()
    print(f"material_quality_testing Part 30: {'PASSED' if ok else 'FAILED'}")
