...
Module: Civil Material Concrete & Bitumen Lab Tests (Part 12)
...
class MaterialQualityTestingService12Engine:
    """
...
class MaterialQualityTestingService12Workflow:
    """
    ...
    def __init__(self, record: MaterialQualityTestingService12Record):
        self.record = record
...
class MaterialQualityTestingService12Manager:
    """
    Service registry and data aggregator for Civil Material Concrete & Bitumen Lab Tests.
    """
    def __init__(self):
        self.store: Dict[str, MaterialQualityTestingService12Record] = {}
        self.engine = MaterialQualityTestingService12Engine()

    def create(self, op_index: float = 50.0) -> MaterialQualityTestingService12Record:
        rec = MaterialQualityTestingService12Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[MaterialQualityTestingService12Record]:
...
def test_subsystem_material_quality_testing_12() -> bool:
    mgr = MaterialQualityTestingService12Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = MaterialQualityTestingService12Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_material_quality_testing_12()
    print(f"material_quality_testing Part 12: {'PASSED' if ok else 'FAILED'}")
