...
Module: Contractor Bank Guarantee & Retention Tracking (Part 8)
...
class RetentionMoneyReleaseService8Engine:
    """
...
class RetentionMoneyReleaseService8Workflow:
    """
    ...
    def __init__(self, record: RetentionMoneyReleaseService8Record):
        self.record = record
...
class RetentionMoneyReleaseService8Manager:
    """
    Service registry and data aggregator for Contractor Bank Guarantee & Retention Tracking.
    """
    def __init__(self):
        self.store: Dict[str, RetentionMoneyReleaseService8Record] = {}
        self.engine = RetentionMoneyReleaseService8Engine()

    def create(self, op_index: float = 50.0) -> RetentionMoneyReleaseService8Record:
        rec = RetentionMoneyReleaseService8Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[RetentionMoneyReleaseService8Record]:
...
def test_subsystem_retention_money_release_8() -> bool:
    mgr = RetentionMoneyReleaseService8Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = RetentionMoneyReleaseService8Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_retention_money_release_8()
    print(f"retention_money_release Part 8: {'PASSED' if ok else 'FAILED'}")
