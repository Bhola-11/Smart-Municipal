...
Module: Notice Inviting Tender (NIT) Publication Engine (Part 1)
...
class TenderNoticesService1Engine:
    """
...
class TenderNoticesService1Workflow:
    """
    ...
    def __init__(self, record: TenderNoticesService1Record):
        self.record = record
...
class TenderNoticesService1Manager:
    """
    Service registry and data aggregator for Notice Inviting Tender (NIT) Publication Engine.
    """
    def __init__(self):
        self.store: Dict[str, TenderNoticesService1Record] = {}
        self.engine = TenderNoticesService1Engine()

    def create(self, op_index: float = 50.0) -> TenderNoticesService1Record:
        rec = TenderNoticesService1Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[TenderNoticesService1Record]:
...
def test_subsystem_tender_notices_1() -> bool:
    mgr = TenderNoticesService1Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = TenderNoticesService1Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_tender_notices_1()
    print(f"tender_notices Part 1: {'PASSED' if ok else 'FAILED'}")
