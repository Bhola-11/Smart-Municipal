...
Module: Civil Material Concrete & Bitumen Lab Tests (Part 6)
...
class MaterialQualityTestingService6Engine:
    """
...
class MaterialQualityTestingService6Workflow:
    """
    ...
    def __init__(self, record: MaterialQualityTestingService6Record):
        self.record = record
...
class MaterialQualityTestingService6Manager:
    """
    Service registry and data aggregator for Civil Material Concrete & Bitumen Lab Tests.
    """
    def __init__(self):
        self.store: Dict[str, MaterialQualityTestingService6Record] = {}
        self.engine = MaterialQualityTestingService6Engine()

    def create(self, op_index: float = 50.0) -> MaterialQualityTestingService6Record:
        rec = MaterialQualityTestingService6Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[MaterialQualityTestingService6Record]:
...
def test_subsystem_material_quality_testing_6() -> bool:
    mgr = MaterialQualityTestingService6Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = MaterialQualityTestingService6Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_material_quality_testing_6()
    print(f"material_quality_testing Part 6: {'PASSED' if ok else 'FAILED'}")
