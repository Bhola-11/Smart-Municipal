...
Module: Civil Engineering Digital Measurement Book (M-Book) (Part 6)
...
class MeasurementBookMbookService6Engine:
    """
...
class MeasurementBookMbookService6Workflow:
    """
    ...
    def __init__(self, record: MeasurementBookMbookService6Record):
        self.record = record
...
class MeasurementBookMbookService6Manager:
    """
    Service registry and data aggregator for Civil Engineering Digital Measurement Book (M-Book).
    """
    def __init__(self):
        self.store: Dict[str, MeasurementBookMbookService6Record] = {}
        self.engine = MeasurementBookMbookService6Engine()

    def create(self, op_index: float = 50.0) -> MeasurementBookMbookService6Record:
        rec = MeasurementBookMbookService6Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[MeasurementBookMbookService6Record]:
...
def test_subsystem_measurement_book_mbook_6() -> bool:
    mgr = MeasurementBookMbookService6Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = MeasurementBookMbookService6Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_measurement_book_mbook_6()
    print(f"measurement_book_mbook Part 6: {'PASSED' if ok else 'FAILED'}")
