"""
CivicFlow Enterprise Production: vendor_tendering_flow Part 96
...
@dataclass
class VendorTenderingFlowService96Record:
    record_id: str = field(default_factory=lambda: str(uuid.uuid4()))
    entity_code: str = "vendor_tendering_flow_96"
...
class VendorTenderingFlowService96Engine:
...
class VendorTenderingFlowService96Workflow:
    STATES = ["SUBMITTED", "INSPECTION_PENDING", "VERIFIED_APPROVED", "IN_EXECUTION", "QUALITY_AUDIT", "COMPLETED_CERTIFIED"]

    def __init__(self, record: VendorTenderingFlowService96Record):
        self.record = record
...
class VendorTenderingFlowService96Manager:
    def __init__(self):
        self.store: Dict[str, VendorTenderingFlowService96Record] = {}
        self.engine = VendorTenderingFlowService96Engine()

    def create(self, op_index: float = 50.0) -> VendorTenderingFlowService96Record:
        rec = VendorTenderingFlowService96Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[VendorTenderingFlowService96Record]:
