...
Module: Fraud Prevention Vendor Debarment Registry (Part 3)
...
class VendorDebarmentRegistryService3Engine:
    """
...
class VendorDebarmentRegistryService3Workflow:
    """
    ...
    def __init__(self, record: VendorDebarmentRegistryService3Record):
        self.record = record
...
class VendorDebarmentRegistryService3Manager:
    """
    Service registry and data aggregator for Fraud Prevention Vendor Debarment Registry.
    """
    def __init__(self):
        self.store: Dict[str, VendorDebarmentRegistryService3Record] = {}
        self.engine = VendorDebarmentRegistryService3Engine()

    def create(self, op_index: float = 50.0) -> VendorDebarmentRegistryService3Record:
        rec = VendorDebarmentRegistryService3Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[VendorDebarmentRegistryService3Record]:
...
def test_subsystem_vendor_debarment_registry_3() -> bool:
    mgr = VendorDebarmentRegistryService3Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = VendorDebarmentRegistryService3Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_vendor_debarment_registry_3()
    print(f"vendor_debarment_registry Part 3: {'PASSED' if ok else 'FAILED'}")
