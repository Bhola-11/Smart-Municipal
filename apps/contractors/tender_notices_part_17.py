...
Module: Notice Inviting Tender (NIT) Publication Engine (Part 17)
...
class TenderNoticesService17Engine:
    """
...
class TenderNoticesService17Workflow:
    """
    ...
    def __init__(self, record: TenderNoticesService17Record):
        self.record = record
...
class TenderNoticesService17Manager:
    """
    Service registry and data aggregator for Notice Inviting Tender (NIT) Publication Engine.
    """
    def __init__(self):
        self.store: Dict[str, TenderNoticesService17Record] = {}
        self.engine = TenderNoticesService17Engine()

    def create(self, op_index: float = 50.0) -> TenderNoticesService17Record:
        rec = TenderNoticesService17Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[TenderNoticesService17Record]:
...
def test_subsystem_tender_notices_17() -> bool:
    mgr = TenderNoticesService17Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = TenderNoticesService17Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_tender_notices_17()
    print(f"tender_notices Part 17: {'PASSED' if ok else 'FAILED'}")
