...
Module: Contractor Technical Bid Scoring Matrix (Part 15)
...
class BidTechnicalEvalService15Engine:
    """
...
class BidTechnicalEvalService15Workflow:
    """
    ...
    def __init__(self, record: BidTechnicalEvalService15Record):
        self.record = record
...
class BidTechnicalEvalService15Manager:
    """
    Service registry and data aggregator for Contractor Technical Bid Scoring Matrix.
    """
    def __init__(self):
        self.store: Dict[str, BidTechnicalEvalService15Record] = {}
        self.engine = BidTechnicalEvalService15Engine()

    def create(self, op_index: float = 50.0) -> BidTechnicalEvalService15Record:
        rec = BidTechnicalEvalService15Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[BidTechnicalEvalService15Record]:
...
def test_subsystem_bid_technical_eval_15() -> bool:
    mgr = BidTechnicalEvalService15Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = BidTechnicalEvalService15Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_bid_technical_eval_15()
    print(f"bid_technical_eval Part 15: {'PASSED' if ok else 'FAILED'}")
