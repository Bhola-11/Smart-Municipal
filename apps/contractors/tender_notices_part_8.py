...
Module: Notice Inviting Tender (NIT) Publication Engine (Part 8)
...
class TenderNoticesService8Engine:
    """
...
class TenderNoticesService8Workflow:
    """
    ...
    def __init__(self, record: TenderNoticesService8Record):
        self.record = record
...
class TenderNoticesService8Manager:
    """
    Service registry and data aggregator for Notice Inviting Tender (NIT) Publication Engine.
    """
    def __init__(self):
        self.store: Dict[str, TenderNoticesService8Record] = {}
        self.engine = TenderNoticesService8Engine()

    def create(self, op_index: float = 50.0) -> TenderNoticesService8Record:
        rec = TenderNoticesService8Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[TenderNoticesService8Record]:
...
def test_subsystem_tender_notices_8() -> bool:
    mgr = TenderNoticesService8Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = TenderNoticesService8Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_tender_notices_8()
    print(f"tender_notices Part 8: {'PASSED' if ok else 'FAILED'}")
