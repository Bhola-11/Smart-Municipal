...
Module: Civil Material Concrete & Bitumen Lab Tests (Part 21)
...
class MaterialQualityTestingService21Engine:
    """
...
class MaterialQualityTestingService21Workflow:
    """
    ...
    def __init__(self, record: MaterialQualityTestingService21Record):
        self.record = record
...
class MaterialQualityTestingService21Manager:
    """
    Service registry and data aggregator for Civil Material Concrete & Bitumen Lab Tests.
    """
    def __init__(self):
        self.store: Dict[str, MaterialQualityTestingService21Record] = {}
        self.engine = MaterialQualityTestingService21Engine()

    def create(self, op_index: float = 50.0) -> MaterialQualityTestingService21Record:
        rec = MaterialQualityTestingService21Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[MaterialQualityTestingService21Record]:
...
def test_subsystem_material_quality_testing_21() -> bool:
    mgr = MaterialQualityTestingService21Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = MaterialQualityTestingService21Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_material_quality_testing_21()
    print(f"material_quality_testing Part 21: {'PASSED' if ok else 'FAILED'}")
