...
Module: Contractor Performance SLA & Penalty Matrix (Part 8)
...
class ContractorSlaScorecardService8Engine:
    """
...
class ContractorSlaScorecardService8Workflow:
    """
    ...
    def __init__(self, record: ContractorSlaScorecardService8Record):
        self.record = record
...
class ContractorSlaScorecardService8Manager:
    """
    Service registry and data aggregator for Contractor Performance SLA & Penalty Matrix.
    """
    def __init__(self):
        self.store: Dict[str, ContractorSlaScorecardService8Record] = {}
        self.engine = ContractorSlaScorecardService8Engine()

    def create(self, op_index: float = 50.0) -> ContractorSlaScorecardService8Record:
        rec = ContractorSlaScorecardService8Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[ContractorSlaScorecardService8Record]:
...
def test_subsystem_contractor_sla_scorecard_8() -> bool:
    mgr = ContractorSlaScorecardService8Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = ContractorSlaScorecardService8Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_contractor_sla_scorecard_8()
    print(f"contractor_sla_scorecard Part 8: {'PASSED' if ok else 'FAILED'}")
