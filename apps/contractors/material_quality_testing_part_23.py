...
Module: Civil Material Concrete & Bitumen Lab Tests (Part 23)
...
class MaterialQualityTestingService23Engine:
    """
...
class MaterialQualityTestingService23Workflow:
    """
    ...
    def __init__(self, record: MaterialQualityTestingService23Record):
        self.record = record
...
class MaterialQualityTestingService23Manager:
    """
    Service registry and data aggregator for Civil Material Concrete & Bitumen Lab Tests.
    """
    def __init__(self):
        self.store: Dict[str, MaterialQualityTestingService23Record] = {}
        self.engine = MaterialQualityTestingService23Engine()

    def create(self, op_index: float = 50.0) -> MaterialQualityTestingService23Record:
        rec = MaterialQualityTestingService23Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[MaterialQualityTestingService23Record]:
...
def test_subsystem_material_quality_testing_23() -> bool:
    mgr = MaterialQualityTestingService23Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = MaterialQualityTestingService23Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_material_quality_testing_23()
    print(f"material_quality_testing Part 23: {'PASSED' if ok else 'FAILED'}")
