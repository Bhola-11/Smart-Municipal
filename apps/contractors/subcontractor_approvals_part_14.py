...
Module: Specialized Subcontractor Clearance Workflow (Part 14)
...
class SubcontractorApprovalsService14Engine:
    """
...
class SubcontractorApprovalsService14Workflow:
    """
    ...
    def __init__(self, record: SubcontractorApprovalsService14Record):
        self.record = record
...
class SubcontractorApprovalsService14Manager:
    """
    Service registry and data aggregator for Specialized Subcontractor Clearance Workflow.
    """
    def __init__(self):
        self.store: Dict[str, SubcontractorApprovalsService14Record] = {}
        self.engine = SubcontractorApprovalsService14Engine()

    def create(self, op_index: float = 50.0) -> SubcontractorApprovalsService14Record:
        rec = SubcontractorApprovalsService14Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[SubcontractorApprovalsService14Record]:
...
def test_subsystem_subcontractor_approvals_14() -> bool:
    mgr = SubcontractorApprovalsService14Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = SubcontractorApprovalsService14Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_subcontractor_approvals_14()
    print(f"subcontractor_approvals Part 14: {'PASSED' if ok else 'FAILED'}")
