...
Module: Contractor Technical Bid Scoring Matrix (Part 14)
...
class BidTechnicalEvalService14Engine:
    """
...
class BidTechnicalEvalService14Workflow:
    """
    ...
    def __init__(self, record: BidTechnicalEvalService14Record):
        self.record = record
...
class BidTechnicalEvalService14Manager:
    """
    Service registry and data aggregator for Contractor Technical Bid Scoring Matrix.
    """
    def __init__(self):
        self.store: Dict[str, BidTechnicalEvalService14Record] = {}
        self.engine = BidTechnicalEvalService14Engine()

    def create(self, op_index: float = 50.0) -> BidTechnicalEvalService14Record:
        rec = BidTechnicalEvalService14Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[BidTechnicalEvalService14Record]:
...
def test_subsystem_bid_technical_eval_14() -> bool:
    mgr = BidTechnicalEvalService14Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = BidTechnicalEvalService14Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_bid_technical_eval_14()
    print(f"bid_technical_eval Part 14: {'PASSED' if ok else 'FAILED'}")
