...
Module: Civil Material Concrete & Bitumen Lab Tests (Part 22)
...
class MaterialQualityTestingService22Engine:
    """
...
class MaterialQualityTestingService22Workflow:
    """
    ...
    def __init__(self, record: MaterialQualityTestingService22Record):
        self.record = record
...
class MaterialQualityTestingService22Manager:
    """
    Service registry and data aggregator for Civil Material Concrete & Bitumen Lab Tests.
    """
    def __init__(self):
        self.store: Dict[str, MaterialQualityTestingService22Record] = {}
        self.engine = MaterialQualityTestingService22Engine()

    def create(self, op_index: float = 50.0) -> MaterialQualityTestingService22Record:
        rec = MaterialQualityTestingService22Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[MaterialQualityTestingService22Record]:
...
def test_subsystem_material_quality_testing_22() -> bool:
    mgr = MaterialQualityTestingService22Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = MaterialQualityTestingService22Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_material_quality_testing_22()
    print(f"material_quality_testing Part 22: {'PASSED' if ok else 'FAILED'}")
