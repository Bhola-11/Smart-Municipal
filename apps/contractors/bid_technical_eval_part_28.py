...
Module: Contractor Technical Bid Scoring Matrix (Part 28)
...
class BidTechnicalEvalService28Engine:
    """
...
class BidTechnicalEvalService28Workflow:
    """
    ...
    def __init__(self, record: BidTechnicalEvalService28Record):
        self.record = record
...
class BidTechnicalEvalService28Manager:
    """
    Service registry and data aggregator for Contractor Technical Bid Scoring Matrix.
    """
    def __init__(self):
        self.store: Dict[str, BidTechnicalEvalService28Record] = {}
        self.engine = BidTechnicalEvalService28Engine()

    def create(self, op_index: float = 50.0) -> BidTechnicalEvalService28Record:
        rec = BidTechnicalEvalService28Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[BidTechnicalEvalService28Record]:
...
def test_subsystem_bid_technical_eval_28() -> bool:
    mgr = BidTechnicalEvalService28Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = BidTechnicalEvalService28Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_bid_technical_eval_28()
    print(f"bid_technical_eval Part 28: {'PASSED' if ok else 'FAILED'}")
