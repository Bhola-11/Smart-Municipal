...
Module: Notice Inviting Tender (NIT) Publication Engine (Part 13)
...
class TenderNoticesService13Engine:
    """
...
class TenderNoticesService13Workflow:
    """
    ...
    def __init__(self, record: TenderNoticesService13Record):
        self.record = record
...
class TenderNoticesService13Manager:
    """
    Service registry and data aggregator for Notice Inviting Tender (NIT) Publication Engine.
    """
    def __init__(self):
        self.store: Dict[str, TenderNoticesService13Record] = {}
        self.engine = TenderNoticesService13Engine()

    def create(self, op_index: float = 50.0) -> TenderNoticesService13Record:
        rec = TenderNoticesService13Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[TenderNoticesService13Record]:
...
def test_subsystem_tender_notices_13() -> bool:
    mgr = TenderNoticesService13Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = TenderNoticesService13Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_tender_notices_13()
    print(f"tender_notices Part 13: {'PASSED' if ok else 'FAILED'}")
