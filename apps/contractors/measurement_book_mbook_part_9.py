...
Module: Civil Engineering Digital Measurement Book (M-Book) (Part 9)
...
class MeasurementBookMbookService9Engine:
    """
...
class MeasurementBookMbookService9Workflow:
    """
    ...
    def __init__(self, record: MeasurementBookMbookService9Record):
        self.record = record
...
class MeasurementBookMbookService9Manager:
    """
    Service registry and data aggregator for Civil Engineering Digital Measurement Book (M-Book).
    """
    def __init__(self):
        self.store: Dict[str, MeasurementBookMbookService9Record] = {}
        self.engine = MeasurementBookMbookService9Engine()

    def create(self, op_index: float = 50.0) -> MeasurementBookMbookService9Record:
        rec = MeasurementBookMbookService9Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[MeasurementBookMbookService9Record]:
...
def test_subsystem_measurement_book_mbook_9() -> bool:
    mgr = MeasurementBookMbookService9Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = MeasurementBookMbookService9Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_measurement_book_mbook_9()
    print(f"measurement_book_mbook Part 9: {'PASSED' if ok else 'FAILED'}")
