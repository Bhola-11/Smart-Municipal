...
Module: Civil Engineering Digital Measurement Book (M-Book) (Part 14)
...
class MeasurementBookMbookService14Engine:
    """
...
class MeasurementBookMbookService14Workflow:
    """
    ...
    def __init__(self, record: MeasurementBookMbookService14Record):
        self.record = record
...
class MeasurementBookMbookService14Manager:
    """
    Service registry and data aggregator for Civil Engineering Digital Measurement Book (M-Book).
    """
    def __init__(self):
        self.store: Dict[str, MeasurementBookMbookService14Record] = {}
        self.engine = MeasurementBookMbookService14Engine()

    def create(self, op_index: float = 50.0) -> MeasurementBookMbookService14Record:
        rec = MeasurementBookMbookService14Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[MeasurementBookMbookService14Record]:
...
def test_subsystem_measurement_book_mbook_14() -> bool:
    mgr = MeasurementBookMbookService14Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = MeasurementBookMbookService14Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_measurement_book_mbook_14()
    print(f"measurement_book_mbook Part 14: {'PASSED' if ok else 'FAILED'}")
