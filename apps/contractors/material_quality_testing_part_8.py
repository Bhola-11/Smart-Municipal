...
Module: Civil Material Concrete & Bitumen Lab Tests (Part 8)
...
class MaterialQualityTestingService8Engine:
    """
...
class MaterialQualityTestingService8Workflow:
    """
    ...
    def __init__(self, record: MaterialQualityTestingService8Record):
        self.record = record
...
class MaterialQualityTestingService8Manager:
    """
    Service registry and data aggregator for Civil Material Concrete & Bitumen Lab Tests.
    """
    def __init__(self):
        self.store: Dict[str, MaterialQualityTestingService8Record] = {}
        self.engine = MaterialQualityTestingService8Engine()

    def create(self, op_index: float = 50.0) -> MaterialQualityTestingService8Record:
        rec = MaterialQualityTestingService8Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[MaterialQualityTestingService8Record]:
...
def test_subsystem_material_quality_testing_8() -> bool:
    mgr = MaterialQualityTestingService8Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = MaterialQualityTestingService8Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_material_quality_testing_8()
    print(f"material_quality_testing Part 8: {'PASSED' if ok else 'FAILED'}")
