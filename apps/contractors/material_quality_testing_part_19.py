...
Module: Civil Material Concrete & Bitumen Lab Tests (Part 19)
...
class MaterialQualityTestingService19Engine:
    """
...
class MaterialQualityTestingService19Workflow:
    """
    ...
    def __init__(self, record: MaterialQualityTestingService19Record):
        self.record = record
...
class MaterialQualityTestingService19Manager:
    """
    Service registry and data aggregator for Civil Material Concrete & Bitumen Lab Tests.
    """
    def __init__(self):
        self.store: Dict[str, MaterialQualityTestingService19Record] = {}
        self.engine = MaterialQualityTestingService19Engine()

    def create(self, op_index: float = 50.0) -> MaterialQualityTestingService19Record:
        rec = MaterialQualityTestingService19Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[MaterialQualityTestingService19Record]:
...
def test_subsystem_material_quality_testing_19() -> bool:
    mgr = MaterialQualityTestingService19Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = MaterialQualityTestingService19Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_material_quality_testing_19()
    print(f"material_quality_testing Part 19: {'PASSED' if ok else 'FAILED'}")
