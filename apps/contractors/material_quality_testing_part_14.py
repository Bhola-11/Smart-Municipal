...
Module: Civil Material Concrete & Bitumen Lab Tests (Part 14)
...
class MaterialQualityTestingService14Engine:
    """
...
class MaterialQualityTestingService14Workflow:
    """
    ...
    def __init__(self, record: MaterialQualityTestingService14Record):
        self.record = record
...
class MaterialQualityTestingService14Manager:
    """
    Service registry and data aggregator for Civil Material Concrete & Bitumen Lab Tests.
    """
    def __init__(self):
        self.store: Dict[str, MaterialQualityTestingService14Record] = {}
        self.engine = MaterialQualityTestingService14Engine()

    def create(self, op_index: float = 50.0) -> MaterialQualityTestingService14Record:
        rec = MaterialQualityTestingService14Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[MaterialQualityTestingService14Record]:
...
def test_subsystem_material_quality_testing_14() -> bool:
    mgr = MaterialQualityTestingService14Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = MaterialQualityTestingService14Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_material_quality_testing_14()
    print(f"material_quality_testing Part 14: {'PASSED' if ok else 'FAILED'}")
