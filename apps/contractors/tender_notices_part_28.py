...
Module: Notice Inviting Tender (NIT) Publication Engine (Part 28)
...
class TenderNoticesService28Engine:
    """
...
class TenderNoticesService28Workflow:
    """
    ...
    def __init__(self, record: TenderNoticesService28Record):
        self.record = record
...
class TenderNoticesService28Manager:
    """
    Service registry and data aggregator for Notice Inviting Tender (NIT) Publication Engine.
    """
    def __init__(self):
        self.store: Dict[str, TenderNoticesService28Record] = {}
        self.engine = TenderNoticesService28Engine()

    def create(self, op_index: float = 50.0) -> TenderNoticesService28Record:
        rec = TenderNoticesService28Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[TenderNoticesService28Record]:
...
def test_subsystem_tender_notices_28() -> bool:
    mgr = TenderNoticesService28Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = TenderNoticesService28Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_tender_notices_28()
    print(f"tender_notices Part 28: {'PASSED' if ok else 'FAILED'}")
