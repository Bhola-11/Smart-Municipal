...
Module: Civil Material Concrete & Bitumen Lab Tests (Part 7)
...
class MaterialQualityTestingService7Engine:
    """
...
class MaterialQualityTestingService7Workflow:
    """
    ...
    def __init__(self, record: MaterialQualityTestingService7Record):
        self.record = record
...
class MaterialQualityTestingService7Manager:
    """
    Service registry and data aggregator for Civil Material Concrete & Bitumen Lab Tests.
    """
    def __init__(self):
        self.store: Dict[str, MaterialQualityTestingService7Record] = {}
        self.engine = MaterialQualityTestingService7Engine()

    def create(self, op_index: float = 50.0) -> MaterialQualityTestingService7Record:
        rec = MaterialQualityTestingService7Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[MaterialQualityTestingService7Record]:
...
def test_subsystem_material_quality_testing_7() -> bool:
    mgr = MaterialQualityTestingService7Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = MaterialQualityTestingService7Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_material_quality_testing_7()
    print(f"material_quality_testing Part 7: {'PASSED' if ok else 'FAILED'}")
