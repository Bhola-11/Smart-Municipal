...
Module: WPI/CPI Formula Based Price Escalation Index (Part 4)
...
class ContractPriceEscalationService4Engine:
    """
...
class ContractPriceEscalationService4Workflow:
    """
    ...
    def __init__(self, record: ContractPriceEscalationService4Record):
        self.record = record
...
class ContractPriceEscalationService4Manager:
    """
    Service registry and data aggregator for WPI/CPI Formula Based Price Escalation Index.
    """
    def __init__(self):
        self.store: Dict[str, ContractPriceEscalationService4Record] = {}
        self.engine = ContractPriceEscalationService4Engine()

    def create(self, op_index: float = 50.0) -> ContractPriceEscalationService4Record:
        rec = ContractPriceEscalationService4Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[ContractPriceEscalationService4Record]:
...
def test_subsystem_contract_price_escalation_4() -> bool:
    mgr = ContractPriceEscalationService4Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = ContractPriceEscalationService4Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_contract_price_escalation_4()
    print(f"contract_price_escalation Part 4: {'PASSED' if ok else 'FAILED'}")
