...
Module: Contractor Bank Guarantee & Retention Tracking (Part 27)
...
class RetentionMoneyReleaseService27Engine:
    """
...
class RetentionMoneyReleaseService27Workflow:
    """
    ...
    def __init__(self, record: RetentionMoneyReleaseService27Record):
        self.record = record
...
class RetentionMoneyReleaseService27Manager:
    """
    Service registry and data aggregator for Contractor Bank Guarantee & Retention Tracking.
    """
    def __init__(self):
        self.store: Dict[str, RetentionMoneyReleaseService27Record] = {}
        self.engine = RetentionMoneyReleaseService27Engine()

    def create(self, op_index: float = 50.0) -> RetentionMoneyReleaseService27Record:
        rec = RetentionMoneyReleaseService27Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[RetentionMoneyReleaseService27Record]:
...
def test_subsystem_retention_money_release_27() -> bool:
    mgr = RetentionMoneyReleaseService27Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = RetentionMoneyReleaseService27Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_retention_money_release_27()
    print(f"retention_money_release Part 27: {'PASSED' if ok else 'FAILED'}")
