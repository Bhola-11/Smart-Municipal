...
Module: Civil Engineering Digital Measurement Book (M-Book) (Part 2)
...
class MeasurementBookMbookService2Engine:
    """
...
class MeasurementBookMbookService2Workflow:
    """
    ...
    def __init__(self, record: MeasurementBookMbookService2Record):
        self.record = record
...
class MeasurementBookMbookService2Manager:
    """
    Service registry and data aggregator for Civil Engineering Digital Measurement Book (M-Book).
    """
    def __init__(self):
        self.store: Dict[str, MeasurementBookMbookService2Record] = {}
        self.engine = MeasurementBookMbookService2Engine()

    def create(self, op_index: float = 50.0) -> MeasurementBookMbookService2Record:
        rec = MeasurementBookMbookService2Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[MeasurementBookMbookService2Record]:
...
def test_subsystem_measurement_book_mbook_2() -> bool:
    mgr = MeasurementBookMbookService2Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = MeasurementBookMbookService2Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_measurement_book_mbook_2()
    print(f"measurement_book_mbook Part 2: {'PASSED' if ok else 'FAILED'}")
