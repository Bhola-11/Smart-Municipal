...
Module: Civil Material Concrete & Bitumen Lab Tests (Part 13)
...
class MaterialQualityTestingService13Engine:
    """
...
class MaterialQualityTestingService13Workflow:
    """
    ...
    def __init__(self, record: MaterialQualityTestingService13Record):
        self.record = record
...
class MaterialQualityTestingService13Manager:
    """
    Service registry and data aggregator for Civil Material Concrete & Bitumen Lab Tests.
    """
    def __init__(self):
        self.store: Dict[str, MaterialQualityTestingService13Record] = {}
        self.engine = MaterialQualityTestingService13Engine()

    def create(self, op_index: float = 50.0) -> MaterialQualityTestingService13Record:
        rec = MaterialQualityTestingService13Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[MaterialQualityTestingService13Record]:
...
def test_subsystem_material_quality_testing_13() -> bool:
    mgr = MaterialQualityTestingService13Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = MaterialQualityTestingService13Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_material_quality_testing_13()
    print(f"material_quality_testing Part 13: {'PASSED' if ok else 'FAILED'}")
