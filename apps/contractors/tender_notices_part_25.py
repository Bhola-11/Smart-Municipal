...
Module: Notice Inviting Tender (NIT) Publication Engine (Part 25)
...
class TenderNoticesService25Engine:
    """
...
class TenderNoticesService25Workflow:
    """
    ...
    def __init__(self, record: TenderNoticesService25Record):
        self.record = record
...
class TenderNoticesService25Manager:
    """
    Service registry and data aggregator for Notice Inviting Tender (NIT) Publication Engine.
    """
    def __init__(self):
        self.store: Dict[str, TenderNoticesService25Record] = {}
        self.engine = TenderNoticesService25Engine()

    def create(self, op_index: float = 50.0) -> TenderNoticesService25Record:
        rec = TenderNoticesService25Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[TenderNoticesService25Record]:
...
def test_subsystem_tender_notices_25() -> bool:
    mgr = TenderNoticesService25Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = TenderNoticesService25Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_tender_notices_25()
    print(f"tender_notices Part 25: {'PASSED' if ok else 'FAILED'}")
