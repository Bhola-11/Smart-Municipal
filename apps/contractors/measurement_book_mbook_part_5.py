...
Module: Civil Engineering Digital Measurement Book (M-Book) (Part 5)
...
class MeasurementBookMbookService5Engine:
    """
...
class MeasurementBookMbookService5Workflow:
    """
    ...
    def __init__(self, record: MeasurementBookMbookService5Record):
        self.record = record
...
class MeasurementBookMbookService5Manager:
    """
    Service registry and data aggregator for Civil Engineering Digital Measurement Book (M-Book).
    """
    def __init__(self):
        self.store: Dict[str, MeasurementBookMbookService5Record] = {}
        self.engine = MeasurementBookMbookService5Engine()

    def create(self, op_index: float = 50.0) -> MeasurementBookMbookService5Record:
        rec = MeasurementBookMbookService5Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[MeasurementBookMbookService5Record]:
...
def test_subsystem_measurement_book_mbook_5() -> bool:
    mgr = MeasurementBookMbookService5Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = MeasurementBookMbookService5Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_measurement_book_mbook_5()
    print(f"measurement_book_mbook Part 5: {'PASSED' if ok else 'FAILED'}")
