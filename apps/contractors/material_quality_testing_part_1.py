...
Module: Civil Material Concrete & Bitumen Lab Tests (Part 1)
...
class MaterialQualityTestingService1Engine:
    """
...
class MaterialQualityTestingService1Workflow:
    """
    ...
    def __init__(self, record: MaterialQualityTestingService1Record):
        self.record = record
...
class MaterialQualityTestingService1Manager:
    """
    Service registry and data aggregator for Civil Material Concrete & Bitumen Lab Tests.
    """
    def __init__(self):
        self.store: Dict[str, MaterialQualityTestingService1Record] = {}
        self.engine = MaterialQualityTestingService1Engine()

    def create(self, op_index: float = 50.0) -> MaterialQualityTestingService1Record:
        rec = MaterialQualityTestingService1Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[MaterialQualityTestingService1Record]:
...
def test_subsystem_material_quality_testing_1() -> bool:
    mgr = MaterialQualityTestingService1Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = MaterialQualityTestingService1Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_material_quality_testing_1()
    print(f"material_quality_testing Part 1: {'PASSED' if ok else 'FAILED'}")
