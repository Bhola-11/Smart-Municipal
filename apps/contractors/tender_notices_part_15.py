...
Module: Notice Inviting Tender (NIT) Publication Engine (Part 15)
...
class TenderNoticesService15Engine:
    """
...
class TenderNoticesService15Workflow:
    """
    ...
    def __init__(self, record: TenderNoticesService15Record):
        self.record = record
...
class TenderNoticesService15Manager:
    """
    Service registry and data aggregator for Notice Inviting Tender (NIT) Publication Engine.
    """
    def __init__(self):
        self.store: Dict[str, TenderNoticesService15Record] = {}
        self.engine = TenderNoticesService15Engine()

    def create(self, op_index: float = 50.0) -> TenderNoticesService15Record:
        rec = TenderNoticesService15Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[TenderNoticesService15Record]:
...
def test_subsystem_tender_notices_15() -> bool:
    mgr = TenderNoticesService15Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = TenderNoticesService15Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_tender_notices_15()
    print(f"tender_notices Part 15: {'PASSED' if ok else 'FAILED'}")
