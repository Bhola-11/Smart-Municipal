...
Module: Notice Inviting Tender (NIT) Publication Engine (Part 21)
...
class TenderNoticesService21Engine:
    """
...
class TenderNoticesService21Workflow:
    """
    ...
    def __init__(self, record: TenderNoticesService21Record):
        self.record = record
...
class TenderNoticesService21Manager:
    """
    Service registry and data aggregator for Notice Inviting Tender (NIT) Publication Engine.
    """
    def __init__(self):
        self.store: Dict[str, TenderNoticesService21Record] = {}
        self.engine = TenderNoticesService21Engine()

    def create(self, op_index: float = 50.0) -> TenderNoticesService21Record:
        rec = TenderNoticesService21Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[TenderNoticesService21Record]:
...
def test_subsystem_tender_notices_21() -> bool:
    mgr = TenderNoticesService21Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = TenderNoticesService21Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_tender_notices_21()
    print(f"tender_notices Part 21: {'PASSED' if ok else 'FAILED'}")
