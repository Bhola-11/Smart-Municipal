...
Module: Civil Material Concrete & Bitumen Lab Tests (Part 24)
...
class MaterialQualityTestingService24Engine:
    """
...
class MaterialQualityTestingService24Workflow:
    """
    ...
    def __init__(self, record: MaterialQualityTestingService24Record):
        self.record = record
...
class MaterialQualityTestingService24Manager:
    """
    Service registry and data aggregator for Civil Material Concrete & Bitumen Lab Tests.
    """
    def __init__(self):
        self.store: Dict[str, MaterialQualityTestingService24Record] = {}
        self.engine = MaterialQualityTestingService24Engine()

    def create(self, op_index: float = 50.0) -> MaterialQualityTestingService24Record:
        rec = MaterialQualityTestingService24Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[MaterialQualityTestingService24Record]:
...
def test_subsystem_material_quality_testing_24() -> bool:
    mgr = MaterialQualityTestingService24Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = MaterialQualityTestingService24Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_material_quality_testing_24()
    print(f"material_quality_testing Part 24: {'PASSED' if ok else 'FAILED'}")
