...
Module: Civil Material Concrete & Bitumen Lab Tests (Part 17)
...
class MaterialQualityTestingService17Engine:
    """
...
class MaterialQualityTestingService17Workflow:
    """
    ...
    def __init__(self, record: MaterialQualityTestingService17Record):
        self.record = record
...
class MaterialQualityTestingService17Manager:
    """
    Service registry and data aggregator for Civil Material Concrete & Bitumen Lab Tests.
    """
    def __init__(self):
        self.store: Dict[str, MaterialQualityTestingService17Record] = {}
        self.engine = MaterialQualityTestingService17Engine()

    def create(self, op_index: float = 50.0) -> MaterialQualityTestingService17Record:
        rec = MaterialQualityTestingService17Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[MaterialQualityTestingService17Record]:
...
def test_subsystem_material_quality_testing_17() -> bool:
    mgr = MaterialQualityTestingService17Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = MaterialQualityTestingService17Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_material_quality_testing_17()
    print(f"material_quality_testing Part 17: {'PASSED' if ok else 'FAILED'}")
