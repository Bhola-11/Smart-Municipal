...
Module: Civil Material Concrete & Bitumen Lab Tests (Part 25)
...
class MaterialQualityTestingService25Engine:
    """
...
class MaterialQualityTestingService25Workflow:
    """
    ...
    def __init__(self, record: MaterialQualityTestingService25Record):
        self.record = record
...
class MaterialQualityTestingService25Manager:
    """
    Service registry and data aggregator for Civil Material Concrete & Bitumen Lab Tests.
    """
    def __init__(self):
        self.store: Dict[str, MaterialQualityTestingService25Record] = {}
        self.engine = MaterialQualityTestingService25Engine()

    def create(self, op_index: float = 50.0) -> MaterialQualityTestingService25Record:
        rec = MaterialQualityTestingService25Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[MaterialQualityTestingService25Record]:
...
def test_subsystem_material_quality_testing_25() -> bool:
    mgr = MaterialQualityTestingService25Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = MaterialQualityTestingService25Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_material_quality_testing_25()
    print(f"material_quality_testing Part 25: {'PASSED' if ok else 'FAILED'}")
