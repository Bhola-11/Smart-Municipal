...
Module: Civil Material Concrete & Bitumen Lab Tests (Part 4)
...
class MaterialQualityTestingService4Engine:
    """
...
class MaterialQualityTestingService4Workflow:
    """
    ...
    def __init__(self, record: MaterialQualityTestingService4Record):
        self.record = record
...
class MaterialQualityTestingService4Manager:
    """
    Service registry and data aggregator for Civil Material Concrete & Bitumen Lab Tests.
    """
    def __init__(self):
        self.store: Dict[str, MaterialQualityTestingService4Record] = {}
        self.engine = MaterialQualityTestingService4Engine()

    def create(self, op_index: float = 50.0) -> MaterialQualityTestingService4Record:
        rec = MaterialQualityTestingService4Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[MaterialQualityTestingService4Record]:
...
def test_subsystem_material_quality_testing_4() -> bool:
    mgr = MaterialQualityTestingService4Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = MaterialQualityTestingService4Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_material_quality_testing_4()
    print(f"material_quality_testing Part 4: {'PASSED' if ok else 'FAILED'}")
