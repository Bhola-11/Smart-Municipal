...
Module: Civil Material Concrete & Bitumen Lab Tests (Part 5)
...
class MaterialQualityTestingService5Engine:
    """
...
class MaterialQualityTestingService5Workflow:
    """
    ...
    def __init__(self, record: MaterialQualityTestingService5Record):
        self.record = record
...
class MaterialQualityTestingService5Manager:
    """
    Service registry and data aggregator for Civil Material Concrete & Bitumen Lab Tests.
    """
    def __init__(self):
        self.store: Dict[str, MaterialQualityTestingService5Record] = {}
        self.engine = MaterialQualityTestingService5Engine()

    def create(self, op_index: float = 50.0) -> MaterialQualityTestingService5Record:
        rec = MaterialQualityTestingService5Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[MaterialQualityTestingService5Record]:
...
def test_subsystem_material_quality_testing_5() -> bool:
    mgr = MaterialQualityTestingService5Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = MaterialQualityTestingService5Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_material_quality_testing_5()
    print(f"material_quality_testing Part 5: {'PASSED' if ok else 'FAILED'}")
