"""
CivicFlow Enterprise Production: vendor_tendering_flow Part 87
...
@dataclass
class VendorTenderingFlowService87Record:
    record_id: str = field(default_factory=lambda: str(uuid.uuid4()))
    entity_code: str = "vendor_tendering_flow_87"
...
class VendorTenderingFlowService87Engine:
...
class VendorTenderingFlowService87Workflow:
    STATES = ["SUBMITTED", "INSPECTION_PENDING", "VERIFIED_APPROVED", "IN_EXECUTION", "QUALITY_AUDIT", "COMPLETED_CERTIFIED"]

    def __init__(self, record: VendorTenderingFlowService87Record):
        self.record = record
...
class VendorTenderingFlowService87Manager:
    def __init__(self):
        self.store: Dict[str, VendorTenderingFlowService87Record] = {}
        self.engine = VendorTenderingFlowService87Engine()

    def create(self, op_index: float = 50.0) -> VendorTenderingFlowService87Record:
        rec = VendorTenderingFlowService87Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[VendorTenderingFlowService87Record]:
