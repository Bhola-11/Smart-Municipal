...
Module: Contractor Technical Bid Scoring Matrix (Part 30)
...
class BidTechnicalEvalService30Engine:
    """
...
class BidTechnicalEvalService30Workflow:
    """
    ...
    def __init__(self, record: BidTechnicalEvalService30Record):
        self.record = record
...
class BidTechnicalEvalService30Manager:
    """
    Service registry and data aggregator for Contractor Technical Bid Scoring Matrix.
    """
    def __init__(self):
        self.store: Dict[str, BidTechnicalEvalService30Record] = {}
        self.engine = BidTechnicalEvalService30Engine()

    def create(self, op_index: float = 50.0) -> BidTechnicalEvalService30Record:
        rec = BidTechnicalEvalService30Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[BidTechnicalEvalService30Record]:
...
def test_subsystem_bid_technical_eval_30() -> bool:
    mgr = BidTechnicalEvalService30Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = BidTechnicalEvalService30Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_bid_technical_eval_30()
    print(f"bid_technical_eval Part 30: {'PASSED' if ok else 'FAILED'}")
