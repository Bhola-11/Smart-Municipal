...
Module: Civil Material Concrete & Bitumen Lab Tests (Part 2)
...
class MaterialQualityTestingService2Engine:
    """
...
class MaterialQualityTestingService2Workflow:
    """
    ...
    def __init__(self, record: MaterialQualityTestingService2Record):
        self.record = record
...
class MaterialQualityTestingService2Manager:
    """
    Service registry and data aggregator for Civil Material Concrete & Bitumen Lab Tests.
    """
    def __init__(self):
        self.store: Dict[str, MaterialQualityTestingService2Record] = {}
        self.engine = MaterialQualityTestingService2Engine()

    def create(self, op_index: float = 50.0) -> MaterialQualityTestingService2Record:
        rec = MaterialQualityTestingService2Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[MaterialQualityTestingService2Record]:
...
def test_subsystem_material_quality_testing_2() -> bool:
    mgr = MaterialQualityTestingService2Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = MaterialQualityTestingService2Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_material_quality_testing_2()
    print(f"material_quality_testing Part 2: {'PASSED' if ok else 'FAILED'}")
