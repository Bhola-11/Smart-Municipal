...
Module: Notice Inviting Tender (NIT) Publication Engine (Part 14)
...
class TenderNoticesService14Engine:
    """
...
class TenderNoticesService14Workflow:
    """
    ...
    def __init__(self, record: TenderNoticesService14Record):
        self.record = record
...
class TenderNoticesService14Manager:
    """
    Service registry and data aggregator for Notice Inviting Tender (NIT) Publication Engine.
    """
    def __init__(self):
        self.store: Dict[str, TenderNoticesService14Record] = {}
        self.engine = TenderNoticesService14Engine()

    def create(self, op_index: float = 50.0) -> TenderNoticesService14Record:
        rec = TenderNoticesService14Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[TenderNoticesService14Record]:
...
def test_subsystem_tender_notices_14() -> bool:
    mgr = TenderNoticesService14Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = TenderNoticesService14Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_tender_notices_14()
    print(f"tender_notices Part 14: {'PASSED' if ok else 'FAILED'}")
