...
Module: Contractor Technical Bid Scoring Matrix (Part 8)
...
class BidTechnicalEvalService8Engine:
    """
...
class BidTechnicalEvalService8Workflow:
    """
    ...
    def __init__(self, record: BidTechnicalEvalService8Record):
        self.record = record
...
class BidTechnicalEvalService8Manager:
    """
    Service registry and data aggregator for Contractor Technical Bid Scoring Matrix.
    """
    def __init__(self):
        self.store: Dict[str, BidTechnicalEvalService8Record] = {}
        self.engine = BidTechnicalEvalService8Engine()

    def create(self, op_index: float = 50.0) -> BidTechnicalEvalService8Record:
        rec = BidTechnicalEvalService8Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[BidTechnicalEvalService8Record]:
...
def test_subsystem_bid_technical_eval_8() -> bool:
    mgr = BidTechnicalEvalService8Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = BidTechnicalEvalService8Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_bid_technical_eval_8()
    print(f"bid_technical_eval Part 8: {'PASSED' if ok else 'FAILED'}")
