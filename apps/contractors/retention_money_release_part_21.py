...
Module: Contractor Bank Guarantee & Retention Tracking (Part 21)
...
class RetentionMoneyReleaseService21Engine:
    """
...
class RetentionMoneyReleaseService21Workflow:
    """
    ...
    def __init__(self, record: RetentionMoneyReleaseService21Record):
        self.record = record
...
class RetentionMoneyReleaseService21Manager:
    """
    Service registry and data aggregator for Contractor Bank Guarantee & Retention Tracking.
    """
    def __init__(self):
        self.store: Dict[str, RetentionMoneyReleaseService21Record] = {}
        self.engine = RetentionMoneyReleaseService21Engine()

    def create(self, op_index: float = 50.0) -> RetentionMoneyReleaseService21Record:
        rec = RetentionMoneyReleaseService21Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[RetentionMoneyReleaseService21Record]:
...
def test_subsystem_retention_money_release_21() -> bool:
    mgr = RetentionMoneyReleaseService21Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = RetentionMoneyReleaseService21Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_retention_money_release_21()
    print(f"retention_money_release Part 21: {'PASSED' if ok else 'FAILED'}")
