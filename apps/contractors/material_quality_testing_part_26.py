...
Module: Civil Material Concrete & Bitumen Lab Tests (Part 26)
...
class MaterialQualityTestingService26Engine:
    """
...
class MaterialQualityTestingService26Workflow:
    """
    ...
    def __init__(self, record: MaterialQualityTestingService26Record):
        self.record = record
...
class MaterialQualityTestingService26Manager:
    """
    Service registry and data aggregator for Civil Material Concrete & Bitumen Lab Tests.
    """
    def __init__(self):
        self.store: Dict[str, MaterialQualityTestingService26Record] = {}
        self.engine = MaterialQualityTestingService26Engine()

    def create(self, op_index: float = 50.0) -> MaterialQualityTestingService26Record:
        rec = MaterialQualityTestingService26Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[MaterialQualityTestingService26Record]:
...
def test_subsystem_material_quality_testing_26() -> bool:
    mgr = MaterialQualityTestingService26Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = MaterialQualityTestingService26Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_material_quality_testing_26()
    print(f"material_quality_testing Part 26: {'PASSED' if ok else 'FAILED'}")
