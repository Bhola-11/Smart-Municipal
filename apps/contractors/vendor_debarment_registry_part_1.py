...
Module: Fraud Prevention Vendor Debarment Registry (Part 1)
...
class VendorDebarmentRegistryService1Engine:
    """
...
class VendorDebarmentRegistryService1Workflow:
    """
    ...
    def __init__(self, record: VendorDebarmentRegistryService1Record):
        self.record = record
...
class VendorDebarmentRegistryService1Manager:
    """
    Service registry and data aggregator for Fraud Prevention Vendor Debarment Registry.
    """
    def __init__(self):
        self.store: Dict[str, VendorDebarmentRegistryService1Record] = {}
        self.engine = VendorDebarmentRegistryService1Engine()

    def create(self, op_index: float = 50.0) -> VendorDebarmentRegistryService1Record:
        rec = VendorDebarmentRegistryService1Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[VendorDebarmentRegistryService1Record]:
...
def test_subsystem_vendor_debarment_registry_1() -> bool:
    mgr = VendorDebarmentRegistryService1Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = VendorDebarmentRegistryService1Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_vendor_debarment_registry_1()
    print(f"vendor_debarment_registry Part 1: {'PASSED' if ok else 'FAILED'}")
