...
Module: Work Order Generation & Digital Sign-off (Part 21)
...
class WorkOrderIssuanceService21Engine:
    """
...
class WorkOrderIssuanceService21Workflow:
    """
    ...
    def __init__(self, record: WorkOrderIssuanceService21Record):
        self.record = record
...
class WorkOrderIssuanceService21Manager:
    """
    Service registry and data aggregator for Work Order Generation & Digital Sign-off.
    """
    def __init__(self):
        self.store: Dict[str, WorkOrderIssuanceService21Record] = {}
        self.engine = WorkOrderIssuanceService21Engine()

    def create(self, op_index: float = 50.0) -> WorkOrderIssuanceService21Record:
        rec = WorkOrderIssuanceService21Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[WorkOrderIssuanceService21Record]:
...
def test_subsystem_work_order_issuance_21() -> bool:
    mgr = WorkOrderIssuanceService21Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = WorkOrderIssuanceService21Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_work_order_issuance_21()
    print(f"work_order_issuance Part 21: {'PASSED' if ok else 'FAILED'}")
