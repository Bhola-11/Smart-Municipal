...
Module: Contractor Bank Guarantee & Retention Tracking (Part 17)
...
class RetentionMoneyReleaseService17Engine:
    """
...
class RetentionMoneyReleaseService17Workflow:
    """
    ...
    def __init__(self, record: RetentionMoneyReleaseService17Record):
        self.record = record
...
class RetentionMoneyReleaseService17Manager:
    """
    Service registry and data aggregator for Contractor Bank Guarantee & Retention Tracking.
    """
    def __init__(self):
        self.store: Dict[str, RetentionMoneyReleaseService17Record] = {}
        self.engine = RetentionMoneyReleaseService17Engine()

    def create(self, op_index: float = 50.0) -> RetentionMoneyReleaseService17Record:
        rec = RetentionMoneyReleaseService17Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[RetentionMoneyReleaseService17Record]:
...
def test_subsystem_retention_money_release_17() -> bool:
    mgr = RetentionMoneyReleaseService17Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = RetentionMoneyReleaseService17Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_retention_money_release_17()
    print(f"retention_money_release Part 17: {'PASSED' if ok else 'FAILED'}")
