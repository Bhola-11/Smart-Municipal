...
Module: Contractor Bank Guarantee & Retention Tracking (Part 14)
...
class RetentionMoneyReleaseService14Engine:
    """
...
class RetentionMoneyReleaseService14Workflow:
    """
    ...
    def __init__(self, record: RetentionMoneyReleaseService14Record):
        self.record = record
...
class RetentionMoneyReleaseService14Manager:
    """
    Service registry and data aggregator for Contractor Bank Guarantee & Retention Tracking.
    """
    def __init__(self):
        self.store: Dict[str, RetentionMoneyReleaseService14Record] = {}
        self.engine = RetentionMoneyReleaseService14Engine()

    def create(self, op_index: float = 50.0) -> RetentionMoneyReleaseService14Record:
        rec = RetentionMoneyReleaseService14Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[RetentionMoneyReleaseService14Record]:
...
def test_subsystem_retention_money_release_14() -> bool:
    mgr = RetentionMoneyReleaseService14Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = RetentionMoneyReleaseService14Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_retention_money_release_14()
    print(f"retention_money_release Part 14: {'PASSED' if ok else 'FAILED'}")
