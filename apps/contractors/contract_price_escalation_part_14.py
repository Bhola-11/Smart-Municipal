...
Module: WPI/CPI Formula Based Price Escalation Index (Part 14)
...
class ContractPriceEscalationService14Engine:
    """
...
class ContractPriceEscalationService14Workflow:
    """
    ...
    def __init__(self, record: ContractPriceEscalationService14Record):
        self.record = record
...
class ContractPriceEscalationService14Manager:
    """
    Service registry and data aggregator for WPI/CPI Formula Based Price Escalation Index.
    """
    def __init__(self):
        self.store: Dict[str, ContractPriceEscalationService14Record] = {}
        self.engine = ContractPriceEscalationService14Engine()

    def create(self, op_index: float = 50.0) -> ContractPriceEscalationService14Record:
        rec = ContractPriceEscalationService14Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[ContractPriceEscalationService14Record]:
...
def test_subsystem_contract_price_escalation_14() -> bool:
    mgr = ContractPriceEscalationService14Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = ContractPriceEscalationService14Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_contract_price_escalation_14()
    print(f"contract_price_escalation Part 14: {'PASSED' if ok else 'FAILED'}")
