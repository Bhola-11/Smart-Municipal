...
Module: Notice Inviting Tender (NIT) Publication Engine (Part 20)
...
class TenderNoticesService20Engine:
    """
...
class TenderNoticesService20Workflow:
    """
    ...
    def __init__(self, record: TenderNoticesService20Record):
        self.record = record
...
class TenderNoticesService20Manager:
    """
    Service registry and data aggregator for Notice Inviting Tender (NIT) Publication Engine.
    """
    def __init__(self):
        self.store: Dict[str, TenderNoticesService20Record] = {}
        self.engine = TenderNoticesService20Engine()

    def create(self, op_index: float = 50.0) -> TenderNoticesService20Record:
        rec = TenderNoticesService20Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[TenderNoticesService20Record]:
...
def test_subsystem_tender_notices_20() -> bool:
    mgr = TenderNoticesService20Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = TenderNoticesService20Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_tender_notices_20()
    print(f"tender_notices Part 20: {'PASSED' if ok else 'FAILED'}")
