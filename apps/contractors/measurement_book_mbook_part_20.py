...
Module: Civil Engineering Digital Measurement Book (M-Book) (Part 20)
...
class MeasurementBookMbookService20Engine:
    """
...
class MeasurementBookMbookService20Workflow:
    """
    ...
    def __init__(self, record: MeasurementBookMbookService20Record):
        self.record = record
...
class MeasurementBookMbookService20Manager:
    """
    Service registry and data aggregator for Civil Engineering Digital Measurement Book (M-Book).
    """
    def __init__(self):
        self.store: Dict[str, MeasurementBookMbookService20Record] = {}
        self.engine = MeasurementBookMbookService20Engine()

    def create(self, op_index: float = 50.0) -> MeasurementBookMbookService20Record:
        rec = MeasurementBookMbookService20Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[MeasurementBookMbookService20Record]:
...
def test_subsystem_measurement_book_mbook_20() -> bool:
    mgr = MeasurementBookMbookService20Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = MeasurementBookMbookService20Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_measurement_book_mbook_20()
    print(f"measurement_book_mbook Part 20: {'PASSED' if ok else 'FAILED'}")
