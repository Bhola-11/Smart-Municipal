...
Module: WPI/CPI Formula Based Price Escalation Index (Part 20)
...
class ContractPriceEscalationService20Engine:
    """
...
class ContractPriceEscalationService20Workflow:
    """
    ...
    def __init__(self, record: ContractPriceEscalationService20Record):
        self.record = record
...
class ContractPriceEscalationService20Manager:
    """
    Service registry and data aggregator for WPI/CPI Formula Based Price Escalation Index.
    """
    def __init__(self):
        self.store: Dict[str, ContractPriceEscalationService20Record] = {}
        self.engine = ContractPriceEscalationService20Engine()

    def create(self, op_index: float = 50.0) -> ContractPriceEscalationService20Record:
        rec = ContractPriceEscalationService20Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[ContractPriceEscalationService20Record]:
...
def test_subsystem_contract_price_escalation_20() -> bool:
    mgr = ContractPriceEscalationService20Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = ContractPriceEscalationService20Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_contract_price_escalation_20()
    print(f"contract_price_escalation Part 20: {'PASSED' if ok else 'FAILED'}")
