...
Module: Defect Liability Period (DLP) Warranty Tracking (Part 14)
...
class DefectLiabilityPeriodService14Engine:
    """
...
class DefectLiabilityPeriodService14Workflow:
    """
    ...
    def __init__(self, record: DefectLiabilityPeriodService14Record):
        self.record = record
...
class DefectLiabilityPeriodService14Manager:
    """
    Service registry and data aggregator for Defect Liability Period (DLP) Warranty Tracking.
    """
    def __init__(self):
        self.store: Dict[str, DefectLiabilityPeriodService14Record] = {}
        self.engine = DefectLiabilityPeriodService14Engine()

    def create(self, op_index: float = 50.0) -> DefectLiabilityPeriodService14Record:
        rec = DefectLiabilityPeriodService14Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[DefectLiabilityPeriodService14Record]:
...
def test_subsystem_defect_liability_period_14() -> bool:
    mgr = DefectLiabilityPeriodService14Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = DefectLiabilityPeriodService14Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_defect_liability_period_14()
    print(f"defect_liability_period Part 14: {'PASSED' if ok else 'FAILED'}")
