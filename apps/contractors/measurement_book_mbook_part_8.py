...
Module: Civil Engineering Digital Measurement Book (M-Book) (Part 8)
...
class MeasurementBookMbookService8Engine:
    """
...
class MeasurementBookMbookService8Workflow:
    """
    ...
    def __init__(self, record: MeasurementBookMbookService8Record):
        self.record = record
...
class MeasurementBookMbookService8Manager:
    """
    Service registry and data aggregator for Civil Engineering Digital Measurement Book (M-Book).
    """
    def __init__(self):
        self.store: Dict[str, MeasurementBookMbookService8Record] = {}
        self.engine = MeasurementBookMbookService8Engine()

    def create(self, op_index: float = 50.0) -> MeasurementBookMbookService8Record:
        rec = MeasurementBookMbookService8Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[MeasurementBookMbookService8Record]:
...
def test_subsystem_measurement_book_mbook_8() -> bool:
    mgr = MeasurementBookMbookService8Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = MeasurementBookMbookService8Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_measurement_book_mbook_8()
    print(f"measurement_book_mbook Part 8: {'PASSED' if ok else 'FAILED'}")
