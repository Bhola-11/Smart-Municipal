...
Module: Notice Inviting Tender (NIT) Publication Engine (Part 4)
...
class TenderNoticesService4Engine:
    """
...
class TenderNoticesService4Workflow:
    """
    ...
    def __init__(self, record: TenderNoticesService4Record):
        self.record = record
...
class TenderNoticesService4Manager:
    """
    Service registry and data aggregator for Notice Inviting Tender (NIT) Publication Engine.
    """
    def __init__(self):
        self.store: Dict[str, TenderNoticesService4Record] = {}
        self.engine = TenderNoticesService4Engine()

    def create(self, op_index: float = 50.0) -> TenderNoticesService4Record:
        rec = TenderNoticesService4Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[TenderNoticesService4Record]:
...
def test_subsystem_tender_notices_4() -> bool:
    mgr = TenderNoticesService4Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = TenderNoticesService4Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_tender_notices_4()
    print(f"tender_notices Part 4: {'PASSED' if ok else 'FAILED'}")
