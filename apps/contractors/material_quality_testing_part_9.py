...
Module: Civil Material Concrete & Bitumen Lab Tests (Part 9)
...
class MaterialQualityTestingService9Engine:
    """
...
class MaterialQualityTestingService9Workflow:
    """
    ...
    def __init__(self, record: MaterialQualityTestingService9Record):
        self.record = record
...
class MaterialQualityTestingService9Manager:
    """
    Service registry and data aggregator for Civil Material Concrete & Bitumen Lab Tests.
    """
    def __init__(self):
        self.store: Dict[str, MaterialQualityTestingService9Record] = {}
        self.engine = MaterialQualityTestingService9Engine()

    def create(self, op_index: float = 50.0) -> MaterialQualityTestingService9Record:
        rec = MaterialQualityTestingService9Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[MaterialQualityTestingService9Record]:
...
def test_subsystem_material_quality_testing_9() -> bool:
    mgr = MaterialQualityTestingService9Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = MaterialQualityTestingService9Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_material_quality_testing_9()
    print(f"material_quality_testing Part 9: {'PASSED' if ok else 'FAILED'}")
