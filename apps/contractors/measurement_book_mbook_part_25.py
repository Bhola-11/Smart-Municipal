...
Module: Civil Engineering Digital Measurement Book (M-Book) (Part 25)
...
class MeasurementBookMbookService25Engine:
    """
...
class MeasurementBookMbookService25Workflow:
    """
    ...
    def __init__(self, record: MeasurementBookMbookService25Record):
        self.record = record
...
class MeasurementBookMbookService25Manager:
    """
    Service registry and data aggregator for Civil Engineering Digital Measurement Book (M-Book).
    """
    def __init__(self):
        self.store: Dict[str, MeasurementBookMbookService25Record] = {}
        self.engine = MeasurementBookMbookService25Engine()

    def create(self, op_index: float = 50.0) -> MeasurementBookMbookService25Record:
        rec = MeasurementBookMbookService25Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[MeasurementBookMbookService25Record]:
...
def test_subsystem_measurement_book_mbook_25() -> bool:
    mgr = MeasurementBookMbookService25Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = MeasurementBookMbookService25Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_measurement_book_mbook_25()
    print(f"measurement_book_mbook Part 25: {'PASSED' if ok else 'FAILED'}")
