...
Module: Vendor Payment Voucher & Check Printing (Part 8)
...
class ContractorDisbursementsService8Engine:
    """
...
class ContractorDisbursementsService8Workflow:
    """
    ...
    def __init__(self, record: ContractorDisbursementsService8Record):
        self.record = record
...
class ContractorDisbursementsService8Manager:
    """
    Service registry and data aggregator for Vendor Payment Voucher & Check Printing.
    """
    def __init__(self):
        self.store: Dict[str, ContractorDisbursementsService8Record] = {}
        self.engine = ContractorDisbursementsService8Engine()

    def create(self, op_index: float = 50.0) -> ContractorDisbursementsService8Record:
        rec = ContractorDisbursementsService8Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[ContractorDisbursementsService8Record]:
...
def test_subsystem_contractor_disbursements_8() -> bool:
    mgr = ContractorDisbursementsService8Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = ContractorDisbursementsService8Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_contractor_disbursements_8()
    print(f"contractor_disbursements Part 8: {'PASSED' if ok else 'FAILED'}")
