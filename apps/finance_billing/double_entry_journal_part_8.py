...
Module: General Ledger Double-Entry Journal Engine (Part 8)
...
class DoubleEntryJournalService8Engine:
    """
...
class DoubleEntryJournalService8Workflow:
    """
    ...
    def __init__(self, record: DoubleEntryJournalService8Record):
        self.record = record
...
class DoubleEntryJournalService8Manager:
    """
    Service registry and data aggregator for General Ledger Double-Entry Journal Engine.
    """
    def __init__(self):
        self.store: Dict[str, DoubleEntryJournalService8Record] = {}
        self.engine = DoubleEntryJournalService8Engine()

    def create(self, op_index: float = 50.0) -> DoubleEntryJournalService8Record:
        rec = DoubleEntryJournalService8Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[DoubleEntryJournalService8Record]:
...
def test_subsystem_double_entry_journal_8() -> bool:
    mgr = DoubleEntryJournalService8Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = DoubleEntryJournalService8Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_double_entry_journal_8()
    print(f"double_entry_journal Part 8: {'PASSED' if ok else 'FAILED'}")
