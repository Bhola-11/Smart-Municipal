...
Module: Volumetric Water Consumption Tariff Calculator (Part 8)
...
class WaterTariffBillingService8Engine:
    """
...
class WaterTariffBillingService8Workflow:
    """
    ...
    def __init__(self, record: WaterTariffBillingService8Record):
        self.record = record
...
class WaterTariffBillingService8Manager:
    """
    Service registry and data aggregator for Volumetric Water Consumption Tariff Calculator.
    """
    def __init__(self):
        self.store: Dict[str, WaterTariffBillingService8Record] = {}
        self.engine = WaterTariffBillingService8Engine()

    def create(self, op_index: float = 50.0) -> WaterTariffBillingService8Record:
        rec = WaterTariffBillingService8Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[WaterTariffBillingService8Record]:
...
def test_subsystem_water_tariff_billing_8() -> bool:
    mgr = WaterTariffBillingService8Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = WaterTariffBillingService8Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_water_tariff_billing_8()
    print(f"water_tariff_billing Part 8: {'PASSED' if ok else 'FAILED'}")
