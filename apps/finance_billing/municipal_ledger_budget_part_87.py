"""
CivicFlow Enterprise Production: municipal_ledger_budget Part 87
...
@dataclass
class MunicipalLedgerBudgetService87Record:
    record_id: str = field(default_factory=lambda: str(uuid.uuid4()))
    entity_code: str = "municipal_ledger_budget_87"
...
class MunicipalLedgerBudgetService87Engine:
...
class MunicipalLedgerBudgetService87Workflow:
    STATES = ["SUBMITTED", "INSPECTION_PENDING", "VERIFIED_APPROVED", "IN_EXECUTION", "QUALITY_AUDIT", "COMPLETED_CERTIFIED"]

    def __init__(self, record: MunicipalLedgerBudgetService87Record):
        self.record = record
...
class MunicipalLedgerBudgetService87Manager:
    def __init__(self):
        self.store: Dict[str, MunicipalLedgerBudgetService87Record] = {}
        self.engine = MunicipalLedgerBudgetService87Engine()

    def create(self, op_index: float = 50.0) -> MunicipalLedgerBudgetService87Record:
        rec = MunicipalLedgerBudgetService87Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[MunicipalLedgerBudgetService87Record]:
