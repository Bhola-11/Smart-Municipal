...
Module: Ward Sub-Office Petty Cash Float Ledgers (Part 10)
...
class PettyCashDisbursementsService10Engine:
    """
...
class PettyCashDisbursementsService10Workflow:
    """
    ...
    def __init__(self, record: PettyCashDisbursementsService10Record):
        self.record = record
...
class PettyCashDisbursementsService10Manager:
    """
    Service registry and data aggregator for Ward Sub-Office Petty Cash Float Ledgers.
    """
    def __init__(self):
        self.store: Dict[str, PettyCashDisbursementsService10Record] = {}
        self.engine = PettyCashDisbursementsService10Engine()

    def create(self, op_index: float = 50.0) -> PettyCashDisbursementsService10Record:
        rec = PettyCashDisbursementsService10Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[PettyCashDisbursementsService10Record]:
...
def test_subsystem_petty_cash_disbursements_10() -> bool:
    mgr = PettyCashDisbursementsService10Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = PettyCashDisbursementsService10Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_petty_cash_disbursements_10()
    print(f"petty_cash_disbursements Part 10: {'PASSED' if ok else 'FAILED'}")
