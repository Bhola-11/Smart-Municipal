...
Module: Ward Discretionary Fund Utilization Ledger (Part 8)
...
class WardFundUtilizationService8Engine:
    """
...
class WardFundUtilizationService8Workflow:
    """
    ...
    def __init__(self, record: WardFundUtilizationService8Record):
        self.record = record
...
class WardFundUtilizationService8Manager:
    """
    Service registry and data aggregator for Ward Discretionary Fund Utilization Ledger.
    """
    def __init__(self):
        self.store: Dict[str, WardFundUtilizationService8Record] = {}
        self.engine = WardFundUtilizationService8Engine()

    def create(self, op_index: float = 50.0) -> WardFundUtilizationService8Record:
        rec = WardFundUtilizationService8Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[WardFundUtilizationService8Record]:
...
def test_subsystem_ward_fund_utilization_8() -> bool:
    mgr = WardFundUtilizationService8Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = WardFundUtilizationService8Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_ward_fund_utilization_8()
    print(f"ward_fund_utilization Part 8: {'PASSED' if ok else 'FAILED'}")
