...
Module: Unit Area Value Property Tax Assessment Engine (Part 14)
...
class PropertyTaxAssessmentService14Engine:
    """
...
class PropertyTaxAssessmentService14Workflow:
    """
    ...
    def __init__(self, record: PropertyTaxAssessmentService14Record):
        self.record = record
...
class PropertyTaxAssessmentService14Manager:
    """
    Service registry and data aggregator for Unit Area Value Property Tax Assessment Engine.
    """
    def __init__(self):
        self.store: Dict[str, PropertyTaxAssessmentService14Record] = {}
        self.engine = PropertyTaxAssessmentService14Engine()

    def create(self, op_index: float = 50.0) -> PropertyTaxAssessmentService14Record:
        rec = PropertyTaxAssessmentService14Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[PropertyTaxAssessmentService14Record]:
...
def test_subsystem_property_tax_assessment_14() -> bool:
    mgr = PropertyTaxAssessmentService14Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = PropertyTaxAssessmentService14Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_property_tax_assessment_14()
    print(f"property_tax_assessment Part 14: {'PASSED' if ok else 'FAILED'}")
