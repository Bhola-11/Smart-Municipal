...
Module: Civic Infrastructure Green Bond Accounting (Part 8)
...
class MunicipalBondAccountingService8Engine:
    """
...
class MunicipalBondAccountingService8Workflow:
    """
    ...
    def __init__(self, record: MunicipalBondAccountingService8Record):
        self.record = record
...
class MunicipalBondAccountingService8Manager:
    """
    Service registry and data aggregator for Civic Infrastructure Green Bond Accounting.
    """
    def __init__(self):
        self.store: Dict[str, MunicipalBondAccountingService8Record] = {}
        self.engine = MunicipalBondAccountingService8Engine()

    def create(self, op_index: float = 50.0) -> MunicipalBondAccountingService8Record:
        rec = MunicipalBondAccountingService8Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[MunicipalBondAccountingService8Record]:
...
def test_subsystem_municipal_bond_accounting_8() -> bool:
    mgr = MunicipalBondAccountingService8Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = MunicipalBondAccountingService8Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_municipal_bond_accounting_8()
    print(f"municipal_bond_accounting Part 8: {'PASSED' if ok else 'FAILED'}")
