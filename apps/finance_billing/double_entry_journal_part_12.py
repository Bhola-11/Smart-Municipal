...
Module: General Ledger Double-Entry Journal Engine (Part 12)
...
class DoubleEntryJournalService12Engine:
    """
...
class DoubleEntryJournalService12Workflow:
    """
    ...
    def __init__(self, record: DoubleEntryJournalService12Record):
        self.record = record
...
class DoubleEntryJournalService12Manager:
    """
    Service registry and data aggregator for General Ledger Double-Entry Journal Engine.
    """
    def __init__(self):
        self.store: Dict[str, DoubleEntryJournalService12Record] = {}
        self.engine = DoubleEntryJournalService12Engine()

    def create(self, op_index: float = 50.0) -> DoubleEntryJournalService12Record:
        rec = DoubleEntryJournalService12Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[DoubleEntryJournalService12Record]:
...
def test_subsystem_double_entry_journal_12() -> bool:
    mgr = DoubleEntryJournalService12Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = DoubleEntryJournalService12Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_double_entry_journal_12()
    print(f"double_entry_journal Part 12: {'PASSED' if ok else 'FAILED'}")
