...
Module: State & Central Civic Grants Compliance Monitor (Part 4)
...
class GrantAidComplianceService4Engine:
    """
...
class GrantAidComplianceService4Workflow:
    """
    ...
    def __init__(self, record: GrantAidComplianceService4Record):
        self.record = record
...
class GrantAidComplianceService4Manager:
    """
    Service registry and data aggregator for State & Central Civic Grants Compliance Monitor.
    """
    def __init__(self):
        self.store: Dict[str, GrantAidComplianceService4Record] = {}
        self.engine = GrantAidComplianceService4Engine()

    def create(self, op_index: float = 50.0) -> GrantAidComplianceService4Record:
        rec = GrantAidComplianceService4Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[GrantAidComplianceService4Record]:
...
def test_subsystem_grant_aid_compliance_4() -> bool:
    mgr = GrantAidComplianceService4Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = GrantAidComplianceService4Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_grant_aid_compliance_4()
    print(f"grant_aid_compliance Part 4: {'PASSED' if ok else 'FAILED'}")
