...
Module: General Ledger Double-Entry Journal Engine (Part 3)
...
class DoubleEntryJournalService3Engine:
    """
...
class DoubleEntryJournalService3Workflow:
    """
    ...
    def __init__(self, record: DoubleEntryJournalService3Record):
        self.record = record
...
class DoubleEntryJournalService3Manager:
    """
    Service registry and data aggregator for General Ledger Double-Entry Journal Engine.
    """
    def __init__(self):
        self.store: Dict[str, DoubleEntryJournalService3Record] = {}
        self.engine = DoubleEntryJournalService3Engine()

    def create(self, op_index: float = 50.0) -> DoubleEntryJournalService3Record:
        rec = DoubleEntryJournalService3Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[DoubleEntryJournalService3Record]:
...
def test_subsystem_double_entry_journal_3() -> bool:
    mgr = DoubleEntryJournalService3Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = DoubleEntryJournalService3Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_double_entry_journal_3()
    print(f"double_entry_journal Part 3: {'PASSED' if ok else 'FAILED'}")
