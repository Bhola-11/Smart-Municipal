...
Module: Tax Deducted at Source (TDS & GST) Ledgers (Part 8)
...
class StatutoryTdsDeductionsService8Engine:
    """
...
class StatutoryTdsDeductionsService8Workflow:
    """
    ...
    def __init__(self, record: StatutoryTdsDeductionsService8Record):
        self.record = record
...
class StatutoryTdsDeductionsService8Manager:
    """
    Service registry and data aggregator for Tax Deducted at Source (TDS & GST) Ledgers.
    """
    def __init__(self):
        self.store: Dict[str, StatutoryTdsDeductionsService8Record] = {}
        self.engine = StatutoryTdsDeductionsService8Engine()

    def create(self, op_index: float = 50.0) -> StatutoryTdsDeductionsService8Record:
        rec = StatutoryTdsDeductionsService8Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[StatutoryTdsDeductionsService8Record]:
...
def test_subsystem_statutory_tds_deductions_8() -> bool:
    mgr = StatutoryTdsDeductionsService8Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = StatutoryTdsDeductionsService8Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_statutory_tds_deductions_8()
    print(f"statutory_tds_deductions Part 8: {'PASSED' if ok else 'FAILED'}")
