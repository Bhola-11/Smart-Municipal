...
Module: Ward Discretionary Fund Utilization Ledger (Part 14)
...
class WardFundUtilizationService14Engine:
    """
...
class WardFundUtilizationService14Workflow:
    """
    ...
    def __init__(self, record: WardFundUtilizationService14Record):
        self.record = record
...
class WardFundUtilizationService14Manager:
    """
    Service registry and data aggregator for Ward Discretionary Fund Utilization Ledger.
    """
    def __init__(self):
        self.store: Dict[str, WardFundUtilizationService14Record] = {}
        self.engine = WardFundUtilizationService14Engine()

    def create(self, op_index: float = 50.0) -> WardFundUtilizationService14Record:
        rec = WardFundUtilizationService14Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[WardFundUtilizationService14Record]:
...
def test_subsystem_ward_fund_utilization_14() -> bool:
    mgr = WardFundUtilizationService14Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = WardFundUtilizationService14Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_ward_fund_utilization_14()
    print(f"ward_fund_utilization Part 14: {'PASSED' if ok else 'FAILED'}")
