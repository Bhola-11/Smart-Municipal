...
Module: General Ledger Double-Entry Journal Engine (Part 17)
...
class DoubleEntryJournalService17Engine:
    """
...
class DoubleEntryJournalService17Workflow:
    """
    ...
    def __init__(self, record: DoubleEntryJournalService17Record):
        self.record = record
...
class DoubleEntryJournalService17Manager:
    """
    Service registry and data aggregator for General Ledger Double-Entry Journal Engine.
    """
    def __init__(self):
        self.store: Dict[str, DoubleEntryJournalService17Record] = {}
        self.engine = DoubleEntryJournalService17Engine()

    def create(self, op_index: float = 50.0) -> DoubleEntryJournalService17Record:
        rec = DoubleEntryJournalService17Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[DoubleEntryJournalService17Record]:
...
def test_subsystem_double_entry_journal_17() -> bool:
    mgr = DoubleEntryJournalService17Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = DoubleEntryJournalService17Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_double_entry_journal_17()
    print(f"double_entry_journal Part 17: {'PASSED' if ok else 'FAILED'}")
