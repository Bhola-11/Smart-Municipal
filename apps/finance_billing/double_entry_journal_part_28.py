...
Module: General Ledger Double-Entry Journal Engine (Part 28)
...
class DoubleEntryJournalService28Engine:
    """
...
class DoubleEntryJournalService28Workflow:
    """
    ...
    def __init__(self, record: DoubleEntryJournalService28Record):
        self.record = record
...
class DoubleEntryJournalService28Manager:
    """
    Service registry and data aggregator for General Ledger Double-Entry Journal Engine.
    """
    def __init__(self):
        self.store: Dict[str, DoubleEntryJournalService28Record] = {}
        self.engine = DoubleEntryJournalService28Engine()

    def create(self, op_index: float = 50.0) -> DoubleEntryJournalService28Record:
        rec = DoubleEntryJournalService28Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[DoubleEntryJournalService28Record]:
...
def test_subsystem_double_entry_journal_28() -> bool:
    mgr = DoubleEntryJournalService28Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = DoubleEntryJournalService28Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_double_entry_journal_28()
    print(f"double_entry_journal Part 28: {'PASSED' if ok else 'FAILED'}")
