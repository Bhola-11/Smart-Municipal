...
Module: General Ledger Double-Entry Journal Engine (Part 23)
...
class DoubleEntryJournalService23Engine:
    """
...
class DoubleEntryJournalService23Workflow:
    """
    ...
    def __init__(self, record: DoubleEntryJournalService23Record):
        self.record = record
...
class DoubleEntryJournalService23Manager:
    """
    Service registry and data aggregator for General Ledger Double-Entry Journal Engine.
    """
    def __init__(self):
        self.store: Dict[str, DoubleEntryJournalService23Record] = {}
        self.engine = DoubleEntryJournalService23Engine()

    def create(self, op_index: float = 50.0) -> DoubleEntryJournalService23Record:
        rec = DoubleEntryJournalService23Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[DoubleEntryJournalService23Record]:
...
def test_subsystem_double_entry_journal_23() -> bool:
    mgr = DoubleEntryJournalService23Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = DoubleEntryJournalService23Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_double_entry_journal_23()
    print(f"double_entry_journal Part 23: {'PASSED' if ok else 'FAILED'}")
