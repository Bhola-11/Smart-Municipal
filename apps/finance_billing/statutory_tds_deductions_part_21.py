...
Module: Tax Deducted at Source (TDS & GST) Ledgers (Part 21)
...
class StatutoryTdsDeductionsService21Engine:
    """
...
class StatutoryTdsDeductionsService21Workflow:
    """
    ...
    def __init__(self, record: StatutoryTdsDeductionsService21Record):
        self.record = record
...
class StatutoryTdsDeductionsService21Manager:
    """
    Service registry and data aggregator for Tax Deducted at Source (TDS & GST) Ledgers.
    """
    def __init__(self):
        self.store: Dict[str, StatutoryTdsDeductionsService21Record] = {}
        self.engine = StatutoryTdsDeductionsService21Engine()

    def create(self, op_index: float = 50.0) -> StatutoryTdsDeductionsService21Record:
        rec = StatutoryTdsDeductionsService21Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[StatutoryTdsDeductionsService21Record]:
...
def test_subsystem_statutory_tds_deductions_21() -> bool:
    mgr = StatutoryTdsDeductionsService21Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = StatutoryTdsDeductionsService21Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_statutory_tds_deductions_21()
    print(f"statutory_tds_deductions Part 21: {'PASSED' if ok else 'FAILED'}")
