...
Module: Ward Sub-Office Petty Cash Float Ledgers (Part 3)
...
class PettyCashDisbursementsService3Engine:
    """
...
class PettyCashDisbursementsService3Workflow:
    """
    ...
    def __init__(self, record: PettyCashDisbursementsService3Record):
        self.record = record
...
class PettyCashDisbursementsService3Manager:
    """
    Service registry and data aggregator for Ward Sub-Office Petty Cash Float Ledgers.
    """
    def __init__(self):
        self.store: Dict[str, PettyCashDisbursementsService3Record] = {}
        self.engine = PettyCashDisbursementsService3Engine()

    def create(self, op_index: float = 50.0) -> PettyCashDisbursementsService3Record:
        rec = PettyCashDisbursementsService3Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[PettyCashDisbursementsService3Record]:
...
def test_subsystem_petty_cash_disbursements_3() -> bool:
    mgr = PettyCashDisbursementsService3Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = PettyCashDisbursementsService3Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_petty_cash_disbursements_3()
    print(f"petty_cash_disbursements Part 3: {'PASSED' if ok else 'FAILED'}")
