...
Module: Ward Sub-Office Petty Cash Float Ledgers (Part 4)
...
class PettyCashDisbursementsService4Engine:
    """
...
class PettyCashDisbursementsService4Workflow:
    """
    ...
    def __init__(self, record: PettyCashDisbursementsService4Record):
        self.record = record
...
class PettyCashDisbursementsService4Manager:
    """
    Service registry and data aggregator for Ward Sub-Office Petty Cash Float Ledgers.
    """
    def __init__(self):
        self.store: Dict[str, PettyCashDisbursementsService4Record] = {}
        self.engine = PettyCashDisbursementsService4Engine()

    def create(self, op_index: float = 50.0) -> PettyCashDisbursementsService4Record:
        rec = PettyCashDisbursementsService4Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[PettyCashDisbursementsService4Record]:
...
def test_subsystem_petty_cash_disbursements_4() -> bool:
    mgr = PettyCashDisbursementsService4Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = PettyCashDisbursementsService4Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_petty_cash_disbursements_4()
    print(f"petty_cash_disbursements Part 4: {'PASSED' if ok else 'FAILED'}")
