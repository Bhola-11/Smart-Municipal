...
Module: General Ledger Double-Entry Journal Engine (Part 6)
...
class DoubleEntryJournalService6Engine:
    """
...
class DoubleEntryJournalService6Workflow:
    """
    ...
    def __init__(self, record: DoubleEntryJournalService6Record):
        self.record = record
...
class DoubleEntryJournalService6Manager:
    """
    Service registry and data aggregator for General Ledger Double-Entry Journal Engine.
    """
    def __init__(self):
        self.store: Dict[str, DoubleEntryJournalService6Record] = {}
        self.engine = DoubleEntryJournalService6Engine()

    def create(self, op_index: float = 50.0) -> DoubleEntryJournalService6Record:
        rec = DoubleEntryJournalService6Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[DoubleEntryJournalService6Record]:
...
def test_subsystem_double_entry_journal_6() -> bool:
    mgr = DoubleEntryJournalService6Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = DoubleEntryJournalService6Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_double_entry_journal_6()
    print(f"double_entry_journal Part 6: {'PASSED' if ok else 'FAILED'}")
