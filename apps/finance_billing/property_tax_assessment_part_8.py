...
Module: Unit Area Value Property Tax Assessment Engine (Part 8)
...
class PropertyTaxAssessmentService8Engine:
    """
...
class PropertyTaxAssessmentService8Workflow:
    """
    ...
    def __init__(self, record: PropertyTaxAssessmentService8Record):
        self.record = record
...
class PropertyTaxAssessmentService8Manager:
    """
    Service registry and data aggregator for Unit Area Value Property Tax Assessment Engine.
    """
    def __init__(self):
        self.store: Dict[str, PropertyTaxAssessmentService8Record] = {}
        self.engine = PropertyTaxAssessmentService8Engine()

    def create(self, op_index: float = 50.0) -> PropertyTaxAssessmentService8Record:
        rec = PropertyTaxAssessmentService8Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[PropertyTaxAssessmentService8Record]:
...
def test_subsystem_property_tax_assessment_8() -> bool:
    mgr = PropertyTaxAssessmentService8Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = PropertyTaxAssessmentService8Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_property_tax_assessment_8()
    print(f"property_tax_assessment Part 8: {'PASSED' if ok else 'FAILED'}")
