...
Module: Vendor Payment Voucher & Check Printing (Part 1)
...
class ContractorDisbursementsService1Engine:
    """
...
class ContractorDisbursementsService1Workflow:
    """
    ...
    def __init__(self, record: ContractorDisbursementsService1Record):
        self.record = record
...
class ContractorDisbursementsService1Manager:
    """
    Service registry and data aggregator for Vendor Payment Voucher & Check Printing.
    """
    def __init__(self):
        self.store: Dict[str, ContractorDisbursementsService1Record] = {}
        self.engine = ContractorDisbursementsService1Engine()

    def create(self, op_index: float = 50.0) -> ContractorDisbursementsService1Record:
        rec = ContractorDisbursementsService1Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[ContractorDisbursementsService1Record]:
...
def test_subsystem_contractor_disbursements_1() -> bool:
    mgr = ContractorDisbursementsService1Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = ContractorDisbursementsService1Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_contractor_disbursements_1()
    print(f"contractor_disbursements Part 1: {'PASSED' if ok else 'FAILED'}")
