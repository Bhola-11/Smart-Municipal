...
Module: Ward Sub-Office Petty Cash Float Ledgers (Part 1)
...
class PettyCashDisbursementsService1Engine:
    """
...
class PettyCashDisbursementsService1Workflow:
    """
    ...
    def __init__(self, record: PettyCashDisbursementsService1Record):
        self.record = record
...
class PettyCashDisbursementsService1Manager:
    """
    Service registry and data aggregator for Ward Sub-Office Petty Cash Float Ledgers.
    """
    def __init__(self):
        self.store: Dict[str, PettyCashDisbursementsService1Record] = {}
        self.engine = PettyCashDisbursementsService1Engine()

    def create(self, op_index: float = 50.0) -> PettyCashDisbursementsService1Record:
        rec = PettyCashDisbursementsService1Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[PettyCashDisbursementsService1Record]:
...
def test_subsystem_petty_cash_disbursements_1() -> bool:
    mgr = PettyCashDisbursementsService1Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = PettyCashDisbursementsService1Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_petty_cash_disbursements_1()
    print(f"petty_cash_disbursements Part 1: {'PASSED' if ok else 'FAILED'}")
