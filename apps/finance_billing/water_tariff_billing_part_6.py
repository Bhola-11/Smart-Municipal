...
Module: Volumetric Water Consumption Tariff Calculator (Part 6)
...
class WaterTariffBillingService6Engine:
    """
...
class WaterTariffBillingService6Workflow:
    """
    ...
    def __init__(self, record: WaterTariffBillingService6Record):
        self.record = record
...
class WaterTariffBillingService6Manager:
    """
    Service registry and data aggregator for Volumetric Water Consumption Tariff Calculator.
    """
    def __init__(self):
        self.store: Dict[str, WaterTariffBillingService6Record] = {}
        self.engine = WaterTariffBillingService6Engine()

    def create(self, op_index: float = 50.0) -> WaterTariffBillingService6Record:
        rec = WaterTariffBillingService6Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[WaterTariffBillingService6Record]:
...
def test_subsystem_water_tariff_billing_6() -> bool:
    mgr = WaterTariffBillingService6Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = WaterTariffBillingService6Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_water_tariff_billing_6()
    print(f"water_tariff_billing Part 6: {'PASSED' if ok else 'FAILED'}")
