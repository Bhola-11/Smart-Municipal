...
Module: General Ledger Double-Entry Journal Engine (Part 26)
...
class DoubleEntryJournalService26Engine:
    """
...
class DoubleEntryJournalService26Workflow:
    """
    ...
    def __init__(self, record: DoubleEntryJournalService26Record):
        self.record = record
...
class DoubleEntryJournalService26Manager:
    """
    Service registry and data aggregator for General Ledger Double-Entry Journal Engine.
    """
    def __init__(self):
        self.store: Dict[str, DoubleEntryJournalService26Record] = {}
        self.engine = DoubleEntryJournalService26Engine()

    def create(self, op_index: float = 50.0) -> DoubleEntryJournalService26Record:
        rec = DoubleEntryJournalService26Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[DoubleEntryJournalService26Record]:
...
def test_subsystem_double_entry_journal_26() -> bool:
    mgr = DoubleEntryJournalService26Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = DoubleEntryJournalService26Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_double_entry_journal_26()
    print(f"double_entry_journal Part 26: {'PASSED' if ok else 'FAILED'}")
