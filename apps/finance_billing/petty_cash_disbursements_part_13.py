...
Module: Ward Sub-Office Petty Cash Float Ledgers (Part 13)
...
class PettyCashDisbursementsService13Engine:
    """
...
class PettyCashDisbursementsService13Workflow:
    """
    ...
    def __init__(self, record: PettyCashDisbursementsService13Record):
        self.record = record
...
class PettyCashDisbursementsService13Manager:
    """
    Service registry and data aggregator for Ward Sub-Office Petty Cash Float Ledgers.
    """
    def __init__(self):
        self.store: Dict[str, PettyCashDisbursementsService13Record] = {}
        self.engine = PettyCashDisbursementsService13Engine()

    def create(self, op_index: float = 50.0) -> PettyCashDisbursementsService13Record:
        rec = PettyCashDisbursementsService13Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[PettyCashDisbursementsService13Record]:
...
def test_subsystem_petty_cash_disbursements_13() -> bool:
    mgr = PettyCashDisbursementsService13Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = PettyCashDisbursementsService13Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_petty_cash_disbursements_13()
    print(f"petty_cash_disbursements Part 13: {'PASSED' if ok else 'FAILED'}")
