...
Module: Municipal Chart of Accounts (COA) Classification (Part 20)
...
class ChartOfAccountsService20Engine:
    """
...
class ChartOfAccountsService20Workflow:
    """
    ...
    def __init__(self, record: ChartOfAccountsService20Record):
        self.record = record
...
class ChartOfAccountsService20Manager:
    """
    Service registry and data aggregator for Municipal Chart of Accounts (COA) Classification.
    """
    def __init__(self):
        self.store: Dict[str, ChartOfAccountsService20Record] = {}
        self.engine = ChartOfAccountsService20Engine()

    def create(self, op_index: float = 50.0) -> ChartOfAccountsService20Record:
        rec = ChartOfAccountsService20Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[ChartOfAccountsService20Record]:
...
def test_subsystem_chart_of_accounts_20() -> bool:
    mgr = ChartOfAccountsService20Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = ChartOfAccountsService20Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_chart_of_accounts_20()
    print(f"chart_of_accounts Part 20: {'PASSED' if ok else 'FAILED'}")
