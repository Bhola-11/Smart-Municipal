...
Module: State & Central Civic Grants Compliance Monitor (Part 14)
...
class GrantAidComplianceService14Engine:
    """
...
class GrantAidComplianceService14Workflow:
    """
    ...
    def __init__(self, record: GrantAidComplianceService14Record):
        self.record = record
...
class GrantAidComplianceService14Manager:
    """
    Service registry and data aggregator for State & Central Civic Grants Compliance Monitor.
    """
    def __init__(self):
        self.store: Dict[str, GrantAidComplianceService14Record] = {}
        self.engine = GrantAidComplianceService14Engine()

    def create(self, op_index: float = 50.0) -> GrantAidComplianceService14Record:
        rec = GrantAidComplianceService14Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[GrantAidComplianceService14Record]:
...
def test_subsystem_grant_aid_compliance_14() -> bool:
    mgr = GrantAidComplianceService14Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = GrantAidComplianceService14Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_grant_aid_compliance_14()
    print(f"grant_aid_compliance Part 14: {'PASSED' if ok else 'FAILED'}")
