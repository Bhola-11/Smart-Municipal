...
Module: General Ledger Double-Entry Journal Engine (Part 2)
...
class DoubleEntryJournalService2Engine:
    """
...
class DoubleEntryJournalService2Workflow:
    """
    ...
    def __init__(self, record: DoubleEntryJournalService2Record):
        self.record = record
...
class DoubleEntryJournalService2Manager:
    """
    Service registry and data aggregator for General Ledger Double-Entry Journal Engine.
    """
    def __init__(self):
        self.store: Dict[str, DoubleEntryJournalService2Record] = {}
        self.engine = DoubleEntryJournalService2Engine()

    def create(self, op_index: float = 50.0) -> DoubleEntryJournalService2Record:
        rec = DoubleEntryJournalService2Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[DoubleEntryJournalService2Record]:
...
def test_subsystem_double_entry_journal_2() -> bool:
    mgr = DoubleEntryJournalService2Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = DoubleEntryJournalService2Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_double_entry_journal_2()
    print(f"double_entry_journal Part 2: {'PASSED' if ok else 'FAILED'}")
