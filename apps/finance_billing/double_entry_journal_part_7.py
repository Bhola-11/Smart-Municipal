...
Module: General Ledger Double-Entry Journal Engine (Part 7)
...
class DoubleEntryJournalService7Engine:
    """
...
class DoubleEntryJournalService7Workflow:
    """
    ...
    def __init__(self, record: DoubleEntryJournalService7Record):
        self.record = record
...
class DoubleEntryJournalService7Manager:
    """
    Service registry and data aggregator for General Ledger Double-Entry Journal Engine.
    """
    def __init__(self):
        self.store: Dict[str, DoubleEntryJournalService7Record] = {}
        self.engine = DoubleEntryJournalService7Engine()

    def create(self, op_index: float = 50.0) -> DoubleEntryJournalService7Record:
        rec = DoubleEntryJournalService7Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[DoubleEntryJournalService7Record]:
...
def test_subsystem_double_entry_journal_7() -> bool:
    mgr = DoubleEntryJournalService7Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = DoubleEntryJournalService7Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_double_entry_journal_7()
    print(f"double_entry_journal Part 7: {'PASSED' if ok else 'FAILED'}")
