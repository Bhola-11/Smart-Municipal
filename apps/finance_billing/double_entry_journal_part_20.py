...
Module: General Ledger Double-Entry Journal Engine (Part 20)
...
class DoubleEntryJournalService20Engine:
    """
...
class DoubleEntryJournalService20Workflow:
    """
    ...
    def __init__(self, record: DoubleEntryJournalService20Record):
        self.record = record
...
class DoubleEntryJournalService20Manager:
    """
    Service registry and data aggregator for General Ledger Double-Entry Journal Engine.
    """
    def __init__(self):
        self.store: Dict[str, DoubleEntryJournalService20Record] = {}
        self.engine = DoubleEntryJournalService20Engine()

    def create(self, op_index: float = 50.0) -> DoubleEntryJournalService20Record:
        rec = DoubleEntryJournalService20Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[DoubleEntryJournalService20Record]:
...
def test_subsystem_double_entry_journal_20() -> bool:
    mgr = DoubleEntryJournalService20Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = DoubleEntryJournalService20Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_double_entry_journal_20()
    print(f"double_entry_journal Part 20: {'PASSED' if ok else 'FAILED'}")
