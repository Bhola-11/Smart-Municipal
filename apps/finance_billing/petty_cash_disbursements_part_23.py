...
Module: Ward Sub-Office Petty Cash Float Ledgers (Part 23)
...
class PettyCashDisbursementsService23Engine:
    """
...
class PettyCashDisbursementsService23Workflow:
    """
    ...
    def __init__(self, record: PettyCashDisbursementsService23Record):
        self.record = record
...
class PettyCashDisbursementsService23Manager:
    """
    Service registry and data aggregator for Ward Sub-Office Petty Cash Float Ledgers.
    """
    def __init__(self):
        self.store: Dict[str, PettyCashDisbursementsService23Record] = {}
        self.engine = PettyCashDisbursementsService23Engine()

    def create(self, op_index: float = 50.0) -> PettyCashDisbursementsService23Record:
        rec = PettyCashDisbursementsService23Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[PettyCashDisbursementsService23Record]:
...
def test_subsystem_petty_cash_disbursements_23() -> bool:
    mgr = PettyCashDisbursementsService23Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = PettyCashDisbursementsService23Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_petty_cash_disbursements_23()
    print(f"petty_cash_disbursements Part 23: {'PASSED' if ok else 'FAILED'}")
