...
Module: State & Central Civic Grants Compliance Monitor (Part 20)
...
class GrantAidComplianceService20Engine:
    """
...
class GrantAidComplianceService20Workflow:
    """
    ...
    def __init__(self, record: GrantAidComplianceService20Record):
        self.record = record
...
class GrantAidComplianceService20Manager:
    """
    Service registry and data aggregator for State & Central Civic Grants Compliance Monitor.
    """
    def __init__(self):
        self.store: Dict[str, GrantAidComplianceService20Record] = {}
        self.engine = GrantAidComplianceService20Engine()

    def create(self, op_index: float = 50.0) -> GrantAidComplianceService20Record:
        rec = GrantAidComplianceService20Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[GrantAidComplianceService20Record]:
...
def test_subsystem_grant_aid_compliance_20() -> bool:
    mgr = GrantAidComplianceService20Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = GrantAidComplianceService20Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_grant_aid_compliance_20()
    print(f"grant_aid_compliance Part 20: {'PASSED' if ok else 'FAILED'}")
