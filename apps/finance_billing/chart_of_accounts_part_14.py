...
Module: Municipal Chart of Accounts (COA) Classification (Part 14)
...
class ChartOfAccountsService14Engine:
    """
...
class ChartOfAccountsService14Workflow:
    """
    ...
    def __init__(self, record: ChartOfAccountsService14Record):
        self.record = record
...
class ChartOfAccountsService14Manager:
    """
    Service registry and data aggregator for Municipal Chart of Accounts (COA) Classification.
    """
    def __init__(self):
        self.store: Dict[str, ChartOfAccountsService14Record] = {}
        self.engine = ChartOfAccountsService14Engine()

    def create(self, op_index: float = 50.0) -> ChartOfAccountsService14Record:
        rec = ChartOfAccountsService14Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[ChartOfAccountsService14Record]:
...
def test_subsystem_chart_of_accounts_14() -> bool:
    mgr = ChartOfAccountsService14Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = ChartOfAccountsService14Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_chart_of_accounts_14()
    print(f"chart_of_accounts Part 14: {'PASSED' if ok else 'FAILED'}")
