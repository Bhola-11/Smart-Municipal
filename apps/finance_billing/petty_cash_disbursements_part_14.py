...
Module: Ward Sub-Office Petty Cash Float Ledgers (Part 14)
...
class PettyCashDisbursementsService14Engine:
    """
...
class PettyCashDisbursementsService14Workflow:
    """
    ...
    def __init__(self, record: PettyCashDisbursementsService14Record):
        self.record = record
...
class PettyCashDisbursementsService14Manager:
    """
    Service registry and data aggregator for Ward Sub-Office Petty Cash Float Ledgers.
    """
    def __init__(self):
        self.store: Dict[str, PettyCashDisbursementsService14Record] = {}
        self.engine = PettyCashDisbursementsService14Engine()

    def create(self, op_index: float = 50.0) -> PettyCashDisbursementsService14Record:
        rec = PettyCashDisbursementsService14Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[PettyCashDisbursementsService14Record]:
...
def test_subsystem_petty_cash_disbursements_14() -> bool:
    mgr = PettyCashDisbursementsService14Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = PettyCashDisbursementsService14Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_petty_cash_disbursements_14()
    print(f"petty_cash_disbursements Part 14: {'PASSED' if ok else 'FAILED'}")
