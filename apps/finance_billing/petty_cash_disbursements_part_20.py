...
Module: Ward Sub-Office Petty Cash Float Ledgers (Part 20)
...
class PettyCashDisbursementsService20Engine:
    """
...
class PettyCashDisbursementsService20Workflow:
    """
    ...
    def __init__(self, record: PettyCashDisbursementsService20Record):
        self.record = record
...
class PettyCashDisbursementsService20Manager:
    """
    Service registry and data aggregator for Ward Sub-Office Petty Cash Float Ledgers.
    """
    def __init__(self):
        self.store: Dict[str, PettyCashDisbursementsService20Record] = {}
        self.engine = PettyCashDisbursementsService20Engine()

    def create(self, op_index: float = 50.0) -> PettyCashDisbursementsService20Record:
        rec = PettyCashDisbursementsService20Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[PettyCashDisbursementsService20Record]:
...
def test_subsystem_petty_cash_disbursements_20() -> bool:
    mgr = PettyCashDisbursementsService20Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = PettyCashDisbursementsService20Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_petty_cash_disbursements_20()
    print(f"petty_cash_disbursements Part 20: {'PASSED' if ok else 'FAILED'}")
