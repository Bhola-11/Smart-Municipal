...
Module: General Ledger Double-Entry Journal Engine (Part 13)
...
class DoubleEntryJournalService13Engine:
    """
...
class DoubleEntryJournalService13Workflow:
    """
    ...
    def __init__(self, record: DoubleEntryJournalService13Record):
        self.record = record
...
class DoubleEntryJournalService13Manager:
    """
    Service registry and data aggregator for General Ledger Double-Entry Journal Engine.
    """
    def __init__(self):
        self.store: Dict[str, DoubleEntryJournalService13Record] = {}
        self.engine = DoubleEntryJournalService13Engine()

    def create(self, op_index: float = 50.0) -> DoubleEntryJournalService13Record:
        rec = DoubleEntryJournalService13Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[DoubleEntryJournalService13Record]:
...
def test_subsystem_double_entry_journal_13() -> bool:
    mgr = DoubleEntryJournalService13Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = DoubleEntryJournalService13Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_double_entry_journal_13()
    print(f"double_entry_journal Part 13: {'PASSED' if ok else 'FAILED'}")
