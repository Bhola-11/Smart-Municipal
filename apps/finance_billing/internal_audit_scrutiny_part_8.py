...
Module: Comptroller & Auditor General Internal Scrutiny (Part 8)
...
class InternalAuditScrutinyService8Engine:
    """
...
class InternalAuditScrutinyService8Workflow:
    """
    ...
    def __init__(self, record: InternalAuditScrutinyService8Record):
        self.record = record
...
class InternalAuditScrutinyService8Manager:
    """
    Service registry and data aggregator for Comptroller & Auditor General Internal Scrutiny.
    """
    def __init__(self):
        self.store: Dict[str, InternalAuditScrutinyService8Record] = {}
        self.engine = InternalAuditScrutinyService8Engine()

    def create(self, op_index: float = 50.0) -> InternalAuditScrutinyService8Record:
        rec = InternalAuditScrutinyService8Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[InternalAuditScrutinyService8Record]:
...
def test_subsystem_internal_audit_scrutiny_8() -> bool:
    mgr = InternalAuditScrutinyService8Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = InternalAuditScrutinyService8Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_internal_audit_scrutiny_8()
    print(f"internal_audit_scrutiny Part 8: {'PASSED' if ok else 'FAILED'}")
