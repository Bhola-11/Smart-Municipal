...
Module: State & Central Civic Grants Compliance Monitor (Part 8)
...
class GrantAidComplianceService8Engine:
    """
...
class GrantAidComplianceService8Workflow:
    """
    ...
    def __init__(self, record: GrantAidComplianceService8Record):
        self.record = record
...
class GrantAidComplianceService8Manager:
    """
    Service registry and data aggregator for State & Central Civic Grants Compliance Monitor.
    """
    def __init__(self):
        self.store: Dict[str, GrantAidComplianceService8Record] = {}
        self.engine = GrantAidComplianceService8Engine()

    def create(self, op_index: float = 50.0) -> GrantAidComplianceService8Record:
        rec = GrantAidComplianceService8Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[GrantAidComplianceService8Record]:
...
def test_subsystem_grant_aid_compliance_8() -> bool:
    mgr = GrantAidComplianceService8Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = GrantAidComplianceService8Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_grant_aid_compliance_8()
    print(f"grant_aid_compliance Part 8: {'PASSED' if ok else 'FAILED'}")
