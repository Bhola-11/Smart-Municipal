...
Module: Ward Sub-Office Petty Cash Float Ledgers (Part 6)
...
class PettyCashDisbursementsService6Engine:
    """
...
class PettyCashDisbursementsService6Workflow:
    """
    ...
    def __init__(self, record: PettyCashDisbursementsService6Record):
        self.record = record
...
class PettyCashDisbursementsService6Manager:
    """
    Service registry and data aggregator for Ward Sub-Office Petty Cash Float Ledgers.
    """
    def __init__(self):
        self.store: Dict[str, PettyCashDisbursementsService6Record] = {}
        self.engine = PettyCashDisbursementsService6Engine()

    def create(self, op_index: float = 50.0) -> PettyCashDisbursementsService6Record:
        rec = PettyCashDisbursementsService6Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[PettyCashDisbursementsService6Record]:
...
def test_subsystem_petty_cash_disbursements_6() -> bool:
    mgr = PettyCashDisbursementsService6Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = PettyCashDisbursementsService6Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_petty_cash_disbursements_6()
    print(f"petty_cash_disbursements Part 6: {'PASSED' if ok else 'FAILED'}")
