...
Module: Ward Sub-Office Petty Cash Float Ledgers (Part 21)
...
class PettyCashDisbursementsService21Engine:
    """
...
class PettyCashDisbursementsService21Workflow:
    """
    ...
    def __init__(self, record: PettyCashDisbursementsService21Record):
        self.record = record
...
class PettyCashDisbursementsService21Manager:
    """
    Service registry and data aggregator for Ward Sub-Office Petty Cash Float Ledgers.
    """
    def __init__(self):
        self.store: Dict[str, PettyCashDisbursementsService21Record] = {}
        self.engine = PettyCashDisbursementsService21Engine()

    def create(self, op_index: float = 50.0) -> PettyCashDisbursementsService21Record:
        rec = PettyCashDisbursementsService21Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[PettyCashDisbursementsService21Record]:
...
def test_subsystem_petty_cash_disbursements_21() -> bool:
    mgr = PettyCashDisbursementsService21Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = PettyCashDisbursementsService21Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_petty_cash_disbursements_21()
    print(f"petty_cash_disbursements Part 21: {'PASSED' if ok else 'FAILED'}")
