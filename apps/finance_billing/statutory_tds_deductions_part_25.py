...
Module: Tax Deducted at Source (TDS & GST) Ledgers (Part 25)
...
class StatutoryTdsDeductionsService25Engine:
    """
...
class StatutoryTdsDeductionsService25Workflow:
    """
    ...
    def __init__(self, record: StatutoryTdsDeductionsService25Record):
        self.record = record
...
class StatutoryTdsDeductionsService25Manager:
    """
    Service registry and data aggregator for Tax Deducted at Source (TDS & GST) Ledgers.
    """
    def __init__(self):
        self.store: Dict[str, StatutoryTdsDeductionsService25Record] = {}
        self.engine = StatutoryTdsDeductionsService25Engine()

    def create(self, op_index: float = 50.0) -> StatutoryTdsDeductionsService25Record:
        rec = StatutoryTdsDeductionsService25Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[StatutoryTdsDeductionsService25Record]:
...
def test_subsystem_statutory_tds_deductions_25() -> bool:
    mgr = StatutoryTdsDeductionsService25Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = StatutoryTdsDeductionsService25Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_statutory_tds_deductions_25()
    print(f"statutory_tds_deductions Part 25: {'PASSED' if ok else 'FAILED'}")
