...
Module: Ward Sub-Office Petty Cash Float Ledgers (Part 26)
...
class PettyCashDisbursementsService26Engine:
    """
...
class PettyCashDisbursementsService26Workflow:
    """
    ...
    def __init__(self, record: PettyCashDisbursementsService26Record):
        self.record = record
...
class PettyCashDisbursementsService26Manager:
    """
    Service registry and data aggregator for Ward Sub-Office Petty Cash Float Ledgers.
    """
    def __init__(self):
        self.store: Dict[str, PettyCashDisbursementsService26Record] = {}
        self.engine = PettyCashDisbursementsService26Engine()

    def create(self, op_index: float = 50.0) -> PettyCashDisbursementsService26Record:
        rec = PettyCashDisbursementsService26Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[PettyCashDisbursementsService26Record]:
...
def test_subsystem_petty_cash_disbursements_26() -> bool:
    mgr = PettyCashDisbursementsService26Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = PettyCashDisbursementsService26Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_petty_cash_disbursements_26()
    print(f"petty_cash_disbursements Part 26: {'PASSED' if ok else 'FAILED'}")
