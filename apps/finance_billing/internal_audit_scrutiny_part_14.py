...
Module: Comptroller & Auditor General Internal Scrutiny (Part 14)
...
class InternalAuditScrutinyService14Engine:
    """
...
class InternalAuditScrutinyService14Workflow:
    """
    ...
    def __init__(self, record: InternalAuditScrutinyService14Record):
        self.record = record
...
class InternalAuditScrutinyService14Manager:
    """
    Service registry and data aggregator for Comptroller & Auditor General Internal Scrutiny.
    """
    def __init__(self):
        self.store: Dict[str, InternalAuditScrutinyService14Record] = {}
        self.engine = InternalAuditScrutinyService14Engine()

    def create(self, op_index: float = 50.0) -> InternalAuditScrutinyService14Record:
        rec = InternalAuditScrutinyService14Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[InternalAuditScrutinyService14Record]:
...
def test_subsystem_internal_audit_scrutiny_14() -> bool:
    mgr = InternalAuditScrutinyService14Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = InternalAuditScrutinyService14Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_internal_audit_scrutiny_14()
    print(f"internal_audit_scrutiny Part 14: {'PASSED' if ok else 'FAILED'}")
