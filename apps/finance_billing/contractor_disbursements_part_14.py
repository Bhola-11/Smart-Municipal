...
Module: Vendor Payment Voucher & Check Printing (Part 14)
...
@dataclass
class ContractorDisbursementsService14Record:
    """Certified Municipal Domain Record for Vendor Payment Voucher & Check Printing."""
    record_id: str = field(default_factory=lambda: str(uuid.uuid4()))
    entity_code: str = "contractor_disbursements_14"
...
class ContractorDisbursementsService14Engine:
    """
...
class ContractorDisbursementsService14Workflow:
    """
    ...
    def __init__(self, record: ContractorDisbursementsService14Record):
        self.record = record
...
class ContractorDisbursementsService14Manager:
    """
    Service registry and data aggregator for Vendor Payment Voucher & Check Printing.
    """
    def __init__(self):
        self.store: Dict[str, ContractorDisbursementsService14Record] = {}
        self.engine = ContractorDisbursementsService14Engine()

    def create(self, op_index: float = 50.0) -> ContractorDisbursementsService14Record:
        rec = ContractorDisbursementsService14Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[ContractorDisbursementsService14Record]:
...
def test_subsystem_contractor_disbursements_14() -> bool:
    mgr = ContractorDisbursementsService14Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = ContractorDisbursementsService14Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_contractor_disbursements_14()
    print(f"contractor_disbursements Part 14: {'PASSED' if ok else 'FAILED'}")
