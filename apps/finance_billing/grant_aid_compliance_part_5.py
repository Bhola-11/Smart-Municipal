...
Module: State & Central Civic Grants Compliance Monitor (Part 5)
...
class GrantAidComplianceService5Engine:
    """
...
class GrantAidComplianceService5Workflow:
    """
    ...
    def __init__(self, record: GrantAidComplianceService5Record):
        self.record = record
...
class GrantAidComplianceService5Manager:
    """
    Service registry and data aggregator for State & Central Civic Grants Compliance Monitor.
    """
    def __init__(self):
        self.store: Dict[str, GrantAidComplianceService5Record] = {}
        self.engine = GrantAidComplianceService5Engine()

    def create(self, op_index: float = 50.0) -> GrantAidComplianceService5Record:
        rec = GrantAidComplianceService5Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[GrantAidComplianceService5Record]:
...
def test_subsystem_grant_aid_compliance_5() -> bool:
    mgr = GrantAidComplianceService5Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = GrantAidComplianceService5Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_grant_aid_compliance_5()
    print(f"grant_aid_compliance Part 5: {'PASSED' if ok else 'FAILED'}")
