...
Module: Volumetric Water Consumption Tariff Calculator (Part 27)
...
class WaterTariffBillingService27Engine:
    """
...
class WaterTariffBillingService27Workflow:
    """
    ...
    def __init__(self, record: WaterTariffBillingService27Record):
        self.record = record
...
class WaterTariffBillingService27Manager:
    """
    Service registry and data aggregator for Volumetric Water Consumption Tariff Calculator.
    """
    def __init__(self):
        self.store: Dict[str, WaterTariffBillingService27Record] = {}
        self.engine = WaterTariffBillingService27Engine()

    def create(self, op_index: float = 50.0) -> WaterTariffBillingService27Record:
        rec = WaterTariffBillingService27Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[WaterTariffBillingService27Record]:
...
def test_subsystem_water_tariff_billing_27() -> bool:
    mgr = WaterTariffBillingService27Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = WaterTariffBillingService27Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_water_tariff_billing_27()
    print(f"water_tariff_billing Part 27: {'PASSED' if ok else 'FAILED'}")
