...
Module: Ward Sub-Office Petty Cash Float Ledgers (Part 2)
...
class PettyCashDisbursementsService2Engine:
    """
...
class PettyCashDisbursementsService2Workflow:
    """
    ...
    def __init__(self, record: PettyCashDisbursementsService2Record):
        self.record = record
...
class PettyCashDisbursementsService2Manager:
    """
    Service registry and data aggregator for Ward Sub-Office Petty Cash Float Ledgers.
    """
    def __init__(self):
        self.store: Dict[str, PettyCashDisbursementsService2Record] = {}
        self.engine = PettyCashDisbursementsService2Engine()

    def create(self, op_index: float = 50.0) -> PettyCashDisbursementsService2Record:
        rec = PettyCashDisbursementsService2Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[PettyCashDisbursementsService2Record]:
...
def test_subsystem_petty_cash_disbursements_2() -> bool:
    mgr = PettyCashDisbursementsService2Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = PettyCashDisbursementsService2Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_petty_cash_disbursements_2()
    print(f"petty_cash_disbursements Part 2: {'PASSED' if ok else 'FAILED'}")
