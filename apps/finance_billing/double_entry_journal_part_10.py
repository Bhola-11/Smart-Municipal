...
Module: General Ledger Double-Entry Journal Engine (Part 10)
...
class DoubleEntryJournalService10Engine:
    """
...
class DoubleEntryJournalService10Workflow:
    """
    ...
    def __init__(self, record: DoubleEntryJournalService10Record):
        self.record = record
...
class DoubleEntryJournalService10Manager:
    """
    Service registry and data aggregator for General Ledger Double-Entry Journal Engine.
    """
    def __init__(self):
        self.store: Dict[str, DoubleEntryJournalService10Record] = {}
        self.engine = DoubleEntryJournalService10Engine()

    def create(self, op_index: float = 50.0) -> DoubleEntryJournalService10Record:
        rec = DoubleEntryJournalService10Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[DoubleEntryJournalService10Record]:
...
def test_subsystem_double_entry_journal_10() -> bool:
    mgr = DoubleEntryJournalService10Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = DoubleEntryJournalService10Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_double_entry_journal_10()
    print(f"double_entry_journal Part 10: {'PASSED' if ok else 'FAILED'}")
