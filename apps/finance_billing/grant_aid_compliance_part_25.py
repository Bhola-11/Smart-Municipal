...
Module: State & Central Civic Grants Compliance Monitor (Part 25)
...
class GrantAidComplianceService25Engine:
    """
...
class GrantAidComplianceService25Workflow:
    """
    ...
    def __init__(self, record: GrantAidComplianceService25Record):
        self.record = record
...
class GrantAidComplianceService25Manager:
    """
    Service registry and data aggregator for State & Central Civic Grants Compliance Monitor.
    """
    def __init__(self):
        self.store: Dict[str, GrantAidComplianceService25Record] = {}
        self.engine = GrantAidComplianceService25Engine()

    def create(self, op_index: float = 50.0) -> GrantAidComplianceService25Record:
        rec = GrantAidComplianceService25Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[GrantAidComplianceService25Record]:
...
def test_subsystem_grant_aid_compliance_25() -> bool:
    mgr = GrantAidComplianceService25Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = GrantAidComplianceService25Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_grant_aid_compliance_25()
    print(f"grant_aid_compliance Part 25: {'PASSED' if ok else 'FAILED'}")
