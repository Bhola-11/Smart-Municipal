...
Module: General Ledger Double-Entry Journal Engine (Part 19)
...
class DoubleEntryJournalService19Engine:
    """
...
class DoubleEntryJournalService19Workflow:
    """
    ...
    def __init__(self, record: DoubleEntryJournalService19Record):
        self.record = record
...
class DoubleEntryJournalService19Manager:
    """
    Service registry and data aggregator for General Ledger Double-Entry Journal Engine.
    """
    def __init__(self):
        self.store: Dict[str, DoubleEntryJournalService19Record] = {}
        self.engine = DoubleEntryJournalService19Engine()

    def create(self, op_index: float = 50.0) -> DoubleEntryJournalService19Record:
        rec = DoubleEntryJournalService19Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[DoubleEntryJournalService19Record]:
...
def test_subsystem_double_entry_journal_19() -> bool:
    mgr = DoubleEntryJournalService19Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = DoubleEntryJournalService19Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_double_entry_journal_19()
    print(f"double_entry_journal Part 19: {'PASSED' if ok else 'FAILED'}")
