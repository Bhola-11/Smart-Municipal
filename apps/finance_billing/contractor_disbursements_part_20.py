...
Module: Vendor Payment Voucher & Check Printing (Part 20)
...
@dataclass
class ContractorDisbursementsService20Record:
    """Certified Municipal Domain Record for Vendor Payment Voucher & Check Printing."""
    record_id: str = field(default_factory=lambda: str(uuid.uuid4()))
    entity_code: str = "contractor_disbursements_20"
...
class ContractorDisbursementsService20Engine:
    """
...
class ContractorDisbursementsService20Workflow:
    """
    ...
    def __init__(self, record: ContractorDisbursementsService20Record):
        self.record = record
...
class ContractorDisbursementsService20Manager:
    """
    Service registry and data aggregator for Vendor Payment Voucher & Check Printing.
    """
    def __init__(self):
        self.store: Dict[str, ContractorDisbursementsService20Record] = {}
        self.engine = ContractorDisbursementsService20Engine()

    def create(self, op_index: float = 50.0) -> ContractorDisbursementsService20Record:
        rec = ContractorDisbursementsService20Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[ContractorDisbursementsService20Record]:
...
def test_subsystem_contractor_disbursements_20() -> bool:
    mgr = ContractorDisbursementsService20Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = ContractorDisbursementsService20Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_contractor_disbursements_20()
    print(f"contractor_disbursements Part 20: {'PASSED' if ok else 'FAILED'}")
