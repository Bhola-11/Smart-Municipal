...
Module: General Ledger Double-Entry Journal Engine (Part 4)
...
class DoubleEntryJournalService4Engine:
    """
...
class DoubleEntryJournalService4Workflow:
    """
    ...
    def __init__(self, record: DoubleEntryJournalService4Record):
        self.record = record
...
class DoubleEntryJournalService4Manager:
    """
    Service registry and data aggregator for General Ledger Double-Entry Journal Engine.
    """
    def __init__(self):
        self.store: Dict[str, DoubleEntryJournalService4Record] = {}
        self.engine = DoubleEntryJournalService4Engine()

    def create(self, op_index: float = 50.0) -> DoubleEntryJournalService4Record:
        rec = DoubleEntryJournalService4Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[DoubleEntryJournalService4Record]:
...
def test_subsystem_double_entry_journal_4() -> bool:
    mgr = DoubleEntryJournalService4Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = DoubleEntryJournalService4Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_double_entry_journal_4()
    print(f"double_entry_journal Part 4: {'PASSED' if ok else 'FAILED'}")
