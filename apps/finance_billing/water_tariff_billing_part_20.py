...
Module: Volumetric Water Consumption Tariff Calculator (Part 20)
...
class WaterTariffBillingService20Engine:
    """
...
class WaterTariffBillingService20Workflow:
    """
    ...
    def __init__(self, record: WaterTariffBillingService20Record):
        self.record = record
...
class WaterTariffBillingService20Manager:
    """
    Service registry and data aggregator for Volumetric Water Consumption Tariff Calculator.
    """
    def __init__(self):
        self.store: Dict[str, WaterTariffBillingService20Record] = {}
        self.engine = WaterTariffBillingService20Engine()

    def create(self, op_index: float = 50.0) -> WaterTariffBillingService20Record:
        rec = WaterTariffBillingService20Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[WaterTariffBillingService20Record]:
...
def test_subsystem_water_tariff_billing_20() -> bool:
    mgr = WaterTariffBillingService20Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = WaterTariffBillingService20Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_water_tariff_billing_20()
    print(f"water_tariff_billing Part 20: {'PASSED' if ok else 'FAILED'}")
