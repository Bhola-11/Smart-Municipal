...
Module: General Ledger Double-Entry Journal Engine (Part 30)
...
class DoubleEntryJournalService30Engine:
    """
...
class DoubleEntryJournalService30Workflow:
    """
    ...
    def __init__(self, record: DoubleEntryJournalService30Record):
        self.record = record
...
class DoubleEntryJournalService30Manager:
    """
    Service registry and data aggregator for General Ledger Double-Entry Journal Engine.
    """
    def __init__(self):
        self.store: Dict[str, DoubleEntryJournalService30Record] = {}
        self.engine = DoubleEntryJournalService30Engine()

    def create(self, op_index: float = 50.0) -> DoubleEntryJournalService30Record:
        rec = DoubleEntryJournalService30Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[DoubleEntryJournalService30Record]:
...
def test_subsystem_double_entry_journal_30() -> bool:
    mgr = DoubleEntryJournalService30Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = DoubleEntryJournalService30Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_double_entry_journal_30()
    print(f"double_entry_journal Part 30: {'PASSED' if ok else 'FAILED'}")
