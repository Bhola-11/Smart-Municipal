...
Module: Comptroller & Auditor General Internal Scrutiny (Part 4)
...
class InternalAuditScrutinyService4Engine:
    """
...
class InternalAuditScrutinyService4Workflow:
    """
    ...
    def __init__(self, record: InternalAuditScrutinyService4Record):
        self.record = record
...
class InternalAuditScrutinyService4Manager:
    """
    Service registry and data aggregator for Comptroller & Auditor General Internal Scrutiny.
    """
    def __init__(self):
        self.store: Dict[str, InternalAuditScrutinyService4Record] = {}
        self.engine = InternalAuditScrutinyService4Engine()

    def create(self, op_index: float = 50.0) -> InternalAuditScrutinyService4Record:
        rec = InternalAuditScrutinyService4Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[InternalAuditScrutinyService4Record]:
...
def test_subsystem_internal_audit_scrutiny_4() -> bool:
    mgr = InternalAuditScrutinyService4Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = InternalAuditScrutinyService4Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_internal_audit_scrutiny_4()
    print(f"internal_audit_scrutiny Part 4: {'PASSED' if ok else 'FAILED'}")
