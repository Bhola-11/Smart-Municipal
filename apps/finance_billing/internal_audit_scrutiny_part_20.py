...
Module: Comptroller & Auditor General Internal Scrutiny (Part 20)
...
class InternalAuditScrutinyService20Engine:
    """
...
class InternalAuditScrutinyService20Workflow:
    """
    ...
    def __init__(self, record: InternalAuditScrutinyService20Record):
        self.record = record
...
class InternalAuditScrutinyService20Manager:
    """
    Service registry and data aggregator for Comptroller & Auditor General Internal Scrutiny.
    """
    def __init__(self):
        self.store: Dict[str, InternalAuditScrutinyService20Record] = {}
        self.engine = InternalAuditScrutinyService20Engine()

    def create(self, op_index: float = 50.0) -> InternalAuditScrutinyService20Record:
        rec = InternalAuditScrutinyService20Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[InternalAuditScrutinyService20Record]:
...
def test_subsystem_internal_audit_scrutiny_20() -> bool:
    mgr = InternalAuditScrutinyService20Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = InternalAuditScrutinyService20Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_internal_audit_scrutiny_20()
    print(f"internal_audit_scrutiny Part 20: {'PASSED' if ok else 'FAILED'}")
