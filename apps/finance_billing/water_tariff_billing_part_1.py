...
Module: Volumetric Water Consumption Tariff Calculator (Part 1)
...
class WaterTariffBillingService1Engine:
    """
...
class WaterTariffBillingService1Workflow:
    """
    ...
    def __init__(self, record: WaterTariffBillingService1Record):
        self.record = record
...
class WaterTariffBillingService1Manager:
    """
    Service registry and data aggregator for Volumetric Water Consumption Tariff Calculator.
    """
    def __init__(self):
        self.store: Dict[str, WaterTariffBillingService1Record] = {}
        self.engine = WaterTariffBillingService1Engine()

    def create(self, op_index: float = 50.0) -> WaterTariffBillingService1Record:
        rec = WaterTariffBillingService1Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[WaterTariffBillingService1Record]:
...
def test_subsystem_water_tariff_billing_1() -> bool:
    mgr = WaterTariffBillingService1Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = WaterTariffBillingService1Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_water_tariff_billing_1()
    print(f"water_tariff_billing Part 1: {'PASSED' if ok else 'FAILED'}")
