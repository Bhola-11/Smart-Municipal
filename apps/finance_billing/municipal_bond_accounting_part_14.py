...
Module: Civic Infrastructure Green Bond Accounting (Part 14)
...
class MunicipalBondAccountingService14Engine:
    """
...
class MunicipalBondAccountingService14Workflow:
    """
    ...
    def __init__(self, record: MunicipalBondAccountingService14Record):
        self.record = record
...
class MunicipalBondAccountingService14Manager:
    """
    Service registry and data aggregator for Civic Infrastructure Green Bond Accounting.
    """
    def __init__(self):
        self.store: Dict[str, MunicipalBondAccountingService14Record] = {}
        self.engine = MunicipalBondAccountingService14Engine()

    def create(self, op_index: float = 50.0) -> MunicipalBondAccountingService14Record:
        rec = MunicipalBondAccountingService14Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[MunicipalBondAccountingService14Record]:
...
def test_subsystem_municipal_bond_accounting_14() -> bool:
    mgr = MunicipalBondAccountingService14Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = MunicipalBondAccountingService14Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_municipal_bond_accounting_14()
    print(f"municipal_bond_accounting Part 14: {'PASSED' if ok else 'FAILED'}")
