...
Module: Volumetric Water Consumption Tariff Calculator (Part 4)
...
class WaterTariffBillingService4Engine:
    """
...
class WaterTariffBillingService4Workflow:
    """
    ...
    def __init__(self, record: WaterTariffBillingService4Record):
        self.record = record
...
class WaterTariffBillingService4Manager:
    """
    Service registry and data aggregator for Volumetric Water Consumption Tariff Calculator.
    """
    def __init__(self):
        self.store: Dict[str, WaterTariffBillingService4Record] = {}
        self.engine = WaterTariffBillingService4Engine()

    def create(self, op_index: float = 50.0) -> WaterTariffBillingService4Record:
        rec = WaterTariffBillingService4Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[WaterTariffBillingService4Record]:
...
def test_subsystem_water_tariff_billing_4() -> bool:
    mgr = WaterTariffBillingService4Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = WaterTariffBillingService4Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_water_tariff_billing_4()
    print(f"water_tariff_billing Part 4: {'PASSED' if ok else 'FAILED'}")
