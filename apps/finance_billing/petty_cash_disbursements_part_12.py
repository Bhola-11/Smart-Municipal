...
Module: Ward Sub-Office Petty Cash Float Ledgers (Part 12)
...
class PettyCashDisbursementsService12Engine:
    """
...
class PettyCashDisbursementsService12Workflow:
    """
    ...
    def __init__(self, record: PettyCashDisbursementsService12Record):
        self.record = record
...
class PettyCashDisbursementsService12Manager:
    """
    Service registry and data aggregator for Ward Sub-Office Petty Cash Float Ledgers.
    """
    def __init__(self):
        self.store: Dict[str, PettyCashDisbursementsService12Record] = {}
        self.engine = PettyCashDisbursementsService12Engine()

    def create(self, op_index: float = 50.0) -> PettyCashDisbursementsService12Record:
        rec = PettyCashDisbursementsService12Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[PettyCashDisbursementsService12Record]:
...
def test_subsystem_petty_cash_disbursements_12() -> bool:
    mgr = PettyCashDisbursementsService12Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = PettyCashDisbursementsService12Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_petty_cash_disbursements_12()
    print(f"petty_cash_disbursements Part 12: {'PASSED' if ok else 'FAILED'}")
