...
Module: General Ledger Double-Entry Journal Engine (Part 21)
...
class DoubleEntryJournalService21Engine:
    """
...
class DoubleEntryJournalService21Workflow:
    """
    ...
    def __init__(self, record: DoubleEntryJournalService21Record):
        self.record = record
...
class DoubleEntryJournalService21Manager:
    """
    Service registry and data aggregator for General Ledger Double-Entry Journal Engine.
    """
    def __init__(self):
        self.store: Dict[str, DoubleEntryJournalService21Record] = {}
        self.engine = DoubleEntryJournalService21Engine()

    def create(self, op_index: float = 50.0) -> DoubleEntryJournalService21Record:
        rec = DoubleEntryJournalService21Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[DoubleEntryJournalService21Record]:
...
def test_subsystem_double_entry_journal_21() -> bool:
    mgr = DoubleEntryJournalService21Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = DoubleEntryJournalService21Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_double_entry_journal_21()
    print(f"double_entry_journal Part 21: {'PASSED' if ok else 'FAILED'}")
