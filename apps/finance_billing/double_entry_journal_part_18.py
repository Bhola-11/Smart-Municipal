...
Module: General Ledger Double-Entry Journal Engine (Part 18)
...
class DoubleEntryJournalService18Engine:
    """
...
class DoubleEntryJournalService18Workflow:
    """
    ...
    def __init__(self, record: DoubleEntryJournalService18Record):
        self.record = record
...
class DoubleEntryJournalService18Manager:
    """
    Service registry and data aggregator for General Ledger Double-Entry Journal Engine.
    """
    def __init__(self):
        self.store: Dict[str, DoubleEntryJournalService18Record] = {}
        self.engine = DoubleEntryJournalService18Engine()

    def create(self, op_index: float = 50.0) -> DoubleEntryJournalService18Record:
        rec = DoubleEntryJournalService18Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[DoubleEntryJournalService18Record]:
...
def test_subsystem_double_entry_journal_18() -> bool:
    mgr = DoubleEntryJournalService18Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = DoubleEntryJournalService18Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_double_entry_journal_18()
    print(f"double_entry_journal Part 18: {'PASSED' if ok else 'FAILED'}")
