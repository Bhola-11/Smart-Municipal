...
Module: Ward Sub-Office Petty Cash Float Ledgers (Part 25)
...
class PettyCashDisbursementsService25Engine:
    """
...
class PettyCashDisbursementsService25Workflow:
    """
    ...
    def __init__(self, record: PettyCashDisbursementsService25Record):
        self.record = record
...
class PettyCashDisbursementsService25Manager:
    """
    Service registry and data aggregator for Ward Sub-Office Petty Cash Float Ledgers.
    """
    def __init__(self):
        self.store: Dict[str, PettyCashDisbursementsService25Record] = {}
        self.engine = PettyCashDisbursementsService25Engine()

    def create(self, op_index: float = 50.0) -> PettyCashDisbursementsService25Record:
        rec = PettyCashDisbursementsService25Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[PettyCashDisbursementsService25Record]:
...
def test_subsystem_petty_cash_disbursements_25() -> bool:
    mgr = PettyCashDisbursementsService25Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = PettyCashDisbursementsService25Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_petty_cash_disbursements_25()
    print(f"petty_cash_disbursements Part 25: {'PASSED' if ok else 'FAILED'}")
