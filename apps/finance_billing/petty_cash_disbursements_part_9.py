...
Module: Ward Sub-Office Petty Cash Float Ledgers (Part 9)
...
class PettyCashDisbursementsService9Engine:
    """
...
class PettyCashDisbursementsService9Workflow:
    """
    ...
    def __init__(self, record: PettyCashDisbursementsService9Record):
        self.record = record
...
class PettyCashDisbursementsService9Manager:
    """
    Service registry and data aggregator for Ward Sub-Office Petty Cash Float Ledgers.
    """
    def __init__(self):
        self.store: Dict[str, PettyCashDisbursementsService9Record] = {}
        self.engine = PettyCashDisbursementsService9Engine()

    def create(self, op_index: float = 50.0) -> PettyCashDisbursementsService9Record:
        rec = PettyCashDisbursementsService9Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[PettyCashDisbursementsService9Record]:
...
def test_subsystem_petty_cash_disbursements_9() -> bool:
    mgr = PettyCashDisbursementsService9Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = PettyCashDisbursementsService9Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_petty_cash_disbursements_9()
    print(f"petty_cash_disbursements Part 9: {'PASSED' if ok else 'FAILED'}")
