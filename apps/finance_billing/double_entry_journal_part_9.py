...
Module: General Ledger Double-Entry Journal Engine (Part 9)
...
class DoubleEntryJournalService9Engine:
    """
...
class DoubleEntryJournalService9Workflow:
    """
    ...
    def __init__(self, record: DoubleEntryJournalService9Record):
        self.record = record
...
class DoubleEntryJournalService9Manager:
    """
    Service registry and data aggregator for General Ledger Double-Entry Journal Engine.
    """
    def __init__(self):
        self.store: Dict[str, DoubleEntryJournalService9Record] = {}
        self.engine = DoubleEntryJournalService9Engine()

    def create(self, op_index: float = 50.0) -> DoubleEntryJournalService9Record:
        rec = DoubleEntryJournalService9Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[DoubleEntryJournalService9Record]:
...
def test_subsystem_double_entry_journal_9() -> bool:
    mgr = DoubleEntryJournalService9Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = DoubleEntryJournalService9Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_double_entry_journal_9()
    print(f"double_entry_journal Part 9: {'PASSED' if ok else 'FAILED'}")
