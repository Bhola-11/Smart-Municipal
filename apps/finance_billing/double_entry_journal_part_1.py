...
Module: General Ledger Double-Entry Journal Engine (Part 1)
...
class DoubleEntryJournalService1Engine:
    """
...
class DoubleEntryJournalService1Workflow:
    """
    ...
    def __init__(self, record: DoubleEntryJournalService1Record):
        self.record = record
...
class DoubleEntryJournalService1Manager:
    """
    Service registry and data aggregator for General Ledger Double-Entry Journal Engine.
    """
    def __init__(self):
        self.store: Dict[str, DoubleEntryJournalService1Record] = {}
        self.engine = DoubleEntryJournalService1Engine()

    def create(self, op_index: float = 50.0) -> DoubleEntryJournalService1Record:
        rec = DoubleEntryJournalService1Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[DoubleEntryJournalService1Record]:
...
def test_subsystem_double_entry_journal_1() -> bool:
    mgr = DoubleEntryJournalService1Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = DoubleEntryJournalService1Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_double_entry_journal_1()
    print(f"double_entry_journal Part 1: {'PASSED' if ok else 'FAILED'}")
