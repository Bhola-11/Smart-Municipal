...
Module: Ward Sub-Office Petty Cash Float Ledgers (Part 8)
...
class PettyCashDisbursementsService8Engine:
    """
...
class PettyCashDisbursementsService8Workflow:
    """
    ...
    def __init__(self, record: PettyCashDisbursementsService8Record):
        self.record = record
...
class PettyCashDisbursementsService8Manager:
    """
    Service registry and data aggregator for Ward Sub-Office Petty Cash Float Ledgers.
    """
    def __init__(self):
        self.store: Dict[str, PettyCashDisbursementsService8Record] = {}
        self.engine = PettyCashDisbursementsService8Engine()

    def create(self, op_index: float = 50.0) -> PettyCashDisbursementsService8Record:
        rec = PettyCashDisbursementsService8Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[PettyCashDisbursementsService8Record]:
...
def test_subsystem_petty_cash_disbursements_8() -> bool:
    mgr = PettyCashDisbursementsService8Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = PettyCashDisbursementsService8Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_petty_cash_disbursements_8()
    print(f"petty_cash_disbursements Part 8: {'PASSED' if ok else 'FAILED'}")
