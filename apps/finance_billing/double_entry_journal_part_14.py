...
Module: General Ledger Double-Entry Journal Engine (Part 14)
...
class DoubleEntryJournalService14Engine:
    """
...
class DoubleEntryJournalService14Workflow:
    """
    ...
    def __init__(self, record: DoubleEntryJournalService14Record):
        self.record = record
...
class DoubleEntryJournalService14Manager:
    """
    Service registry and data aggregator for General Ledger Double-Entry Journal Engine.
    """
    def __init__(self):
        self.store: Dict[str, DoubleEntryJournalService14Record] = {}
        self.engine = DoubleEntryJournalService14Engine()

    def create(self, op_index: float = 50.0) -> DoubleEntryJournalService14Record:
        rec = DoubleEntryJournalService14Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[DoubleEntryJournalService14Record]:
...
def test_subsystem_double_entry_journal_14() -> bool:
    mgr = DoubleEntryJournalService14Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = DoubleEntryJournalService14Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_double_entry_journal_14()
    print(f"double_entry_journal Part 14: {'PASSED' if ok else 'FAILED'}")
