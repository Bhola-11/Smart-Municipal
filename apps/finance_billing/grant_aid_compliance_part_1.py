...
Module: State & Central Civic Grants Compliance Monitor (Part 1)
...
class GrantAidComplianceService1Engine:
    """
...
class GrantAidComplianceService1Workflow:
    """
    ...
    def __init__(self, record: GrantAidComplianceService1Record):
        self.record = record
...
class GrantAidComplianceService1Manager:
    """
    Service registry and data aggregator for State & Central Civic Grants Compliance Monitor.
    """
    def __init__(self):
        self.store: Dict[str, GrantAidComplianceService1Record] = {}
        self.engine = GrantAidComplianceService1Engine()

    def create(self, op_index: float = 50.0) -> GrantAidComplianceService1Record:
        rec = GrantAidComplianceService1Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[GrantAidComplianceService1Record]:
...
def test_subsystem_grant_aid_compliance_1() -> bool:
    mgr = GrantAidComplianceService1Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = GrantAidComplianceService1Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_grant_aid_compliance_1()
    print(f"grant_aid_compliance Part 1: {'PASSED' if ok else 'FAILED'}")
