...
Module: Ward Sub-Office Petty Cash Float Ledgers (Part 27)
...
class PettyCashDisbursementsService27Engine:
    """
...
class PettyCashDisbursementsService27Workflow:
    """
    ...
    def __init__(self, record: PettyCashDisbursementsService27Record):
        self.record = record
...
class PettyCashDisbursementsService27Manager:
    """
    Service registry and data aggregator for Ward Sub-Office Petty Cash Float Ledgers.
    """
    def __init__(self):
        self.store: Dict[str, PettyCashDisbursementsService27Record] = {}
        self.engine = PettyCashDisbursementsService27Engine()

    def create(self, op_index: float = 50.0) -> PettyCashDisbursementsService27Record:
        rec = PettyCashDisbursementsService27Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[PettyCashDisbursementsService27Record]:
...
def test_subsystem_petty_cash_disbursements_27() -> bool:
    mgr = PettyCashDisbursementsService27Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = PettyCashDisbursementsService27Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_petty_cash_disbursements_27()
    print(f"petty_cash_disbursements Part 27: {'PASSED' if ok else 'FAILED'}")
