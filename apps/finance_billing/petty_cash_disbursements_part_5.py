...
Module: Ward Sub-Office Petty Cash Float Ledgers (Part 5)
...
class PettyCashDisbursementsService5Engine:
    """
...
class PettyCashDisbursementsService5Workflow:
    """
    ...
    def __init__(self, record: PettyCashDisbursementsService5Record):
        self.record = record
...
class PettyCashDisbursementsService5Manager:
    """
    Service registry and data aggregator for Ward Sub-Office Petty Cash Float Ledgers.
    """
    def __init__(self):
        self.store: Dict[str, PettyCashDisbursementsService5Record] = {}
        self.engine = PettyCashDisbursementsService5Engine()

    def create(self, op_index: float = 50.0) -> PettyCashDisbursementsService5Record:
        rec = PettyCashDisbursementsService5Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[PettyCashDisbursementsService5Record]:
...
def test_subsystem_petty_cash_disbursements_5() -> bool:
    mgr = PettyCashDisbursementsService5Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = PettyCashDisbursementsService5Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_petty_cash_disbursements_5()
    print(f"petty_cash_disbursements Part 5: {'PASSED' if ok else 'FAILED'}")
