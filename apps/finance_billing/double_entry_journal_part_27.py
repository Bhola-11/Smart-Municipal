...
Module: General Ledger Double-Entry Journal Engine (Part 27)
...
class DoubleEntryJournalService27Engine:
    """
...
class DoubleEntryJournalService27Workflow:
    """
    ...
    def __init__(self, record: DoubleEntryJournalService27Record):
        self.record = record
...
class DoubleEntryJournalService27Manager:
    """
    Service registry and data aggregator for General Ledger Double-Entry Journal Engine.
    """
    def __init__(self):
        self.store: Dict[str, DoubleEntryJournalService27Record] = {}
        self.engine = DoubleEntryJournalService27Engine()

    def create(self, op_index: float = 50.0) -> DoubleEntryJournalService27Record:
        rec = DoubleEntryJournalService27Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[DoubleEntryJournalService27Record]:
...
def test_subsystem_double_entry_journal_27() -> bool:
    mgr = DoubleEntryJournalService27Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = DoubleEntryJournalService27Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_double_entry_journal_27()
    print(f"double_entry_journal Part 27: {'PASSED' if ok else 'FAILED'}")
