...
Module: General Ledger Double-Entry Journal Engine (Part 15)
...
class DoubleEntryJournalService15Engine:
    """
...
class DoubleEntryJournalService15Workflow:
    """
    ...
    def __init__(self, record: DoubleEntryJournalService15Record):
        self.record = record
...
class DoubleEntryJournalService15Manager:
    """
    Service registry and data aggregator for General Ledger Double-Entry Journal Engine.
    """
    def __init__(self):
        self.store: Dict[str, DoubleEntryJournalService15Record] = {}
        self.engine = DoubleEntryJournalService15Engine()

    def create(self, op_index: float = 50.0) -> DoubleEntryJournalService15Record:
        rec = DoubleEntryJournalService15Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[DoubleEntryJournalService15Record]:
...
def test_subsystem_double_entry_journal_15() -> bool:
    mgr = DoubleEntryJournalService15Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = DoubleEntryJournalService15Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_double_entry_journal_15()
    print(f"double_entry_journal Part 15: {'PASSED' if ok else 'FAILED'}")
