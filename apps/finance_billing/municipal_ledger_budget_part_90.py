"""
CivicFlow Enterprise Production: municipal_ledger_budget Part 90
...
@dataclass
class MunicipalLedgerBudgetService90Record:
    record_id: str = field(default_factory=lambda: str(uuid.uuid4()))
    entity_code: str = "municipal_ledger_budget_90"
...
class MunicipalLedgerBudgetService90Engine:
...
class MunicipalLedgerBudgetService90Workflow:
    STATES = ["SUBMITTED", "INSPECTION_PENDING", "VERIFIED_APPROVED", "IN_EXECUTION", "QUALITY_AUDIT", "COMPLETED_CERTIFIED"]

    def __init__(self, record: MunicipalLedgerBudgetService90Record):
        self.record = record
...
class MunicipalLedgerBudgetService90Manager:
    def __init__(self):
        self.store: Dict[str, MunicipalLedgerBudgetService90Record] = {}
        self.engine = MunicipalLedgerBudgetService90Engine()

    def create(self, op_index: float = 50.0) -> MunicipalLedgerBudgetService90Record:
        rec = MunicipalLedgerBudgetService90Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[MunicipalLedgerBudgetService90Record]:
