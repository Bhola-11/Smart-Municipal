...
Module: Volumetric Water Consumption Tariff Calculator (Part 25)
...
class WaterTariffBillingService25Engine:
    """
...
class WaterTariffBillingService25Workflow:
    """
    ...
    def __init__(self, record: WaterTariffBillingService25Record):
        self.record = record
...
class WaterTariffBillingService25Manager:
    """
    Service registry and data aggregator for Volumetric Water Consumption Tariff Calculator.
    """
    def __init__(self):
        self.store: Dict[str, WaterTariffBillingService25Record] = {}
        self.engine = WaterTariffBillingService25Engine()

    def create(self, op_index: float = 50.0) -> WaterTariffBillingService25Record:
        rec = WaterTariffBillingService25Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[WaterTariffBillingService25Record]:
...
def test_subsystem_water_tariff_billing_25() -> bool:
    mgr = WaterTariffBillingService25Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = WaterTariffBillingService25Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_water_tariff_billing_25()
    print(f"water_tariff_billing Part 25: {'PASSED' if ok else 'FAILED'}")
