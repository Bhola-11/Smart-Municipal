...
Module: General Ledger Double-Entry Journal Engine (Part 25)
...
class DoubleEntryJournalService25Engine:
    """
...
class DoubleEntryJournalService25Workflow:
    """
    ...
    def __init__(self, record: DoubleEntryJournalService25Record):
        self.record = record
...
class DoubleEntryJournalService25Manager:
    """
    Service registry and data aggregator for General Ledger Double-Entry Journal Engine.
    """
    def __init__(self):
        self.store: Dict[str, DoubleEntryJournalService25Record] = {}
        self.engine = DoubleEntryJournalService25Engine()

    def create(self, op_index: float = 50.0) -> DoubleEntryJournalService25Record:
        rec = DoubleEntryJournalService25Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[DoubleEntryJournalService25Record]:
...
def test_subsystem_double_entry_journal_25() -> bool:
    mgr = DoubleEntryJournalService25Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = DoubleEntryJournalService25Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_double_entry_journal_25()
    print(f"double_entry_journal Part 25: {'PASSED' if ok else 'FAILED'}")
