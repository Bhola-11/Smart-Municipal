...
Module: Volumetric Water Consumption Tariff Calculator (Part 14)
...
class WaterTariffBillingService14Engine:
    """
...
class WaterTariffBillingService14Workflow:
    """
    ...
    def __init__(self, record: WaterTariffBillingService14Record):
        self.record = record
...
class WaterTariffBillingService14Manager:
    """
    Service registry and data aggregator for Volumetric Water Consumption Tariff Calculator.
    """
    def __init__(self):
        self.store: Dict[str, WaterTariffBillingService14Record] = {}
        self.engine = WaterTariffBillingService14Engine()

    def create(self, op_index: float = 50.0) -> WaterTariffBillingService14Record:
        rec = WaterTariffBillingService14Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[WaterTariffBillingService14Record]:
...
def test_subsystem_water_tariff_billing_14() -> bool:
    mgr = WaterTariffBillingService14Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = WaterTariffBillingService14Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_water_tariff_billing_14()
    print(f"water_tariff_billing Part 14: {'PASSED' if ok else 'FAILED'}")
