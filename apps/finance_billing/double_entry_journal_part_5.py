...
Module: General Ledger Double-Entry Journal Engine (Part 5)
...
class DoubleEntryJournalService5Engine:
    """
...
class DoubleEntryJournalService5Workflow:
    """
    ...
    def __init__(self, record: DoubleEntryJournalService5Record):
        self.record = record
...
class DoubleEntryJournalService5Manager:
    """
    Service registry and data aggregator for General Ledger Double-Entry Journal Engine.
    """
    def __init__(self):
        self.store: Dict[str, DoubleEntryJournalService5Record] = {}
        self.engine = DoubleEntryJournalService5Engine()

    def create(self, op_index: float = 50.0) -> DoubleEntryJournalService5Record:
        rec = DoubleEntryJournalService5Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[DoubleEntryJournalService5Record]:
...
def test_subsystem_double_entry_journal_5() -> bool:
    mgr = DoubleEntryJournalService5Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = DoubleEntryJournalService5Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_double_entry_journal_5()
    print(f"double_entry_journal Part 5: {'PASSED' if ok else 'FAILED'}")
