...
Module: Civic Infrastructure Green Bond Accounting (Part 20)
...
class MunicipalBondAccountingService20Engine:
    """
...
class MunicipalBondAccountingService20Workflow:
    """
    ...
    def __init__(self, record: MunicipalBondAccountingService20Record):
        self.record = record
...
class MunicipalBondAccountingService20Manager:
    """
    Service registry and data aggregator for Civic Infrastructure Green Bond Accounting.
    """
    def __init__(self):
        self.store: Dict[str, MunicipalBondAccountingService20Record] = {}
        self.engine = MunicipalBondAccountingService20Engine()

    def create(self, op_index: float = 50.0) -> MunicipalBondAccountingService20Record:
        rec = MunicipalBondAccountingService20Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[MunicipalBondAccountingService20Record]:
...
def test_subsystem_municipal_bond_accounting_20() -> bool:
    mgr = MunicipalBondAccountingService20Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = MunicipalBondAccountingService20Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_municipal_bond_accounting_20()
    print(f"municipal_bond_accounting Part 20: {'PASSED' if ok else 'FAILED'}")
