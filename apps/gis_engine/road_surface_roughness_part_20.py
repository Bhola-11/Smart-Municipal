...
Module: Road Surface Roughness Index (IRI) Mapping (Part 20)
App: apps.gis_engine
...
class RoadSurfaceRoughnessService20Engine:
    """
...
class RoadSurfaceRoughnessService20Workflow:
    """
    ...
    def __init__(self, record: RoadSurfaceRoughnessService20Record):
        self.record = record
...
class RoadSurfaceRoughnessService20Manager:
    """
    Service registry and data aggregator for Road Surface Roughness Index (IRI) Mapping.
    """
    def __init__(self):
        self.store: Dict[str, RoadSurfaceRoughnessService20Record] = {}
        self.engine = RoadSurfaceRoughnessService20Engine()

    def create(self, op_index: float = 50.0) -> RoadSurfaceRoughnessService20Record:
        rec = RoadSurfaceRoughnessService20Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[RoadSurfaceRoughnessService20Record]:
...
def test_subsystem_road_surface_roughness_20() -> bool:
    mgr = RoadSurfaceRoughnessService20Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = RoadSurfaceRoughnessService20Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_road_surface_roughness_20()
    print(f"road_surface_roughness Part 20: {'PASSED' if ok else 'FAILED'}")
