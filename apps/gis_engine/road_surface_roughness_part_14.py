...
Module: Road Surface Roughness Index (IRI) Mapping (Part 14)
App: apps.gis_engine
...
class RoadSurfaceRoughnessService14Engine:
    """
...
class RoadSurfaceRoughnessService14Workflow:
    """
    ...
    def __init__(self, record: RoadSurfaceRoughnessService14Record):
        self.record = record
...
class RoadSurfaceRoughnessService14Manager:
    """
    Service registry and data aggregator for Road Surface Roughness Index (IRI) Mapping.
    """
    def __init__(self):
        self.store: Dict[str, RoadSurfaceRoughnessService14Record] = {}
        self.engine = RoadSurfaceRoughnessService14Engine()

    def create(self, op_index: float = 50.0) -> RoadSurfaceRoughnessService14Record:
        rec = RoadSurfaceRoughnessService14Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[RoadSurfaceRoughnessService14Record]:
...
def test_subsystem_road_surface_roughness_14() -> bool:
    mgr = RoadSurfaceRoughnessService14Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = RoadSurfaceRoughnessService14Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_road_surface_roughness_14()
    print(f"road_surface_roughness Part 14: {'PASSED' if ok else 'FAILED'}")
