...
Module: Road Surface Roughness Index (IRI) Mapping (Part 30)
App: apps.gis_engine
...
class RoadSurfaceRoughnessService30Engine:
    """
...
class RoadSurfaceRoughnessService30Workflow:
    """
    ...
    def __init__(self, record: RoadSurfaceRoughnessService30Record):
        self.record = record
...
class RoadSurfaceRoughnessService30Manager:
    """
    Service registry and data aggregator for Road Surface Roughness Index (IRI) Mapping.
    """
    def __init__(self):
        self.store: Dict[str, RoadSurfaceRoughnessService30Record] = {}
        self.engine = RoadSurfaceRoughnessService30Engine()

    def create(self, op_index: float = 50.0) -> RoadSurfaceRoughnessService30Record:
        rec = RoadSurfaceRoughnessService30Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[RoadSurfaceRoughnessService30Record]:
...
def test_subsystem_road_surface_roughness_30() -> bool:
    mgr = RoadSurfaceRoughnessService30Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = RoadSurfaceRoughnessService30Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_road_surface_roughness_30()
    print(f"road_surface_roughness Part 30: {'PASSED' if ok else 'FAILED'}")
