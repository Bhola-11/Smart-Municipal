...
Module: Road Surface Roughness Index (IRI) Mapping (Part 4)
App: apps.gis_engine
...
class RoadSurfaceRoughnessService4Engine:
    """
...
class RoadSurfaceRoughnessService4Workflow:
    """
    ...
    def __init__(self, record: RoadSurfaceRoughnessService4Record):
        self.record = record
...
class RoadSurfaceRoughnessService4Manager:
    """
    Service registry and data aggregator for Road Surface Roughness Index (IRI) Mapping.
    """
    def __init__(self):
        self.store: Dict[str, RoadSurfaceRoughnessService4Record] = {}
        self.engine = RoadSurfaceRoughnessService4Engine()

    def create(self, op_index: float = 50.0) -> RoadSurfaceRoughnessService4Record:
        rec = RoadSurfaceRoughnessService4Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[RoadSurfaceRoughnessService4Record]:
...
def test_subsystem_road_surface_roughness_4() -> bool:
    mgr = RoadSurfaceRoughnessService4Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = RoadSurfaceRoughnessService4Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_road_surface_roughness_4()
    print(f"road_surface_roughness Part 4: {'PASSED' if ok else 'FAILED'}")
