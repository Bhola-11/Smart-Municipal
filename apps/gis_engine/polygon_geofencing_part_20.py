...
Module: Point-in-Polygon Geofencing Raycasting (Part 20)
App: apps.gis_engine
...
class PolygonGeofencingService20Engine:
    """
...
class PolygonGeofencingService20Workflow:
    """
    ...
    def __init__(self, record: PolygonGeofencingService20Record):
        self.record = record
...
class PolygonGeofencingService20Manager:
    """
    Service registry and data aggregator for Point-in-Polygon Geofencing Raycasting.
    """
    def __init__(self):
        self.store: Dict[str, PolygonGeofencingService20Record] = {}
        self.engine = PolygonGeofencingService20Engine()

    def create(self, op_index: float = 50.0) -> PolygonGeofencingService20Record:
        rec = PolygonGeofencingService20Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[PolygonGeofencingService20Record]:
...
def test_subsystem_polygon_geofencing_20() -> bool:
    mgr = PolygonGeofencingService20Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = PolygonGeofencingService20Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_polygon_geofencing_20()
    print(f"polygon_geofencing Part 20: {'PASSED' if ok else 'FAILED'}")
