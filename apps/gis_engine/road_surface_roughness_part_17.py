...
Module: Road Surface Roughness Index (IRI) Mapping (Part 17)
App: apps.gis_engine
...
class RoadSurfaceRoughnessService17Engine:
    """
...
class RoadSurfaceRoughnessService17Workflow:
    """
    ...
    def __init__(self, record: RoadSurfaceRoughnessService17Record):
        self.record = record
...
class RoadSurfaceRoughnessService17Manager:
    """
    Service registry and data aggregator for Road Surface Roughness Index (IRI) Mapping.
    """
    def __init__(self):
        self.store: Dict[str, RoadSurfaceRoughnessService17Record] = {}
        self.engine = RoadSurfaceRoughnessService17Engine()

    def create(self, op_index: float = 50.0) -> RoadSurfaceRoughnessService17Record:
        rec = RoadSurfaceRoughnessService17Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[RoadSurfaceRoughnessService17Record]:
...
def test_subsystem_road_surface_roughness_17() -> bool:
    mgr = RoadSurfaceRoughnessService17Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = RoadSurfaceRoughnessService17Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_road_surface_roughness_17()
    print(f"road_surface_roughness Part 17: {'PASSED' if ok else 'FAILED'}")
