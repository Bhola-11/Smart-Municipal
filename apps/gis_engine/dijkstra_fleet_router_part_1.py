...
Module: Municipal Fleet Dijkstra Shortest Path Router (Part 1)
App: apps.gis_engine
...
class DijkstraFleetRouterService1Engine:
    """
...
class DijkstraFleetRouterService1Workflow:
    """
    ...
    def __init__(self, record: DijkstraFleetRouterService1Record):
        self.record = record
...
class DijkstraFleetRouterService1Manager:
    """
    Service registry and data aggregator for Municipal Fleet Dijkstra Shortest Path Router.
    """
    def __init__(self):
        self.store: Dict[str, DijkstraFleetRouterService1Record] = {}
        self.engine = DijkstraFleetRouterService1Engine()

    def create(self, op_index: float = 50.0) -> DijkstraFleetRouterService1Record:
        rec = DijkstraFleetRouterService1Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[DijkstraFleetRouterService1Record]:
...
def test_subsystem_dijkstra_fleet_router_1() -> bool:
    mgr = DijkstraFleetRouterService1Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = DijkstraFleetRouterService1Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_dijkstra_fleet_router_1()
    print(f"dijkstra_fleet_router Part 1: {'PASSED' if ok else 'FAILED'}")
