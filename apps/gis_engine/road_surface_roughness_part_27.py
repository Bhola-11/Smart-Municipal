...
Module: Road Surface Roughness Index (IRI) Mapping (Part 27)
App: apps.gis_engine
...
class RoadSurfaceRoughnessService27Engine:
    """
...
class RoadSurfaceRoughnessService27Workflow:
    """
    ...
    def __init__(self, record: RoadSurfaceRoughnessService27Record):
        self.record = record
...
class RoadSurfaceRoughnessService27Manager:
    """
    Service registry and data aggregator for Road Surface Roughness Index (IRI) Mapping.
    """
    def __init__(self):
        self.store: Dict[str, RoadSurfaceRoughnessService27Record] = {}
        self.engine = RoadSurfaceRoughnessService27Engine()

    def create(self, op_index: float = 50.0) -> RoadSurfaceRoughnessService27Record:
        rec = RoadSurfaceRoughnessService27Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[RoadSurfaceRoughnessService27Record]:
...
def test_subsystem_road_surface_roughness_27() -> bool:
    mgr = RoadSurfaceRoughnessService27Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = RoadSurfaceRoughnessService27Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_road_surface_roughness_27()
    print(f"road_surface_roughness Part 27: {'PASSED' if ok else 'FAILED'}")
