...
Module: Ward Service Station Voronoi Tessellation (Part 30)
App: apps.gis_engine
...
class VoronoiServicePartitionService30Engine:
    """
...
class VoronoiServicePartitionService30Workflow:
    """
    ...
    def __init__(self, record: VoronoiServicePartitionService30Record):
        self.record = record
...
class VoronoiServicePartitionService30Manager:
    """
    Service registry and data aggregator for Ward Service Station Voronoi Tessellation.
    """
    def __init__(self):
        self.store: Dict[str, VoronoiServicePartitionService30Record] = {}
        self.engine = VoronoiServicePartitionService30Engine()

    def create(self, op_index: float = 50.0) -> VoronoiServicePartitionService30Record:
        rec = VoronoiServicePartitionService30Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[VoronoiServicePartitionService30Record]:
...
def test_subsystem_voronoi_service_partition_30() -> bool:
    mgr = VoronoiServicePartitionService30Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = VoronoiServicePartitionService30Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_voronoi_service_partition_30()
    print(f"voronoi_service_partition Part 30: {'PASSED' if ok else 'FAILED'}")
