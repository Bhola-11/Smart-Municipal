...
Module: Road Surface Roughness Index (IRI) Mapping (Part 13)
App: apps.gis_engine
...
class RoadSurfaceRoughnessService13Engine:
    """
...
class RoadSurfaceRoughnessService13Workflow:
    """
    ...
    def __init__(self, record: RoadSurfaceRoughnessService13Record):
        self.record = record
...
class RoadSurfaceRoughnessService13Manager:
    """
    Service registry and data aggregator for Road Surface Roughness Index (IRI) Mapping.
    """
    def __init__(self):
        self.store: Dict[str, RoadSurfaceRoughnessService13Record] = {}
        self.engine = RoadSurfaceRoughnessService13Engine()

    def create(self, op_index: float = 50.0) -> RoadSurfaceRoughnessService13Record:
        rec = RoadSurfaceRoughnessService13Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[RoadSurfaceRoughnessService13Record]:
...
def test_subsystem_road_surface_roughness_13() -> bool:
    mgr = RoadSurfaceRoughnessService13Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = RoadSurfaceRoughnessService13Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_road_surface_roughness_13()
    print(f"road_surface_roughness Part 13: {'PASSED' if ok else 'FAILED'}")
