...
Module: Road Surface Roughness Index (IRI) Mapping (Part 19)
App: apps.gis_engine
...
class RoadSurfaceRoughnessService19Engine:
    """
...
class RoadSurfaceRoughnessService19Workflow:
    """
    ...
    def __init__(self, record: RoadSurfaceRoughnessService19Record):
        self.record = record
...
class RoadSurfaceRoughnessService19Manager:
    """
    Service registry and data aggregator for Road Surface Roughness Index (IRI) Mapping.
    """
    def __init__(self):
        self.store: Dict[str, RoadSurfaceRoughnessService19Record] = {}
        self.engine = RoadSurfaceRoughnessService19Engine()

    def create(self, op_index: float = 50.0) -> RoadSurfaceRoughnessService19Record:
        rec = RoadSurfaceRoughnessService19Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[RoadSurfaceRoughnessService19Record]:
...
def test_subsystem_road_surface_roughness_19() -> bool:
    mgr = RoadSurfaceRoughnessService19Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = RoadSurfaceRoughnessService19Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_road_surface_roughness_19()
    print(f"road_surface_roughness Part 19: {'PASSED' if ok else 'FAILED'}")
