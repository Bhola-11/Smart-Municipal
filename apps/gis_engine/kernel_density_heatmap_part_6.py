...
Module: Civic Incident Kernel Density Heatmap Generator (Part 6)
App: apps.gis_engine
...
class KernelDensityHeatmapService6Engine:
    """
...
class KernelDensityHeatmapService6Workflow:
    """
    ...
    def __init__(self, record: KernelDensityHeatmapService6Record):
        self.record = record
...
class KernelDensityHeatmapService6Manager:
    """
    Service registry and data aggregator for Civic Incident Kernel Density Heatmap Generator.
    """
    def __init__(self):
        self.store: Dict[str, KernelDensityHeatmapService6Record] = {}
        self.engine = KernelDensityHeatmapService6Engine()

    def create(self, op_index: float = 50.0) -> KernelDensityHeatmapService6Record:
        rec = KernelDensityHeatmapService6Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[KernelDensityHeatmapService6Record]:
...
def test_subsystem_kernel_density_heatmap_6() -> bool:
    mgr = KernelDensityHeatmapService6Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = KernelDensityHeatmapService6Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_kernel_density_heatmap_6()
    print(f"kernel_density_heatmap Part 6: {'PASSED' if ok else 'FAILED'}")
