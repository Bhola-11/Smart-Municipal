...
Module: Civic Incident Kernel Density Heatmap Generator (Part 8)
App: apps.gis_engine
...
class KernelDensityHeatmapService8Engine:
    """
...
class KernelDensityHeatmapService8Workflow:
    """
    ...
    def __init__(self, record: KernelDensityHeatmapService8Record):
        self.record = record
...
class KernelDensityHeatmapService8Manager:
    """
    Service registry and data aggregator for Civic Incident Kernel Density Heatmap Generator.
    """
    def __init__(self):
        self.store: Dict[str, KernelDensityHeatmapService8Record] = {}
        self.engine = KernelDensityHeatmapService8Engine()

    def create(self, op_index: float = 50.0) -> KernelDensityHeatmapService8Record:
        rec = KernelDensityHeatmapService8Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[KernelDensityHeatmapService8Record]:
...
def test_subsystem_kernel_density_heatmap_8() -> bool:
    mgr = KernelDensityHeatmapService8Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = KernelDensityHeatmapService8Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_kernel_density_heatmap_8()
    print(f"kernel_density_heatmap Part 8: {'PASSED' if ok else 'FAILED'}")
