...
Module: Vehicle GPS Track-to-Road Snapping Engine (Part 20)
App: apps.gis_engine
...
class GpsSnappingMatcherService20Engine:
    """
...
class GpsSnappingMatcherService20Workflow:
    """
    ...
    def __init__(self, record: GpsSnappingMatcherService20Record):
        self.record = record
...
class GpsSnappingMatcherService20Manager:
    """
    Service registry and data aggregator for Vehicle GPS Track-to-Road Snapping Engine.
    """
    def __init__(self):
        self.store: Dict[str, GpsSnappingMatcherService20Record] = {}
        self.engine = GpsSnappingMatcherService20Engine()

    def create(self, op_index: float = 50.0) -> GpsSnappingMatcherService20Record:
        rec = GpsSnappingMatcherService20Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[GpsSnappingMatcherService20Record]:
...
def test_subsystem_gps_snapping_matcher_20() -> bool:
    mgr = GpsSnappingMatcherService20Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = GpsSnappingMatcherService20Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_gps_snapping_matcher_20()
    print(f"gps_snapping_matcher Part 20: {'PASSED' if ok else 'FAILED'}")
