...
Module: Ward Service Station Voronoi Tessellation (Part 15)
App: apps.gis_engine
...
class VoronoiServicePartitionService15Engine:
    """
...
class VoronoiServicePartitionService15Workflow:
    """
    ...
    def __init__(self, record: VoronoiServicePartitionService15Record):
        self.record = record
...
class VoronoiServicePartitionService15Manager:
    """
    Service registry and data aggregator for Ward Service Station Voronoi Tessellation.
    """
    def __init__(self):
        self.store: Dict[str, VoronoiServicePartitionService15Record] = {}
        self.engine = VoronoiServicePartitionService15Engine()

    def create(self, op_index: float = 50.0) -> VoronoiServicePartitionService15Record:
        rec = VoronoiServicePartitionService15Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[VoronoiServicePartitionService15Record]:
...
def test_subsystem_voronoi_service_partition_15() -> bool:
    mgr = VoronoiServicePartitionService15Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = VoronoiServicePartitionService15Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_voronoi_service_partition_15()
    print(f"voronoi_service_partition Part 15: {'PASSED' if ok else 'FAILED'}")
