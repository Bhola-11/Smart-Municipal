...
Module: Road Surface Roughness Index (IRI) Mapping (Part 25)
App: apps.gis_engine
...
class RoadSurfaceRoughnessService25Engine:
    """
...
class RoadSurfaceRoughnessService25Workflow:
    """
    ...
    def __init__(self, record: RoadSurfaceRoughnessService25Record):
        self.record = record
...
class RoadSurfaceRoughnessService25Manager:
    """
    Service registry and data aggregator for Road Surface Roughness Index (IRI) Mapping.
    """
    def __init__(self):
        self.store: Dict[str, RoadSurfaceRoughnessService25Record] = {}
        self.engine = RoadSurfaceRoughnessService25Engine()

    def create(self, op_index: float = 50.0) -> RoadSurfaceRoughnessService25Record:
        rec = RoadSurfaceRoughnessService25Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[RoadSurfaceRoughnessService25Record]:
...
def test_subsystem_road_surface_roughness_25() -> bool:
    mgr = RoadSurfaceRoughnessService25Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = RoadSurfaceRoughnessService25Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_road_surface_roughness_25()
    print(f"road_surface_roughness Part 25: {'PASSED' if ok else 'FAILED'}")
