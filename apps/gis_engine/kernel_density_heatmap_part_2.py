...
Module: Civic Incident Kernel Density Heatmap Generator (Part 2)
App: apps.gis_engine
...
class KernelDensityHeatmapService2Engine:
    """
...
class KernelDensityHeatmapService2Workflow:
    """
    ...
    def __init__(self, record: KernelDensityHeatmapService2Record):
        self.record = record
...
class KernelDensityHeatmapService2Manager:
    """
    Service registry and data aggregator for Civic Incident Kernel Density Heatmap Generator.
    """
    def __init__(self):
        self.store: Dict[str, KernelDensityHeatmapService2Record] = {}
        self.engine = KernelDensityHeatmapService2Engine()

    def create(self, op_index: float = 50.0) -> KernelDensityHeatmapService2Record:
        rec = KernelDensityHeatmapService2Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[KernelDensityHeatmapService2Record]:
...
def test_subsystem_kernel_density_heatmap_2() -> bool:
    mgr = KernelDensityHeatmapService2Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = KernelDensityHeatmapService2Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_kernel_density_heatmap_2()
    print(f"kernel_density_heatmap Part 2: {'PASSED' if ok else 'FAILED'}")
