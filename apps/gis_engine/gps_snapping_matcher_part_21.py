...
Module: Vehicle GPS Track-to-Road Snapping Engine (Part 21)
App: apps.gis_engine
...
class GpsSnappingMatcherService21Engine:
    """
...
class GpsSnappingMatcherService21Workflow:
    """
    ...
    def __init__(self, record: GpsSnappingMatcherService21Record):
        self.record = record
...
class GpsSnappingMatcherService21Manager:
    """
    Service registry and data aggregator for Vehicle GPS Track-to-Road Snapping Engine.
    """
    def __init__(self):
        self.store: Dict[str, GpsSnappingMatcherService21Record] = {}
        self.engine = GpsSnappingMatcherService21Engine()

    def create(self, op_index: float = 50.0) -> GpsSnappingMatcherService21Record:
        rec = GpsSnappingMatcherService21Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[GpsSnappingMatcherService21Record]:
...
def test_subsystem_gps_snapping_matcher_21() -> bool:
    mgr = GpsSnappingMatcherService21Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = GpsSnappingMatcherService21Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_gps_snapping_matcher_21()
    print(f"gps_snapping_matcher Part 21: {'PASSED' if ok else 'FAILED'}")
