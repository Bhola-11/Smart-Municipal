...
Module: Ward Service Station Voronoi Tessellation (Part 20)
App: apps.gis_engine
...
class VoronoiServicePartitionService20Engine:
    """
...
class VoronoiServicePartitionService20Workflow:
    """
    ...
    def __init__(self, record: VoronoiServicePartitionService20Record):
        self.record = record
...
class VoronoiServicePartitionService20Manager:
    """
    Service registry and data aggregator for Ward Service Station Voronoi Tessellation.
    """
    def __init__(self):
        self.store: Dict[str, VoronoiServicePartitionService20Record] = {}
        self.engine = VoronoiServicePartitionService20Engine()

    def create(self, op_index: float = 50.0) -> VoronoiServicePartitionService20Record:
        rec = VoronoiServicePartitionService20Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[VoronoiServicePartitionService20Record]:
...
def test_subsystem_voronoi_service_partition_20() -> bool:
    mgr = VoronoiServicePartitionService20Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = VoronoiServicePartitionService20Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_voronoi_service_partition_20()
    print(f"voronoi_service_partition Part 20: {'PASSED' if ok else 'FAILED'}")
