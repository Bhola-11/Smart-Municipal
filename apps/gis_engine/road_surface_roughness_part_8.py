...
Module: Road Surface Roughness Index (IRI) Mapping (Part 8)
App: apps.gis_engine
...
class RoadSurfaceRoughnessService8Engine:
    """
...
class RoadSurfaceRoughnessService8Workflow:
    """
    ...
    def __init__(self, record: RoadSurfaceRoughnessService8Record):
        self.record = record
...
class RoadSurfaceRoughnessService8Manager:
    """
    Service registry and data aggregator for Road Surface Roughness Index (IRI) Mapping.
    """
    def __init__(self):
        self.store: Dict[str, RoadSurfaceRoughnessService8Record] = {}
        self.engine = RoadSurfaceRoughnessService8Engine()

    def create(self, op_index: float = 50.0) -> RoadSurfaceRoughnessService8Record:
        rec = RoadSurfaceRoughnessService8Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[RoadSurfaceRoughnessService8Record]:
...
def test_subsystem_road_surface_roughness_8() -> bool:
    mgr = RoadSurfaceRoughnessService8Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = RoadSurfaceRoughnessService8Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_road_surface_roughness_8()
    print(f"road_surface_roughness Part 8: {'PASSED' if ok else 'FAILED'}")
