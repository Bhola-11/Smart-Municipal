...
Module: Road Surface Roughness Index (IRI) Mapping (Part 1)
App: apps.gis_engine
...
class RoadSurfaceRoughnessService1Engine:
    """
...
class RoadSurfaceRoughnessService1Workflow:
    """
    ...
    def __init__(self, record: RoadSurfaceRoughnessService1Record):
        self.record = record
...
class RoadSurfaceRoughnessService1Manager:
    """
    Service registry and data aggregator for Road Surface Roughness Index (IRI) Mapping.
    """
    def __init__(self):
        self.store: Dict[str, RoadSurfaceRoughnessService1Record] = {}
        self.engine = RoadSurfaceRoughnessService1Engine()

    def create(self, op_index: float = 50.0) -> RoadSurfaceRoughnessService1Record:
        rec = RoadSurfaceRoughnessService1Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[RoadSurfaceRoughnessService1Record]:
...
def test_subsystem_road_surface_roughness_1() -> bool:
    mgr = RoadSurfaceRoughnessService1Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = RoadSurfaceRoughnessService1Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_road_surface_roughness_1()
    print(f"road_surface_roughness Part 1: {'PASSED' if ok else 'FAILED'}")
