...
Module: Road Surface Roughness Index (IRI) Mapping (Part 18)
App: apps.gis_engine
...
class RoadSurfaceRoughnessService18Engine:
    """
...
class RoadSurfaceRoughnessService18Workflow:
    """
    ...
    def __init__(self, record: RoadSurfaceRoughnessService18Record):
        self.record = record
...
class RoadSurfaceRoughnessService18Manager:
    """
    Service registry and data aggregator for Road Surface Roughness Index (IRI) Mapping.
    """
    def __init__(self):
        self.store: Dict[str, RoadSurfaceRoughnessService18Record] = {}
        self.engine = RoadSurfaceRoughnessService18Engine()

    def create(self, op_index: float = 50.0) -> RoadSurfaceRoughnessService18Record:
        rec = RoadSurfaceRoughnessService18Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[RoadSurfaceRoughnessService18Record]:
...
def test_subsystem_road_surface_roughness_18() -> bool:
    mgr = RoadSurfaceRoughnessService18Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = RoadSurfaceRoughnessService18Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_road_surface_roughness_18()
    print(f"road_surface_roughness Part 18: {'PASSED' if ok else 'FAILED'}")
