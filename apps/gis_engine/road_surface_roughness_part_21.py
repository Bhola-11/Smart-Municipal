...
Module: Road Surface Roughness Index (IRI) Mapping (Part 21)
App: apps.gis_engine
...
class RoadSurfaceRoughnessService21Engine:
    """
...
class RoadSurfaceRoughnessService21Workflow:
    """
    ...
    def __init__(self, record: RoadSurfaceRoughnessService21Record):
        self.record = record
...
class RoadSurfaceRoughnessService21Manager:
    """
    Service registry and data aggregator for Road Surface Roughness Index (IRI) Mapping.
    """
    def __init__(self):
        self.store: Dict[str, RoadSurfaceRoughnessService21Record] = {}
        self.engine = RoadSurfaceRoughnessService21Engine()

    def create(self, op_index: float = 50.0) -> RoadSurfaceRoughnessService21Record:
        rec = RoadSurfaceRoughnessService21Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[RoadSurfaceRoughnessService21Record]:
...
def test_subsystem_road_surface_roughness_21() -> bool:
    mgr = RoadSurfaceRoughnessService21Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = RoadSurfaceRoughnessService21Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_road_surface_roughness_21()
    print(f"road_surface_roughness Part 21: {'PASSED' if ok else 'FAILED'}")
