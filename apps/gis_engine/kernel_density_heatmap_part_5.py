...
Module: Civic Incident Kernel Density Heatmap Generator (Part 5)
App: apps.gis_engine
...
class KernelDensityHeatmapService5Engine:
    """
...
class KernelDensityHeatmapService5Workflow:
    """
    ...
    def __init__(self, record: KernelDensityHeatmapService5Record):
        self.record = record
...
class KernelDensityHeatmapService5Manager:
    """
    Service registry and data aggregator for Civic Incident Kernel Density Heatmap Generator.
    """
    def __init__(self):
        self.store: Dict[str, KernelDensityHeatmapService5Record] = {}
        self.engine = KernelDensityHeatmapService5Engine()

    def create(self, op_index: float = 50.0) -> KernelDensityHeatmapService5Record:
        rec = KernelDensityHeatmapService5Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[KernelDensityHeatmapService5Record]:
...
def test_subsystem_kernel_density_heatmap_5() -> bool:
    mgr = KernelDensityHeatmapService5Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = KernelDensityHeatmapService5Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_kernel_density_heatmap_5()
    print(f"kernel_density_heatmap Part 5: {'PASSED' if ok else 'FAILED'}")
