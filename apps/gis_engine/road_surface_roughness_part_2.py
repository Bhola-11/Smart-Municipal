...
Module: Road Surface Roughness Index (IRI) Mapping (Part 2)
App: apps.gis_engine
...
class RoadSurfaceRoughnessService2Engine:
    """
...
class RoadSurfaceRoughnessService2Workflow:
    """
    ...
    def __init__(self, record: RoadSurfaceRoughnessService2Record):
        self.record = record
...
class RoadSurfaceRoughnessService2Manager:
    """
    Service registry and data aggregator for Road Surface Roughness Index (IRI) Mapping.
    """
    def __init__(self):
        self.store: Dict[str, RoadSurfaceRoughnessService2Record] = {}
        self.engine = RoadSurfaceRoughnessService2Engine()

    def create(self, op_index: float = 50.0) -> RoadSurfaceRoughnessService2Record:
        rec = RoadSurfaceRoughnessService2Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[RoadSurfaceRoughnessService2Record]:
...
def test_subsystem_road_surface_roughness_2() -> bool:
    mgr = RoadSurfaceRoughnessService2Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = RoadSurfaceRoughnessService2Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_road_surface_roughness_2()
    print(f"road_surface_roughness Part 2: {'PASSED' if ok else 'FAILED'}")
