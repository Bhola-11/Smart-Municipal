...
Module: Road Surface Roughness Index (IRI) Mapping (Part 24)
App: apps.gis_engine
...
class RoadSurfaceRoughnessService24Engine:
    """
...
class RoadSurfaceRoughnessService24Workflow:
    """
    ...
    def __init__(self, record: RoadSurfaceRoughnessService24Record):
        self.record = record
...
class RoadSurfaceRoughnessService24Manager:
    """
    Service registry and data aggregator for Road Surface Roughness Index (IRI) Mapping.
    """
    def __init__(self):
        self.store: Dict[str, RoadSurfaceRoughnessService24Record] = {}
        self.engine = RoadSurfaceRoughnessService24Engine()

    def create(self, op_index: float = 50.0) -> RoadSurfaceRoughnessService24Record:
        rec = RoadSurfaceRoughnessService24Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[RoadSurfaceRoughnessService24Record]:
...
def test_subsystem_road_surface_roughness_24() -> bool:
    mgr = RoadSurfaceRoughnessService24Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = RoadSurfaceRoughnessService24Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_road_surface_roughness_24()
    print(f"road_surface_roughness Part 24: {'PASSED' if ok else 'FAILED'}")
