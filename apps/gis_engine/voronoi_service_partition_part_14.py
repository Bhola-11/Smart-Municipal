...
Module: Ward Service Station Voronoi Tessellation (Part 14)
App: apps.gis_engine
...
class VoronoiServicePartitionService14Engine:
    """
...
class VoronoiServicePartitionService14Workflow:
    """
    ...
    def __init__(self, record: VoronoiServicePartitionService14Record):
        self.record = record
...
class VoronoiServicePartitionService14Manager:
    """
    Service registry and data aggregator for Ward Service Station Voronoi Tessellation.
    """
    def __init__(self):
        self.store: Dict[str, VoronoiServicePartitionService14Record] = {}
        self.engine = VoronoiServicePartitionService14Engine()

    def create(self, op_index: float = 50.0) -> VoronoiServicePartitionService14Record:
        rec = VoronoiServicePartitionService14Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[VoronoiServicePartitionService14Record]:
...
def test_subsystem_voronoi_service_partition_14() -> bool:
    mgr = VoronoiServicePartitionService14Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = VoronoiServicePartitionService14Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_voronoi_service_partition_14()
    print(f"voronoi_service_partition Part 14: {'PASSED' if ok else 'FAILED'}")
