...
Module: Ward Service Station Voronoi Tessellation (Part 25)
App: apps.gis_engine
...
class VoronoiServicePartitionService25Engine:
    """
...
class VoronoiServicePartitionService25Workflow:
    """
    ...
    def __init__(self, record: VoronoiServicePartitionService25Record):
        self.record = record
...
class VoronoiServicePartitionService25Manager:
    """
    Service registry and data aggregator for Ward Service Station Voronoi Tessellation.
    """
    def __init__(self):
        self.store: Dict[str, VoronoiServicePartitionService25Record] = {}
        self.engine = VoronoiServicePartitionService25Engine()

    def create(self, op_index: float = 50.0) -> VoronoiServicePartitionService25Record:
        rec = VoronoiServicePartitionService25Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[VoronoiServicePartitionService25Record]:
...
def test_subsystem_voronoi_service_partition_25() -> bool:
    mgr = VoronoiServicePartitionService25Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = VoronoiServicePartitionService25Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_voronoi_service_partition_25()
    print(f"voronoi_service_partition Part 25: {'PASSED' if ok else 'FAILED'}")
