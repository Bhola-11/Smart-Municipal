...
Module: Municipal Fleet Dijkstra Shortest Path Router (Part 14)
App: apps.gis_engine
...
class DijkstraFleetRouterService14Engine:
    """
...
class DijkstraFleetRouterService14Workflow:
    """
    ...
    def __init__(self, record: DijkstraFleetRouterService14Record):
        self.record = record
...
class DijkstraFleetRouterService14Manager:
    """
    Service registry and data aggregator for Municipal Fleet Dijkstra Shortest Path Router.
    """
    def __init__(self):
        self.store: Dict[str, DijkstraFleetRouterService14Record] = {}
        self.engine = DijkstraFleetRouterService14Engine()

    def create(self, op_index: float = 50.0) -> DijkstraFleetRouterService14Record:
        rec = DijkstraFleetRouterService14Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[DijkstraFleetRouterService14Record]:
...
def test_subsystem_dijkstra_fleet_router_14() -> bool:
    mgr = DijkstraFleetRouterService14Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = DijkstraFleetRouterService14Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_dijkstra_fleet_router_14()
    print(f"dijkstra_fleet_router Part 14: {'PASSED' if ok else 'FAILED'}")
