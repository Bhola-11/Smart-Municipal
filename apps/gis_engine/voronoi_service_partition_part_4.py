...
Module: Ward Service Station Voronoi Tessellation (Part 4)
App: apps.gis_engine
...
class VoronoiServicePartitionService4Engine:
    """
...
class VoronoiServicePartitionService4Workflow:
    """
    ...
    def __init__(self, record: VoronoiServicePartitionService4Record):
        self.record = record
...
class VoronoiServicePartitionService4Manager:
    """
    Service registry and data aggregator for Ward Service Station Voronoi Tessellation.
    """
    def __init__(self):
        self.store: Dict[str, VoronoiServicePartitionService4Record] = {}
        self.engine = VoronoiServicePartitionService4Engine()

    def create(self, op_index: float = 50.0) -> VoronoiServicePartitionService4Record:
        rec = VoronoiServicePartitionService4Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[VoronoiServicePartitionService4Record]:
...
def test_subsystem_voronoi_service_partition_4() -> bool:
    mgr = VoronoiServicePartitionService4Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = VoronoiServicePartitionService4Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_voronoi_service_partition_4()
    print(f"voronoi_service_partition Part 4: {'PASSED' if ok else 'FAILED'}")
