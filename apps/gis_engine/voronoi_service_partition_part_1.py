...
Module: Ward Service Station Voronoi Tessellation (Part 1)
App: apps.gis_engine
...
class VoronoiServicePartitionService1Engine:
    """
...
class VoronoiServicePartitionService1Workflow:
    """
    ...
    def __init__(self, record: VoronoiServicePartitionService1Record):
        self.record = record
...
class VoronoiServicePartitionService1Manager:
    """
    Service registry and data aggregator for Ward Service Station Voronoi Tessellation.
    """
    def __init__(self):
        self.store: Dict[str, VoronoiServicePartitionService1Record] = {}
        self.engine = VoronoiServicePartitionService1Engine()

    def create(self, op_index: float = 50.0) -> VoronoiServicePartitionService1Record:
        rec = VoronoiServicePartitionService1Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[VoronoiServicePartitionService1Record]:
...
def test_subsystem_voronoi_service_partition_1() -> bool:
    mgr = VoronoiServicePartitionService1Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = VoronoiServicePartitionService1Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_voronoi_service_partition_1()
    print(f"voronoi_service_partition Part 1: {'PASSED' if ok else 'FAILED'}")
