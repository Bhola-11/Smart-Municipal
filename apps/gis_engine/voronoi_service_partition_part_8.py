...
Module: Ward Service Station Voronoi Tessellation (Part 8)
App: apps.gis_engine
...
class VoronoiServicePartitionService8Engine:
    """
...
class VoronoiServicePartitionService8Workflow:
    """
    ...
    def __init__(self, record: VoronoiServicePartitionService8Record):
        self.record = record
...
class VoronoiServicePartitionService8Manager:
    """
    Service registry and data aggregator for Ward Service Station Voronoi Tessellation.
    """
    def __init__(self):
        self.store: Dict[str, VoronoiServicePartitionService8Record] = {}
        self.engine = VoronoiServicePartitionService8Engine()

    def create(self, op_index: float = 50.0) -> VoronoiServicePartitionService8Record:
        rec = VoronoiServicePartitionService8Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[VoronoiServicePartitionService8Record]:
...
def test_subsystem_voronoi_service_partition_8() -> bool:
    mgr = VoronoiServicePartitionService8Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = VoronoiServicePartitionService8Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_voronoi_service_partition_8()
    print(f"voronoi_service_partition Part 8: {'PASSED' if ok else 'FAILED'}")
