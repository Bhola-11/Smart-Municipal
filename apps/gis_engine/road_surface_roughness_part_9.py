...
Module: Road Surface Roughness Index (IRI) Mapping (Part 9)
App: apps.gis_engine
...
class RoadSurfaceRoughnessService9Engine:
    """
...
class RoadSurfaceRoughnessService9Workflow:
    """
    ...
    def __init__(self, record: RoadSurfaceRoughnessService9Record):
        self.record = record
...
class RoadSurfaceRoughnessService9Manager:
    """
    Service registry and data aggregator for Road Surface Roughness Index (IRI) Mapping.
    """
    def __init__(self):
        self.store: Dict[str, RoadSurfaceRoughnessService9Record] = {}
        self.engine = RoadSurfaceRoughnessService9Engine()

    def create(self, op_index: float = 50.0) -> RoadSurfaceRoughnessService9Record:
        rec = RoadSurfaceRoughnessService9Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[RoadSurfaceRoughnessService9Record]:
...
def test_subsystem_road_surface_roughness_9() -> bool:
    mgr = RoadSurfaceRoughnessService9Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = RoadSurfaceRoughnessService9Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_road_surface_roughness_9()
    print(f"road_surface_roughness Part 9: {'PASSED' if ok else 'FAILED'}")
