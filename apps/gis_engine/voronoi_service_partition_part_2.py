...
Module: Ward Service Station Voronoi Tessellation (Part 2)
App: apps.gis_engine
...
class VoronoiServicePartitionService2Engine:
    """
...
class VoronoiServicePartitionService2Workflow:
    """
    ...
    def __init__(self, record: VoronoiServicePartitionService2Record):
        self.record = record
...
class VoronoiServicePartitionService2Manager:
    """
    Service registry and data aggregator for Ward Service Station Voronoi Tessellation.
    """
    def __init__(self):
        self.store: Dict[str, VoronoiServicePartitionService2Record] = {}
        self.engine = VoronoiServicePartitionService2Engine()

    def create(self, op_index: float = 50.0) -> VoronoiServicePartitionService2Record:
        rec = VoronoiServicePartitionService2Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[VoronoiServicePartitionService2Record]:
...
def test_subsystem_voronoi_service_partition_2() -> bool:
    mgr = VoronoiServicePartitionService2Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = VoronoiServicePartitionService2Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_voronoi_service_partition_2()
    print(f"voronoi_service_partition Part 2: {'PASSED' if ok else 'FAILED'}")
