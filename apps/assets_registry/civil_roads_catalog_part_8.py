...
Module: Road Network, Bridges & Flyover Asset Registry (Part 8)
...
class CivilRoadsCatalogService8Engine:
    """
...
class CivilRoadsCatalogService8Workflow:
    """
    ...
    def __init__(self, record: CivilRoadsCatalogService8Record):
        self.record = record
...
class CivilRoadsCatalogService8Manager:
    """
    Service registry and data aggregator for Road Network, Bridges & Flyover Asset Registry.
    """
    def __init__(self):
        self.store: Dict[str, CivilRoadsCatalogService8Record] = {}
        self.engine = CivilRoadsCatalogService8Engine()

    def create(self, op_index: float = 50.0) -> CivilRoadsCatalogService8Record:
        rec = CivilRoadsCatalogService8Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[CivilRoadsCatalogService8Record]:
...
def test_subsystem_civil_roads_catalog_8() -> bool:
    mgr = CivilRoadsCatalogService8Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = CivilRoadsCatalogService8Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_civil_roads_catalog_8()
    print(f"civil_roads_catalog Part 8: {'PASSED' if ok else 'FAILED'}")
