...
Module: Public Parks, Gardens & Tree Canopy Registry (Part 9)
...
class HorticultureParksCatalogService9Engine:
    """
...
class HorticultureParksCatalogService9Workflow:
    """
    ...
    def __init__(self, record: HorticultureParksCatalogService9Record):
        self.record = record
...
class HorticultureParksCatalogService9Manager:
    """
    Service registry and data aggregator for Public Parks, Gardens & Tree Canopy Registry.
    """
    def __init__(self):
        self.store: Dict[str, HorticultureParksCatalogService9Record] = {}
        self.engine = HorticultureParksCatalogService9Engine()

    def create(self, op_index: float = 50.0) -> HorticultureParksCatalogService9Record:
        rec = HorticultureParksCatalogService9Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[HorticultureParksCatalogService9Record]:
...
def test_subsystem_horticulture_parks_catalog_9() -> bool:
    mgr = HorticultureParksCatalogService9Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = HorticultureParksCatalogService9Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_horticulture_parks_catalog_9()
    print(f"horticulture_parks_catalog Part 9: {'PASSED' if ok else 'FAILED'}")
