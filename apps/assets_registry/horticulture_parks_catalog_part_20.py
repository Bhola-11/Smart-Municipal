...
Module: Public Parks, Gardens & Tree Canopy Registry (Part 20)
...
@dataclass
class HorticultureParksCatalogService20Record:
    """Certified Municipal Domain Record for Public Parks, Gardens & Tree Canopy Registry."""
    record_id: str = field(default_factory=lambda: str(uuid.uuid4()))
    entity_code: str = "horticulture_parks_catalog_20"
...
class HorticultureParksCatalogService20Engine:
    """
...
class HorticultureParksCatalogService20Workflow:
    """
    ...
    def __init__(self, record: HorticultureParksCatalogService20Record):
        self.record = record
...
class HorticultureParksCatalogService20Manager:
    """
    Service registry and data aggregator for Public Parks, Gardens & Tree Canopy Registry.
    """
    def __init__(self):
        self.store: Dict[str, HorticultureParksCatalogService20Record] = {}
        self.engine = HorticultureParksCatalogService20Engine()

    def create(self, op_index: float = 50.0) -> HorticultureParksCatalogService20Record:
        rec = HorticultureParksCatalogService20Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[HorticultureParksCatalogService20Record]:
...
def test_subsystem_horticulture_parks_catalog_20() -> bool:
    mgr = HorticultureParksCatalogService20Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = HorticultureParksCatalogService20Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_horticulture_parks_catalog_20()
    print(f"horticulture_parks_catalog Part 20: {'PASSED' if ok else 'FAILED'}")
