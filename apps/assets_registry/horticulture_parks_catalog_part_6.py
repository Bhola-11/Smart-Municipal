...
Module: Public Parks, Gardens & Tree Canopy Registry (Part 6)
...
class HorticultureParksCatalogService6Engine:
    """
...
class HorticultureParksCatalogService6Workflow:
    """
    ...
    def __init__(self, record: HorticultureParksCatalogService6Record):
        self.record = record
...
class HorticultureParksCatalogService6Manager:
    """
    Service registry and data aggregator for Public Parks, Gardens & Tree Canopy Registry.
    """
    def __init__(self):
        self.store: Dict[str, HorticultureParksCatalogService6Record] = {}
        self.engine = HorticultureParksCatalogService6Engine()

    def create(self, op_index: float = 50.0) -> HorticultureParksCatalogService6Record:
        rec = HorticultureParksCatalogService6Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[HorticultureParksCatalogService6Record]:
...
def test_subsystem_horticulture_parks_catalog_6() -> bool:
    mgr = HorticultureParksCatalogService6Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = HorticultureParksCatalogService6Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_horticulture_parks_catalog_6()
    print(f"horticulture_parks_catalog Part 6: {'PASSED' if ok else 'FAILED'}")
