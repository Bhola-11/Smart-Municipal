...
Module: Road Network, Bridges & Flyover Asset Registry (Part 25)
...
class CivilRoadsCatalogService25Engine:
    """
...
class CivilRoadsCatalogService25Workflow:
    """
    ...
    def __init__(self, record: CivilRoadsCatalogService25Record):
        self.record = record
...
class CivilRoadsCatalogService25Manager:
    """
    Service registry and data aggregator for Road Network, Bridges & Flyover Asset Registry.
    """
    def __init__(self):
        self.store: Dict[str, CivilRoadsCatalogService25Record] = {}
        self.engine = CivilRoadsCatalogService25Engine()

    def create(self, op_index: float = 50.0) -> CivilRoadsCatalogService25Record:
        rec = CivilRoadsCatalogService25Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[CivilRoadsCatalogService25Record]:
...
def test_subsystem_civil_roads_catalog_25() -> bool:
    mgr = CivilRoadsCatalogService25Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = CivilRoadsCatalogService25Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_civil_roads_catalog_25()
    print(f"civil_roads_catalog Part 25: {'PASSED' if ok else 'FAILED'}")
