...
Module: Public Parks, Gardens & Tree Canopy Registry (Part 4)
...
class HorticultureParksCatalogService4Engine:
    """
...
class HorticultureParksCatalogService4Workflow:
    """
    ...
    def __init__(self, record: HorticultureParksCatalogService4Record):
        self.record = record
...
class HorticultureParksCatalogService4Manager:
    """
    Service registry and data aggregator for Public Parks, Gardens & Tree Canopy Registry.
    """
    def __init__(self):
        self.store: Dict[str, HorticultureParksCatalogService4Record] = {}
        self.engine = HorticultureParksCatalogService4Engine()

    def create(self, op_index: float = 50.0) -> HorticultureParksCatalogService4Record:
        rec = HorticultureParksCatalogService4Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[HorticultureParksCatalogService4Record]:
...
def test_subsystem_horticulture_parks_catalog_4() -> bool:
    mgr = HorticultureParksCatalogService4Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = HorticultureParksCatalogService4Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_horticulture_parks_catalog_4()
    print(f"horticulture_parks_catalog Part 4: {'PASSED' if ok else 'FAILED'}")
