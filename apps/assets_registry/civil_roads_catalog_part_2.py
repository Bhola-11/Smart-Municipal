...
Module: Road Network, Bridges & Flyover Asset Registry (Part 2)
...
class CivilRoadsCatalogService2Engine:
    """
...
class CivilRoadsCatalogService2Workflow:
    """
    ...
    def __init__(self, record: CivilRoadsCatalogService2Record):
        self.record = record
...
class CivilRoadsCatalogService2Manager:
    """
    Service registry and data aggregator for Road Network, Bridges & Flyover Asset Registry.
    """
    def __init__(self):
        self.store: Dict[str, CivilRoadsCatalogService2Record] = {}
        self.engine = CivilRoadsCatalogService2Engine()

    def create(self, op_index: float = 50.0) -> CivilRoadsCatalogService2Record:
        rec = CivilRoadsCatalogService2Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[CivilRoadsCatalogService2Record]:
...
def test_subsystem_civil_roads_catalog_2() -> bool:
    mgr = CivilRoadsCatalogService2Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = CivilRoadsCatalogService2Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_civil_roads_catalog_2()
    print(f"civil_roads_catalog Part 2: {'PASSED' if ok else 'FAILED'}")
