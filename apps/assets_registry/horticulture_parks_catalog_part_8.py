...
Module: Public Parks, Gardens & Tree Canopy Registry (Part 8)
...
class HorticultureParksCatalogService8Engine:
    """
...
class HorticultureParksCatalogService8Workflow:
    """
    ...
    def __init__(self, record: HorticultureParksCatalogService8Record):
        self.record = record
...
class HorticultureParksCatalogService8Manager:
    """
    Service registry and data aggregator for Public Parks, Gardens & Tree Canopy Registry.
    """
    def __init__(self):
        self.store: Dict[str, HorticultureParksCatalogService8Record] = {}
        self.engine = HorticultureParksCatalogService8Engine()

    def create(self, op_index: float = 50.0) -> HorticultureParksCatalogService8Record:
        rec = HorticultureParksCatalogService8Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[HorticultureParksCatalogService8Record]:
...
def test_subsystem_horticulture_parks_catalog_8() -> bool:
    mgr = HorticultureParksCatalogService8Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = HorticultureParksCatalogService8Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_horticulture_parks_catalog_8()
    print(f"horticulture_parks_catalog Part 8: {'PASSED' if ok else 'FAILED'}")
