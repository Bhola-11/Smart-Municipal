...
Module: Transformers, Substations & Poles Registry (Part 4)
...
class ElectricalGridCatalogService4Engine:
    """
...
class ElectricalGridCatalogService4Workflow:
    """
    ...
    def __init__(self, record: ElectricalGridCatalogService4Record):
        self.record = record
...
class ElectricalGridCatalogService4Manager:
    """
    Service registry and data aggregator for Transformers, Substations & Poles Registry.
    """
    def __init__(self):
        self.store: Dict[str, ElectricalGridCatalogService4Record] = {}
        self.engine = ElectricalGridCatalogService4Engine()

    def create(self, op_index: float = 50.0) -> ElectricalGridCatalogService4Record:
        rec = ElectricalGridCatalogService4Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[ElectricalGridCatalogService4Record]:
...
def test_subsystem_electrical_grid_catalog_4() -> bool:
    mgr = ElectricalGridCatalogService4Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = ElectricalGridCatalogService4Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_electrical_grid_catalog_4()
    print(f"electrical_grid_catalog Part 4: {'PASSED' if ok else 'FAILED'}")
