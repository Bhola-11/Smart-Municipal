...
Module: Municipal Primary & Secondary Schools Registry (Part 14)
...
class EducationSchoolsCatalogService14Engine:
    """
...
class EducationSchoolsCatalogService14Workflow:
    """
    ...
    def __init__(self, record: EducationSchoolsCatalogService14Record):
        self.record = record
...
class EducationSchoolsCatalogService14Manager:
    """
    Service registry and data aggregator for Municipal Primary & Secondary Schools Registry.
    """
    def __init__(self):
        self.store: Dict[str, EducationSchoolsCatalogService14Record] = {}
        self.engine = EducationSchoolsCatalogService14Engine()

    def create(self, op_index: float = 50.0) -> EducationSchoolsCatalogService14Record:
        rec = EducationSchoolsCatalogService14Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[EducationSchoolsCatalogService14Record]:
...
def test_subsystem_education_schools_catalog_14() -> bool:
    mgr = EducationSchoolsCatalogService14Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = EducationSchoolsCatalogService14Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_education_schools_catalog_14()
    print(f"education_schools_catalog Part 14: {'PASSED' if ok else 'FAILED'}")
