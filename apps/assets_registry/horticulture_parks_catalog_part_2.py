...
Module: Public Parks, Gardens & Tree Canopy Registry (Part 2)
...
class HorticultureParksCatalogService2Engine:
    """
...
class HorticultureParksCatalogService2Workflow:
    """
    ...
    def __init__(self, record: HorticultureParksCatalogService2Record):
        self.record = record
...
class HorticultureParksCatalogService2Manager:
    """
    Service registry and data aggregator for Public Parks, Gardens & Tree Canopy Registry.
    """
    def __init__(self):
        self.store: Dict[str, HorticultureParksCatalogService2Record] = {}
        self.engine = HorticultureParksCatalogService2Engine()

    def create(self, op_index: float = 50.0) -> HorticultureParksCatalogService2Record:
        rec = HorticultureParksCatalogService2Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[HorticultureParksCatalogService2Record]:
...
def test_subsystem_horticulture_parks_catalog_2() -> bool:
    mgr = HorticultureParksCatalogService2Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = HorticultureParksCatalogService2Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_horticulture_parks_catalog_2()
    print(f"horticulture_parks_catalog Part 2: {'PASSED' if ok else 'FAILED'}")
