...
Module: Preventative Maintenance & Inspection Schedules (Part 14)
...
class PreventativeMaintenanceService14Engine:
    """
...
class PreventativeMaintenanceService14Workflow:
    """
    ...
    def __init__(self, record: PreventativeMaintenanceService14Record):
        self.record = record
...
class PreventativeMaintenanceService14Manager:
    """
    Service registry and data aggregator for Preventative Maintenance & Inspection Schedules.
    """
    def __init__(self):
        self.store: Dict[str, PreventativeMaintenanceService14Record] = {}
        self.engine = PreventativeMaintenanceService14Engine()

    def create(self, op_index: float = 50.0) -> PreventativeMaintenanceService14Record:
        rec = PreventativeMaintenanceService14Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[PreventativeMaintenanceService14Record]:
...
def test_subsystem_preventative_maintenance_14() -> bool:
    mgr = PreventativeMaintenanceService14Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = PreventativeMaintenanceService14Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_preventative_maintenance_14()
    print(f"preventative_maintenance Part 14: {'PASSED' if ok else 'FAILED'}")
