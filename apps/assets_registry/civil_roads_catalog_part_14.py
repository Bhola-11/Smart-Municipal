...
Module: Road Network, Bridges & Flyover Asset Registry (Part 14)
...
class CivilRoadsCatalogService14Engine:
    """
...
class CivilRoadsCatalogService14Workflow:
    """
    ...
    def __init__(self, record: CivilRoadsCatalogService14Record):
        self.record = record
...
class CivilRoadsCatalogService14Manager:
    """
    Service registry and data aggregator for Road Network, Bridges & Flyover Asset Registry.
    """
    def __init__(self):
        self.store: Dict[str, CivilRoadsCatalogService14Record] = {}
        self.engine = CivilRoadsCatalogService14Engine()

    def create(self, op_index: float = 50.0) -> CivilRoadsCatalogService14Record:
        rec = CivilRoadsCatalogService14Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[CivilRoadsCatalogService14Record]:
...
def test_subsystem_civil_roads_catalog_14() -> bool:
    mgr = CivilRoadsCatalogService14Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = CivilRoadsCatalogService14Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_civil_roads_catalog_14()
    print(f"civil_roads_catalog Part 14: {'PASSED' if ok else 'FAILED'}")
