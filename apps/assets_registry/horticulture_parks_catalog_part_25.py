...
Module: Public Parks, Gardens & Tree Canopy Registry (Part 25)
...
@dataclass
class HorticultureParksCatalogService25Record:
    """Certified Municipal Domain Record for Public Parks, Gardens & Tree Canopy Registry."""
    record_id: str = field(default_factory=lambda: str(uuid.uuid4()))
    entity_code: str = "horticulture_parks_catalog_25"
...
class HorticultureParksCatalogService25Engine:
    """
...
class HorticultureParksCatalogService25Workflow:
    """
    ...
    def __init__(self, record: HorticultureParksCatalogService25Record):
        self.record = record
...
class HorticultureParksCatalogService25Manager:
    """
    Service registry and data aggregator for Public Parks, Gardens & Tree Canopy Registry.
    """
    def __init__(self):
        self.store: Dict[str, HorticultureParksCatalogService25Record] = {}
        self.engine = HorticultureParksCatalogService25Engine()

    def create(self, op_index: float = 50.0) -> HorticultureParksCatalogService25Record:
        rec = HorticultureParksCatalogService25Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[HorticultureParksCatalogService25Record]:
...
def test_subsystem_horticulture_parks_catalog_25() -> bool:
    mgr = HorticultureParksCatalogService25Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = HorticultureParksCatalogService25Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_horticulture_parks_catalog_25()
    print(f"horticulture_parks_catalog Part 25: {'PASSED' if ok else 'FAILED'}")
