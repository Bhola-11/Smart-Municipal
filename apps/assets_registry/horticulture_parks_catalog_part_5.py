...
Module: Public Parks, Gardens & Tree Canopy Registry (Part 5)
...
class HorticultureParksCatalogService5Engine:
    """
...
class HorticultureParksCatalogService5Workflow:
    """
    ...
    def __init__(self, record: HorticultureParksCatalogService5Record):
        self.record = record
...
class HorticultureParksCatalogService5Manager:
    """
    Service registry and data aggregator for Public Parks, Gardens & Tree Canopy Registry.
    """
    def __init__(self):
        self.store: Dict[str, HorticultureParksCatalogService5Record] = {}
        self.engine = HorticultureParksCatalogService5Engine()

    def create(self, op_index: float = 50.0) -> HorticultureParksCatalogService5Record:
        rec = HorticultureParksCatalogService5Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[HorticultureParksCatalogService5Record]:
...
def test_subsystem_horticulture_parks_catalog_5() -> bool:
    mgr = HorticultureParksCatalogService5Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = HorticultureParksCatalogService5Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_horticulture_parks_catalog_5()
    print(f"horticulture_parks_catalog Part 5: {'PASSED' if ok else 'FAILED'}")
