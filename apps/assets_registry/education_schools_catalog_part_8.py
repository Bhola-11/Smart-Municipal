...
Module: Municipal Primary & Secondary Schools Registry (Part 8)
...
class EducationSchoolsCatalogService8Engine:
    """
...
class EducationSchoolsCatalogService8Workflow:
    """
    ...
    def __init__(self, record: EducationSchoolsCatalogService8Record):
        self.record = record
...
class EducationSchoolsCatalogService8Manager:
    """
    Service registry and data aggregator for Municipal Primary & Secondary Schools Registry.
    """
    def __init__(self):
        self.store: Dict[str, EducationSchoolsCatalogService8Record] = {}
        self.engine = EducationSchoolsCatalogService8Engine()

    def create(self, op_index: float = 50.0) -> EducationSchoolsCatalogService8Record:
        rec = EducationSchoolsCatalogService8Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[EducationSchoolsCatalogService8Record]:
...
def test_subsystem_education_schools_catalog_8() -> bool:
    mgr = EducationSchoolsCatalogService8Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = EducationSchoolsCatalogService8Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_education_schools_catalog_8()
    print(f"education_schools_catalog Part 8: {'PASSED' if ok else 'FAILED'}")
