...
Module: Public Parks, Gardens & Tree Canopy Registry (Part 3)
...
class HorticultureParksCatalogService3Engine:
    """
...
class HorticultureParksCatalogService3Workflow:
    """
    ...
    def __init__(self, record: HorticultureParksCatalogService3Record):
        self.record = record
...
class HorticultureParksCatalogService3Manager:
    """
    Service registry and data aggregator for Public Parks, Gardens & Tree Canopy Registry.
    """
    def __init__(self):
        self.store: Dict[str, HorticultureParksCatalogService3Record] = {}
        self.engine = HorticultureParksCatalogService3Engine()

    def create(self, op_index: float = 50.0) -> HorticultureParksCatalogService3Record:
        rec = HorticultureParksCatalogService3Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[HorticultureParksCatalogService3Record]:
...
def test_subsystem_horticulture_parks_catalog_3() -> bool:
    mgr = HorticultureParksCatalogService3Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = HorticultureParksCatalogService3Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_horticulture_parks_catalog_3()
    print(f"horticulture_parks_catalog Part 3: {'PASSED' if ok else 'FAILED'}")
