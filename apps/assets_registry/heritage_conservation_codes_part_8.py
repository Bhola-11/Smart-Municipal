...
Module: Urban Heritage Building Conservation Codes (Part 8)
...
class HeritageConservationCodesService8Engine:
    """
...
class HeritageConservationCodesService8Workflow:
    """
    ...
    def __init__(self, record: HeritageConservationCodesService8Record):
        self.record = record
...
class HeritageConservationCodesService8Manager:
    """
    Service registry and data aggregator for Urban Heritage Building Conservation Codes.
    """
    def __init__(self):
        self.store: Dict[str, HeritageConservationCodesService8Record] = {}
        self.engine = HeritageConservationCodesService8Engine()

    def create(self, op_index: float = 50.0) -> HeritageConservationCodesService8Record:
        rec = HeritageConservationCodesService8Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[HeritageConservationCodesService8Record]:
...
def test_subsystem_heritage_conservation_codes_8() -> bool:
    mgr = HeritageConservationCodesService8Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = HeritageConservationCodesService8Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_heritage_conservation_codes_8()
    print(f"heritage_conservation_codes Part 8: {'PASSED' if ok else 'FAILED'}")
