...
Module: Public Parks, Gardens & Tree Canopy Registry (Part 27)
...
@dataclass
class HorticultureParksCatalogService27Record:
    """Certified Municipal Domain Record for Public Parks, Gardens & Tree Canopy Registry."""
    record_id: str = field(default_factory=lambda: str(uuid.uuid4()))
    entity_code: str = "horticulture_parks_catalog_27"
...
class HorticultureParksCatalogService27Engine:
    """
...
class HorticultureParksCatalogService27Workflow:
    """
    ...
    def __init__(self, record: HorticultureParksCatalogService27Record):
        self.record = record
...
class HorticultureParksCatalogService27Manager:
    """
    Service registry and data aggregator for Public Parks, Gardens & Tree Canopy Registry.
    """
    def __init__(self):
        self.store: Dict[str, HorticultureParksCatalogService27Record] = {}
        self.engine = HorticultureParksCatalogService27Engine()

    def create(self, op_index: float = 50.0) -> HorticultureParksCatalogService27Record:
        rec = HorticultureParksCatalogService27Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[HorticultureParksCatalogService27Record]:
...
def test_subsystem_horticulture_parks_catalog_27() -> bool:
    mgr = HorticultureParksCatalogService27Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = HorticultureParksCatalogService27Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_horticulture_parks_catalog_27()
    print(f"horticulture_parks_catalog Part 27: {'PASSED' if ok else 'FAILED'}")
