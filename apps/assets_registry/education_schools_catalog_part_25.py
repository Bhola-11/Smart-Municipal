...
Module: Municipal Primary & Secondary Schools Registry (Part 25)
...
class EducationSchoolsCatalogService25Engine:
    """
...
class EducationSchoolsCatalogService25Workflow:
    """
    ...
    def __init__(self, record: EducationSchoolsCatalogService25Record):
        self.record = record
...
class EducationSchoolsCatalogService25Manager:
    """
    Service registry and data aggregator for Municipal Primary & Secondary Schools Registry.
    """
    def __init__(self):
        self.store: Dict[str, EducationSchoolsCatalogService25Record] = {}
        self.engine = EducationSchoolsCatalogService25Engine()

    def create(self, op_index: float = 50.0) -> EducationSchoolsCatalogService25Record:
        rec = EducationSchoolsCatalogService25Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[EducationSchoolsCatalogService25Record]:
...
def test_subsystem_education_schools_catalog_25() -> bool:
    mgr = EducationSchoolsCatalogService25Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = EducationSchoolsCatalogService25Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_education_schools_catalog_25()
    print(f"education_schools_catalog Part 25: {'PASSED' if ok else 'FAILED'}")
