...
Module: National Clean Air Programme (NCAP) Compliance (Part 14)
...
class AirQualityRegulationsService14Engine:
    """
...
class AirQualityRegulationsService14Workflow:
    """
    ...
    def __init__(self, record: AirQualityRegulationsService14Record):
        self.record = record
...
class AirQualityRegulationsService14Manager:
    """
    Service registry and data aggregator for National Clean Air Programme (NCAP) Compliance.
    """
    def __init__(self):
        self.store: Dict[str, AirQualityRegulationsService14Record] = {}
        self.engine = AirQualityRegulationsService14Engine()

    def create(self, op_index: float = 50.0) -> AirQualityRegulationsService14Record:
        rec = AirQualityRegulationsService14Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[AirQualityRegulationsService14Record]:
...
def test_subsystem_air_quality_regulations_14() -> bool:
    mgr = AirQualityRegulationsService14Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = AirQualityRegulationsService14Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_air_quality_regulations_14()
    print(f"air_quality_regulations Part 14: {'PASSED' if ok else 'FAILED'}")
