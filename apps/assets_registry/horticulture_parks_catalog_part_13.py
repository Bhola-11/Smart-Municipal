...
Module: Public Parks, Gardens & Tree Canopy Registry (Part 13)
...
@dataclass
class HorticultureParksCatalogService13Record:
    """Certified Municipal Domain Record for Public Parks, Gardens & Tree Canopy Registry."""
    record_id: str = field(default_factory=lambda: str(uuid.uuid4()))
    entity_code: str = "horticulture_parks_catalog_13"
...
class HorticultureParksCatalogService13Engine:
    """
...
class HorticultureParksCatalogService13Workflow:
    """
    ...
    def __init__(self, record: HorticultureParksCatalogService13Record):
        self.record = record
...
class HorticultureParksCatalogService13Manager:
    """
    Service registry and data aggregator for Public Parks, Gardens & Tree Canopy Registry.
    """
    def __init__(self):
        self.store: Dict[str, HorticultureParksCatalogService13Record] = {}
        self.engine = HorticultureParksCatalogService13Engine()

    def create(self, op_index: float = 50.0) -> HorticultureParksCatalogService13Record:
        rec = HorticultureParksCatalogService13Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[HorticultureParksCatalogService13Record]:
...
def test_subsystem_horticulture_parks_catalog_13() -> bool:
    mgr = HorticultureParksCatalogService13Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = HorticultureParksCatalogService13Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_horticulture_parks_catalog_13()
    print(f"horticulture_parks_catalog Part 13: {'PASSED' if ok else 'FAILED'}")
