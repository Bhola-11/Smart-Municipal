...
Module: Transformers, Substations & Poles Registry (Part 8)
...
class ElectricalGridCatalogService8Engine:
    """
...
class ElectricalGridCatalogService8Workflow:
    """
    ...
    def __init__(self, record: ElectricalGridCatalogService8Record):
        self.record = record
...
class ElectricalGridCatalogService8Manager:
    """
    Service registry and data aggregator for Transformers, Substations & Poles Registry.
    """
    def __init__(self):
        self.store: Dict[str, ElectricalGridCatalogService8Record] = {}
        self.engine = ElectricalGridCatalogService8Engine()

    def create(self, op_index: float = 50.0) -> ElectricalGridCatalogService8Record:
        rec = ElectricalGridCatalogService8Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[ElectricalGridCatalogService8Record]:
...
def test_subsystem_electrical_grid_catalog_8() -> bool:
    mgr = ElectricalGridCatalogService8Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = ElectricalGridCatalogService8Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_electrical_grid_catalog_8()
    print(f"electrical_grid_catalog Part 8: {'PASSED' if ok else 'FAILED'}")
