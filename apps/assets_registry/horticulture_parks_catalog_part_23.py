...
Module: Public Parks, Gardens & Tree Canopy Registry (Part 23)
...
@dataclass
class HorticultureParksCatalogService23Record:
    """Certified Municipal Domain Record for Public Parks, Gardens & Tree Canopy Registry."""
    record_id: str = field(default_factory=lambda: str(uuid.uuid4()))
    entity_code: str = "horticulture_parks_catalog_23"
...
class HorticultureParksCatalogService23Engine:
    """
...
class HorticultureParksCatalogService23Workflow:
    """
    ...
    def __init__(self, record: HorticultureParksCatalogService23Record):
        self.record = record
...
class HorticultureParksCatalogService23Manager:
    """
    Service registry and data aggregator for Public Parks, Gardens & Tree Canopy Registry.
    """
    def __init__(self):
        self.store: Dict[str, HorticultureParksCatalogService23Record] = {}
        self.engine = HorticultureParksCatalogService23Engine()

    def create(self, op_index: float = 50.0) -> HorticultureParksCatalogService23Record:
        rec = HorticultureParksCatalogService23Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[HorticultureParksCatalogService23Record]:
...
def test_subsystem_horticulture_parks_catalog_23() -> bool:
    mgr = HorticultureParksCatalogService23Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = HorticultureParksCatalogService23Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_horticulture_parks_catalog_23()
    print(f"horticulture_parks_catalog Part 23: {'PASSED' if ok else 'FAILED'}")
