...
Module: Transformers, Substations & Poles Registry (Part 14)
...
class ElectricalGridCatalogService14Engine:
    """
...
class ElectricalGridCatalogService14Workflow:
    """
    ...
    def __init__(self, record: ElectricalGridCatalogService14Record):
        self.record = record
...
class ElectricalGridCatalogService14Manager:
    """
    Service registry and data aggregator for Transformers, Substations & Poles Registry.
    """
    def __init__(self):
        self.store: Dict[str, ElectricalGridCatalogService14Record] = {}
        self.engine = ElectricalGridCatalogService14Engine()

    def create(self, op_index: float = 50.0) -> ElectricalGridCatalogService14Record:
        rec = ElectricalGridCatalogService14Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[ElectricalGridCatalogService14Record]:
...
def test_subsystem_electrical_grid_catalog_14() -> bool:
    mgr = ElectricalGridCatalogService14Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = ElectricalGridCatalogService14Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_electrical_grid_catalog_14()
    print(f"electrical_grid_catalog Part 14: {'PASSED' if ok else 'FAILED'}")
