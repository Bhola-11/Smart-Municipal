...
Module: Municipal Primary & Secondary Schools Registry (Part 20)
...
class EducationSchoolsCatalogService20Engine:
    """
...
class EducationSchoolsCatalogService20Workflow:
    """
    ...
    def __init__(self, record: EducationSchoolsCatalogService20Record):
        self.record = record
...
class EducationSchoolsCatalogService20Manager:
    """
    Service registry and data aggregator for Municipal Primary & Secondary Schools Registry.
    """
    def __init__(self):
        self.store: Dict[str, EducationSchoolsCatalogService20Record] = {}
        self.engine = EducationSchoolsCatalogService20Engine()

    def create(self, op_index: float = 50.0) -> EducationSchoolsCatalogService20Record:
        rec = EducationSchoolsCatalogService20Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[EducationSchoolsCatalogService20Record]:
...
def test_subsystem_education_schools_catalog_20() -> bool:
    mgr = EducationSchoolsCatalogService20Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = EducationSchoolsCatalogService20Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_education_schools_catalog_20()
    print(f"education_schools_catalog Part 20: {'PASSED' if ok else 'FAILED'}")
