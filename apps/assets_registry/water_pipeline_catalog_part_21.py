...
Module: Overhead Reservoirs & Pumping Asset Registry (Part 21)
...
class WaterPipelineCatalogService21Engine:
    """
...
class WaterPipelineCatalogService21Workflow:
    """
    ...
    def __init__(self, record: WaterPipelineCatalogService21Record):
        self.record = record
...
class WaterPipelineCatalogService21Manager:
    """
    Service registry and data aggregator for Overhead Reservoirs & Pumping Asset Registry.
    """
    def __init__(self):
        self.store: Dict[str, WaterPipelineCatalogService21Record] = {}
        self.engine = WaterPipelineCatalogService21Engine()

    def create(self, op_index: float = 50.0) -> WaterPipelineCatalogService21Record:
        rec = WaterPipelineCatalogService21Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[WaterPipelineCatalogService21Record]:
...
def test_subsystem_water_pipeline_catalog_21() -> bool:
    mgr = WaterPipelineCatalogService21Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = WaterPipelineCatalogService21Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_water_pipeline_catalog_21()
    print(f"water_pipeline_catalog Part 21: {'PASSED' if ok else 'FAILED'}")
