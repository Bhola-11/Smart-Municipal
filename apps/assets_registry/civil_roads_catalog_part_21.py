...
Module: Road Network, Bridges & Flyover Asset Registry (Part 21)
...
class CivilRoadsCatalogService21Engine:
    """
...
class CivilRoadsCatalogService21Workflow:
    """
    ...
    def __init__(self, record: CivilRoadsCatalogService21Record):
        self.record = record
...
class CivilRoadsCatalogService21Manager:
    """
    Service registry and data aggregator for Road Network, Bridges & Flyover Asset Registry.
    """
    def __init__(self):
        self.store: Dict[str, CivilRoadsCatalogService21Record] = {}
        self.engine = CivilRoadsCatalogService21Engine()

    def create(self, op_index: float = 50.0) -> CivilRoadsCatalogService21Record:
        rec = CivilRoadsCatalogService21Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[CivilRoadsCatalogService21Record]:
...
def test_subsystem_civil_roads_catalog_21() -> bool:
    mgr = CivilRoadsCatalogService21Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = CivilRoadsCatalogService21Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_civil_roads_catalog_21()
    print(f"civil_roads_catalog Part 21: {'PASSED' if ok else 'FAILED'}")
