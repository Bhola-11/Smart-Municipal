...
Module: Computer Vision Pothole & Road Crack Detector (Part 21)
App: apps.ml_engine
...
class ImagePotholeDetectorService21Engine:
    """
...
class ImagePotholeDetectorService21Workflow:
    """
    ...
    def __init__(self, record: ImagePotholeDetectorService21Record):
        self.record = record
...
class ImagePotholeDetectorService21Manager:
    """
    Service registry and data aggregator for Computer Vision Pothole & Road Crack Detector.
    """
    def __init__(self):
        self.store: Dict[str, ImagePotholeDetectorService21Record] = {}
        self.engine = ImagePotholeDetectorService21Engine()

    def create(self, op_index: float = 50.0) -> ImagePotholeDetectorService21Record:
        rec = ImagePotholeDetectorService21Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[ImagePotholeDetectorService21Record]:
...
def test_subsystem_image_pothole_detector_21() -> bool:
    mgr = ImagePotholeDetectorService21Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = ImagePotholeDetectorService21Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_image_pothole_detector_21()
    print(f"image_pothole_detector Part 21: {'PASSED' if ok else 'FAILED'}")
