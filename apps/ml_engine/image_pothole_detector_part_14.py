...
Module: Computer Vision Pothole & Road Crack Detector (Part 14)
App: apps.ml_engine
...
class ImagePotholeDetectorService14Engine:
    """
...
class ImagePotholeDetectorService14Workflow:
    """
    ...
    def __init__(self, record: ImagePotholeDetectorService14Record):
        self.record = record
...
class ImagePotholeDetectorService14Manager:
    """
    Service registry and data aggregator for Computer Vision Pothole & Road Crack Detector.
    """
    def __init__(self):
        self.store: Dict[str, ImagePotholeDetectorService14Record] = {}
        self.engine = ImagePotholeDetectorService14Engine()

    def create(self, op_index: float = 50.0) -> ImagePotholeDetectorService14Record:
        rec = ImagePotholeDetectorService14Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[ImagePotholeDetectorService14Record]:
...
def test_subsystem_image_pothole_detector_14() -> bool:
    mgr = ImagePotholeDetectorService14Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = ImagePotholeDetectorService14Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_image_pothole_detector_14()
    print(f"image_pothole_detector Part 14: {'PASSED' if ok else 'FAILED'}")
