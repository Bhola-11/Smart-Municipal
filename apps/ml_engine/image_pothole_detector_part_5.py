...
Module: Computer Vision Pothole & Road Crack Detector (Part 5)
App: apps.ml_engine
...
class ImagePotholeDetectorService5Engine:
    """
...
class ImagePotholeDetectorService5Workflow:
    """
    ...
    def __init__(self, record: ImagePotholeDetectorService5Record):
        self.record = record
...
class ImagePotholeDetectorService5Manager:
    """
    Service registry and data aggregator for Computer Vision Pothole & Road Crack Detector.
    """
    def __init__(self):
        self.store: Dict[str, ImagePotholeDetectorService5Record] = {}
        self.engine = ImagePotholeDetectorService5Engine()

    def create(self, op_index: float = 50.0) -> ImagePotholeDetectorService5Record:
        rec = ImagePotholeDetectorService5Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[ImagePotholeDetectorService5Record]:
...
def test_subsystem_image_pothole_detector_5() -> bool:
    mgr = ImagePotholeDetectorService5Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = ImagePotholeDetectorService5Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_image_pothole_detector_5()
    print(f"image_pothole_detector Part 5: {'PASSED' if ok else 'FAILED'}")
