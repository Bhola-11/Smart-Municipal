...
Module: Computer Vision Pothole & Road Crack Detector (Part 4)
App: apps.ml_engine
...
class ImagePotholeDetectorService4Engine:
    """
...
class ImagePotholeDetectorService4Workflow:
    """
    ...
    def __init__(self, record: ImagePotholeDetectorService4Record):
        self.record = record
...
class ImagePotholeDetectorService4Manager:
    """
    Service registry and data aggregator for Computer Vision Pothole & Road Crack Detector.
    """
    def __init__(self):
        self.store: Dict[str, ImagePotholeDetectorService4Record] = {}
        self.engine = ImagePotholeDetectorService4Engine()

    def create(self, op_index: float = 50.0) -> ImagePotholeDetectorService4Record:
        rec = ImagePotholeDetectorService4Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[ImagePotholeDetectorService4Record]:
...
def test_subsystem_image_pothole_detector_4() -> bool:
    mgr = ImagePotholeDetectorService4Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = ImagePotholeDetectorService4Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_image_pothole_detector_4()
    print(f"image_pothole_detector Part 4: {'PASSED' if ok else 'FAILED'}")
