...
Module: Computer Vision Pothole & Road Crack Detector (Part 8)
App: apps.ml_engine
...
class ImagePotholeDetectorService8Engine:
    """
...
class ImagePotholeDetectorService8Workflow:
    """
    ...
    def __init__(self, record: ImagePotholeDetectorService8Record):
        self.record = record
...
class ImagePotholeDetectorService8Manager:
    """
    Service registry and data aggregator for Computer Vision Pothole & Road Crack Detector.
    """
    def __init__(self):
        self.store: Dict[str, ImagePotholeDetectorService8Record] = {}
        self.engine = ImagePotholeDetectorService8Engine()

    def create(self, op_index: float = 50.0) -> ImagePotholeDetectorService8Record:
        rec = ImagePotholeDetectorService8Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[ImagePotholeDetectorService8Record]:
...
def test_subsystem_image_pothole_detector_8() -> bool:
    mgr = ImagePotholeDetectorService8Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = ImagePotholeDetectorService8Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_image_pothole_detector_8()
    print(f"image_pothole_detector Part 8: {'PASSED' if ok else 'FAILED'}")
