...
Module: Computer Vision Pothole & Road Crack Detector (Part 20)
App: apps.ml_engine
...
class ImagePotholeDetectorService20Engine:
    """
...
class ImagePotholeDetectorService20Workflow:
    """
    ...
    def __init__(self, record: ImagePotholeDetectorService20Record):
        self.record = record
...
class ImagePotholeDetectorService20Manager:
    """
    Service registry and data aggregator for Computer Vision Pothole & Road Crack Detector.
    """
    def __init__(self):
        self.store: Dict[str, ImagePotholeDetectorService20Record] = {}
        self.engine = ImagePotholeDetectorService20Engine()

    def create(self, op_index: float = 50.0) -> ImagePotholeDetectorService20Record:
        rec = ImagePotholeDetectorService20Record(operational_index=op_index)
        self.store[rec.record_id] = rec
        self.engine.record_sample(op_index)
        return rec

    def get(self, record_id: str) -> Optional[ImagePotholeDetectorService20Record]:
...
def test_subsystem_image_pothole_detector_20() -> bool:
    mgr = ImagePotholeDetectorService20Manager()
    r = mgr.create(88.0)
    assert r.operational_index == 88.0
    wf = ImagePotholeDetectorService20Workflow(r)
...
if __name__ == "__main__":
    ok = test_subsystem_image_pothole_detector_20()
    print(f"image_pothole_detector Part 20: {'PASSED' if ok else 'FAILED'}")
